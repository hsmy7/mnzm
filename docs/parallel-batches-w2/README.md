...
| 基线实测值（勿凭记忆；**2026-09-14 §2.58 收口批后实测**） | ActionId **170**（maxId=**1733**，实用段 1520–1531 / 1550–1559 / 1570–1573 / 1590–1594 / 1610–1616 / 1630–1632 / 1650–1657 / 1670–1672 / 1680–1682 / 1690–1693 / 1710–1712 / **1730–1733**；§2.58 **零新增**）；桌面 C++ **1322/1322**；引擎全量 **3260 用例 / 290 类 / 1 失败 / 0 跳过**（余 `DiffYearSettlementTest` AI 招募逐字段分歧，见 handover §2.58.7）；`:app` **1018 用例 / 1 失败 / 2 跳过**（余 `SpriteCodegenSyncTest` 生成物，纹理批次）；`:core:data` 全绿；`:feature:game` **868 用例 / 10 失败**（两族预存夹具）；`execute_dispatch.cpp` handler **33** 个。**🔴 勘误：原写「并行阻塞项 = 纹理重构族在途破损 ⇒ lint / NDK / 模块回归三关不可走」——该断言已不成立**（2026-09-14 实测核实）：`NativeBridge.cpp:623/625/1499/1500` 用的是**全局** `KtxInfo`/`loadKtx1`（无 `ktx1::` 限定符）；`VulkanRenderBackend.kt:224/230` 的 `hasAnyCliffTexture`/`cliffTextureCount` 已在 `NativeSurfaceView.kt` 提供、`MainGameScreen.kt` 的 `textureMask` 已接线⇒ `:feature:game` 编译已恢复（本批实测 868 用例全部可跑）。**⚠️ 另需勘误「detekt 全绿」**：`main` 实跑 `./gradlew detekt` 报 **25 处活违规**（`core:engine` 15 + `:feature:game` 10；baseline 表确为全 0 ⇒ 未被 baseline 覆盖的活违规）。§2.58 已清偿 15 处，余 `:feature:game` 10 处（纹理/浮空岛批次）——详见 handover §2.58.8 |
