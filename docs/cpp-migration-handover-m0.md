...
批次: M0 主体 | 硬规格: release ndk debugSymbolLevel SYMBOL_TABLE；astcenc 缺失由静默跳过改 GradleException fail；新增 detekt-baseline-count.guard + ci.yml "Detekt baseline must not grow"；P0-3 清屏色已核实无需改（VulkanBackend.cpp:2352 已纯黑；🔧 勘误标注 2026-09-17/D6 补 §7.2-5：行号现值 **:2733**——"★ 纯黑清屏"注释处）
...
坑: **顺手根治真实移植缺陷**——`secret_realm_settlement.h` 的 `kOpenYears` **原值 50 误取 `COOLDOWN_YEARS`**，Kotlin 权威值 `GameConfig.SecretRealm.OPEN_YEARS = 5` ⇒ AUTHORITATIVE 月结到期判定"秘境现世满 5 年后还要再挂 45 年"，与 Kotlin 回退臂口径分裂；改为 5 并附勘误注释，到期判定单源锚定 `secret_realm_cfg::kOpenYears`（🔧 勘误标注 2026-09-17/D6 补 §7.2-2：**"单源"未彻底且至今如此**——`secret_realm_settlement.h:53` 与 `secret_realm.h:81` 双常量并存（均 =5，两命名空间各自消费，`secret_realm_settlement.h:199` 仍用自持常量）；值相等无行为差，收敛为单一常量未实施，维持低危技术债登记）
...
### §2.75 W4-D/D4 w3-13 反向通道删除（终局）——阶段 A+B 观察窗交付 + 删除步阻断判定（2026-09-15）

批次: [W4 剩余工作实施文档](parallel-batches-w4/remaining-work-implementation.md) §0 序 D4（串行链第四项）| 性质: **观察窗仪器 + 判定批——零生产行为变更（传输默认照常）、零协议面、零 ActionId 变更、零 C++ 改动**；按 [ADR reverse-channel-elimination](adr/reverse-channel-elimination.md) §4 安全下线五步推进，五步走完前三步（禁用 → 完整业务周期验证 → 指纹零差异），第四/五步（归档删除）按规程判定**阻断**并登记完成路径。

**① 阶段 A 观察窗仪器（"先禁用"——信封体积 = 0 但代码在）**:
- `ReverseChannelPolicy` 新增 `reverseTransportEnabled` 停发开关（默认 `true` = 现状传输；`setReverseTransportEnabled` 为测试/观察窗专用入口，仪器随删除批一并移除）；
- `StateSyncService.sendReverseEnvelope` 停发分支：信封照常**构建**（关闭域写入检测保持存活——禁用不得掩盖漏域写者），但**不发送** C++（传输体积 = 0 可观测）；版本号、AI 池与锁定集变化检测缓存、反向锚点均不推进（与"C++ 未收到任何字节"语义一致）；捕获窗口照常消费（防无界累积）。debug 构建每次停发一行 `DomainLog.d`（观测面）。

**② 阶段 B 观察窗验证（仪器进 CI，可复跑）**——新增 2 用例（引擎 3305 → 3307）:
- `DiffAuthoritativeTickTest.authoritative pipeline runs clean with reverse transport disabled`（100 旬 AUTHORITATIVE 全管线 × 停发态，与主对拍用例同场景同构）：三闸门全绿——① 零信封发送；② 关闭域写入检测零命中；③ 终态全量结构对拍逐位一致 + 时间线（y4m7）/年俸扣减/年报 3 份不变量保持（结算周期、前向镜像与窗口消费/锚点停推/缓存停更路径全被踩过，停发态无退化）。
- `ReverseChannelCloseoutTest.observation window - disabled transport sends nothing while capture and detection stay alive`（窗口语义单元面：零发送 + 检测存活 + 窗口消费）。

**③ 删除步判定 = 🔴 阻断（诚实口径，按五步规程"观察窗内任何意外活动即重置流程"处置）**:
- **硬前置不成立**（w3 README §4）："ReverseChannelPolicy 关闭清单 = 全部传输单元（无'在册保留'项）"——当前 `transportedGameDataFields`（四分片在册保留并集）+ 弟子通道 + 9 类实体集合 + `aiSectDisciples` 段仍开放传输；
- **AUTHORITATIVE 稳态 Kotlin 写者实测在位**（删通道 = 这些写入永不达 C++，被前向镜像以 C++ 侧值覆盖 = 玩家可见数据丢失）：
  1. **交谈效果** `DiscipleDelegate.kt:238 applyConversationEffects` → `GameEngineCoordination.kt:99 updateDisciple`——弟子行 `cultivation`/`skills`/`statusData` 直改，无 native 臂（W4-A A5 证据明记"写入面留待弟子通道关闭决策（W4-D）"）；C++ 结算/年变老化重写同弟子行 ⇒ 前向镜像按行覆盖 = 交谈增益一年内被清；
  2. **任务派遣/完成** `GameEngineMissionOps.kt:26 startMission`（`activeMissions` 追加 + 槽位释放）与 `:44 checkAndProcessCompletedMissions`（完成结算 + `activeMissions` 收缩）——无 native 臂；C++ 月结子事件 13 同写该字段 ⇒ 冲突；
  3. **存档前自愈** `SaveFacadeImpl.kt:56 regenerateSectsBeforeSave`——`worldMapSects`/`sectRelations`/`aiSectDisciples` 会话中途写者；C++ AI 流同读同写 ⇒ 冲突；
  4. **守卫红线在位**：`ReverseChannelPolicyGuardTest.audit red line`——弟子通道与 `aiSectDisciples` 段必须保持传输（关闭即测试红）。
- **生产行为零变更得证**：开关默认 `true`（传输照常），停发仅存在于 CI 用例内部；本批引擎全量用例数 = 基线 + 2（纯新增用例），其余 3305 用例零漂移。

**④ 完成路径（删除步重启条件，按依赖序；重启后走五步④归档 tag `w3-13-pre-delete` + bundle → ⑤ 删除）**:
1. **弟子通道收口批**：交谈效果写面下沉——C++ 列式行写事务（段 **1860–1869** 预留段，README §6 "W4-D 机动"）；效果增量已由引擎侧 `RngPartition.CHAT` 签发（A5 已达规 R1/R3），事务零 RNG、参数化应用 ⇒ 抽取序零变更、无拍板负担；完成后 `DISCIPLE_CHANNEL` 关闭（守卫红线断言同步翻转）；
2. **任务域收口**：`startMission`/完成结算 native 臂（或 C++ 任务事务族）→ `activeMissions` 关闭；
3. **存档自愈收口**：`regenerateSectsBeforeSave` 改"自愈后全量重建基线"（复用 ADR 保留面 `importToNative`——读档/新档基线路径，存档前一次性 O(状态) 成本可接受）或 C++ 世界重生事务 → `aiSectDisciples` 段 + `worldMapSects` 关闭；
4. **retained 字段逐域判定**（D2 §2.73 同款"逐条判定"格式）：钱包三阶/年度收支账/收藏与预警/设置项族——C++ 已在位者转 closedUnits；真 Kotlin-only 非协议面登记豁免口径；9 类集合与 `battleTeams`/`productionSlots` 等运行态字段按域逐一裁决；
5. 全部传输单元关闭（`ReverseChannelPolicyGuardTest` 穷尽分类守卫自动验证"关闭清单 = 全字段"）→ 删除批落地：删除面 = `captureReverseDirty`/`ReverseDirtyAccumulator`/`applyDirtyToNative` + 反向信封构建/`consumeReverseDirty`/`GameCore::applyReverseDirty` + JNI 导出/`ReverseChannelPolicy` 本体；保留面 = 前向镜像 + 全量导入 + 各域回退臂；防复发 = 镜像只读契约（结构性/编译期约束优先）+ "稳态零写入断言"转正式门禁。

**测试**: 新增 2 用例（引擎 **3307** = 3305 + 2；`Diff*` 47 类全绿，新增停发用例为第 48 个对拍场景形态但同类内非独立类）。

**门禁实跑**: 桌面 C++ 全量**豁免**（本批零 C++ 改动——纯 Kotlin 策略/测试面）｜`:core:engine` **3307 / 304 类 / 0 失败 / 0 错误 / 0 跳过**（= §2.74 基线 + 2；47 `Diff*` 全绿含停发 100 旬用例，JNI 指向本树 desktop-jni 重建产物）｜`:core:domain` **1758/0**（`ReverseChannelPolicyGuardTest` 6/6）｜六模块 detekt 全绿｜主源 + 测试源编译绿（六模块全量）｜`:app:externalNativeBuildRelease` + `:app:lintRelease` **豁免**（零 C++/资源/Manifest 改动）｜生成器幂等 + 生成物 `git diff --exit-code` 空（**196 动作 / maxId=1843**）。

**遗留**: 删除步重启 = 上述完成路径 1–5（属各域收尾批，逐批独立可验收）；观察窗仪器随删除批移除；`w4-rem/05` tag + bundle 已落盘。


### §2.76 W4-D/D4 续·弟子通道收口（第一段）：交谈效果写面下沉（2026-09-15）

批次: handover §2.75④ 完成路径**第 1 项**（弟子通道收口·交谈写面下沉）| ActionId: **1860 `DISCIPLE_CHAT_EFFECT_TX`**（段 1860–1869 首个实裁；**197 动作 / maxId=1860**）| 性质: **玩家可见语义零变更、双臂抽取增量恒 0 的写者下沉批**。

**审计先行（完成路径修正登记——诚实口径）**: 通道整体关闭的阻断面经逐点核实比 §2.75④ 原登记更宽：
1. `startMission`（`GameEngineMissionOps.kt:26`）内 `releaseDiscipleToIdleInside` 写弟子**槽位/状态协议列** ⇒ `DISCIPLE_CHANNEL` 关闭还依赖任务域收口（原第 2 项，非可选项）；
2. 月/年残留执行器的 `lifeEvents` **协议外列投影**（购买日志/丧亲，`MonthSettlementResidualExecutor.appendPurchaseLog` / `YearSettlementResidualExecutor.appendBereavementEvent`）是 AUTHORITATIVE 稳态 Kotlin 行写者，且 `DiscipleTables.bindAllOnWrite` 对**全部列**（含协议外列）绑定 `changedIdTracker` ⇒ 通道关闭后行级"关闭域写入检测"将对协议外投影误报——关闭批须一并裁决（检测 AUTHORITATIVE 门控 / 投影改非捕获路径，二选一，届时拍板）。

据此本批收窄为"交谈写面下沉"本体，通道关闭（守卫红线翻转）顺延至任务域收口批。

**① 新事务 `DISCIPLE_CHAT_EFFECT_TX=1860`**:
- 新 `chat_effect_tx.h`（纯 C++20 零 Android 依赖）——语义与 Kotlin `updateDisciple` lambda **逐位一致**：① 弟子不存在 = 成功无操作（`return@update` 同语义，found=false 非失败信封）；② 修为 `max(0.0, cultivation+delta)`；③ 道德/忠诚/悟性 `(原值+delta)` clamp [1,100]；④ `statusData["lastChatYear"]` 冷却标记恒写（与增量是否为零无关）；⑤ 已故行照常应用（Kotlin 无存活检查同语义）。行序机制差异（Kotlin remove+insert 移末 vs C++ 原位列写）沿 W4-A `DISCIPLE_OP_*` 家族既定先例（C++ 行序 = AUTHORITATIVE 真相源，保序）。
- 🔴 **RNG 红线（红线 1）**：事务**零抽取**——交谈决策抽取（树/分支/效果随机化）已由 W4-A·A5 收敛引擎侧 `RngPartition.CHAT`（`GameEngineConversationDraw`），增量数值作为事务参数传入；双臂抽取增量恒 0（GTest rngStates 快照差分恒零锚定）。
- 分发：`dispatch_w4d.cpp` 新 handler（参数校验 → `INVALID_PARAMS` 失败信封 → Kotlin 回退臂）；`w4d.mjs` 目录 +1；`dispatch_guard_test` 对新号"分派可达"自动覆盖。

**② Kotlin native 臂**:
- `GameEngineCoordination.applyConversationEffectAtomic`——C++ 真相先行（`tryDiscipleOpNative(1860)`）；失败信封/降级 null → `updateDisciple` 回退臂（红线 3 不变）；**非数字 id 不尝试 C++ 臂**（协议为整数 id），Kotlin `toInt()` 原抛出 + delegate 捕获语义保持。
- `DiscipleDelegate.applyConversationEffects` 改调新入口（launchOnEngine 协程边界/异常翻译不变）。

**③ 测试**: 新 GTest `chat_effect_tx_test.cpp` **7 用例**（成功路径逐位 / clamp 边界 100·1·0.0 / 缺弟子无操作零写入 / 零增量冷却标记 / 已故行 / 零抽取锚点 / 端口形状含缺参 INVALID_PARAMS）；新 `DiscipleChatEffectNativeTxGateTest` **3 用例**（回退臂逐位 / 修为下限+静默无操作 / flag OFF vs 降级 AUTHORITATIVE 两臂等价——GuideRewardNativeTxGateTest 同口径，JVM 守护降级臂）。

**门禁实跑**: 桌面 C++ **1424/1424**（1417 + 7；ctest + 单进程直跑复核）｜`:core:engine` **3310 / 305 类 / 0 失败 / 0 跳过**（3307 + 3；47 `Diff*` 全绿）｜`:feature:game` **872/0**（delegate 触碰面复跑）｜六模块 detekt 全绿｜主源 + 测试源编译绿｜`:app:externalNativeBuildRelease` + `:app:lintRelease` 绿（C++ 触碰必跑）｜生成器幂等 + 生成物零漂移（**197 动作 / maxId=1860**）。

**遗留**: ① 任务域收口批（`startMission`/完成结算 native 臂 → `activeMissions` 关闭）+ `DISCIPLE_CHANNEL` 关闭（红线翻转）+ 检测门控/投影裁决；② 其后按 §2.75④ 第 3/4 项推进；`w4-rem/06` tag + bundle 已落盘。


### §2.77 W4-D/D4 续·任务域收口 + 弟子通道关闭（2026-09-15）

批次: handover §2.76 完成路径剩余项（任务域收口 + `DISCIPLE_CHANNEL` 关闭）| ActionId: **1861 `MISSION_START_TX`**（段 1860–1869；**198 动作 / maxId=1861**）| 性质: **玩家可见语义零变更、双臂抽取增量恒 0 的写者下沉 + 通道关闭批**。

**① 审计结论（写者收敛）**: 任务域 AUTHORITATIVE 可达写者收敛为 `startMission` 一个——`checkAndProcessCompletedMissions`（`GameEngine.kt:160` missionCheck → `GameEngineCorePausOps4.kt:136/:146`）在 native 分支虽被调用，但 C++ 月结子事件 5 已完成任务并经前向镜像同步 ⇒ 其跑在镜像上是**防御性 no-op**（零写入）。

**② 新事务 `MISSION_START_TX=1861`**（`mission_start_tx.h` 纯 C++20 零 Android 依赖）:
- 语义与 Kotlin `startMission` 事务体逐位一致：availableMissions 按 id 查模板（不存在 = `MISSION_NOT_FOUND` 失败信封零写入；Kotlin 回退臂按原语义继续——原路径不校验可用性）→ 逐队员 `clearAllSlotsDataOnly(includeResidence=false)`（11 类槽位清理保留住所，复用 slot_cleanup.h，与 appointment_tx 同款拷入/拷出）→ 状态重置 IDLE（REFLECTING 剥离 reflectionStart/EndYear、REFINING 剥离 buildingId）→ ActiveMission 模板快照（队员 id/name/realmName——C++ `realmName` 同源映射）→ `activeMissions` 追加。
- 🔴 **零 RNG**：`ActiveMission.id` 在 Kotlin 为 `UUID.randomUUID()`（Java 随机非游戏分区——原基线即零抽取），由调用方生成后作参数传入；缺行队员静默跳过清理但保留成员位（Kotlin 同语义）。
- 分发：`dispatch_w4d.cpp` 新 handler（缺参 `INVALID_PARAMS`）；`dispatch_guard_test` 自动覆盖新号可达。
- Kotlin 臂：`GameEngineMissionOps.startMission` 首行 `tryDiscipleOpNative(1861)`（非数字 id 不尝试 C++ 臂）；失败信封/降级 → 原路径回退臂；事务外部平台面（assignmentGate 释放/Room Repository 清理/syncAllDiscipleStatuses）两臂同形照原序。

**③ `DISCIPLE_CHANNEL` 关闭（守卫红线翻转）**:
- **关闭单元登记**：`W4DChannelClosures` 新增 `discipleChannel(DISCIPLE)` + `activeMissions` 转 `closedUnits`（原 W4-D retained 条目转出）；`ChannelClosureEntries` 新增 `discipleChannel()` 条目助手。
- **守卫红线翻转**：`ReverseChannelPolicyGuardTest.audit red line` 改为双断言——弟子通道**必须关闭**（写者收口完成）；`aiSectDisciples` 段**仍保持传输**（存档前自愈 `regenerateSectsBeforeSave` / 攻宗守军清理等写者在位，§2.75④ 第 3 项未完成）。穷尽分类守卫自动验证 activeMissions 转出后无遗漏。

**④ 配套裁决（§2.76 审计先行登记两项的落地）**:
- **lifeEvents 协议外投影 → 非捕获路径**：月/年残留执行器事务（生产 `GameEngineCoreMonthOps.kt:93` / `GameEngineCoreYearOps.kt:126` + harness `DiffAuthoritativeTickTest.runNativeBoundary` 两处）由 `update` 改 `updateMirror`——购买日志/丧亲/秘境关闭草稿均为 **C++ 结算事实的 Kotlin 显示投影**（协议外 lifeEvents 列 + 邮件 DAO），无需回导；`updateMirror` 与 `update` 唯一差异即捕获豁免（`GameStateStoreImpl.executeUpdateTransaction(captureReverse=false)`），行为零变更。
- **捕获侧关闭域写入检测 AUTHORITATIVE 门控**：`GameStateStoreImpl.captureReverseDirty` 弟子通道/集合两分支 + 测试 `FakeGameStateStore.captureReverse` 同源——flag-OFF 下无 C++ 真相源（写入即真相），不存在回导缺口，不计数；AUTHORITATIVE 下漏域写者照常可观测。信封侧检测（`detectClosedFieldWrites`）本就仅在 AUTHORITATIVE 回导路径可达，不改。

**⑤ 机械面测试恢复传输语义**: `GameStateStoreReverseDirtyTest`（:app 捕获机械）/ `StateSyncServiceReverseTest`（信封机械）/ `ReverseChannelVolumeProfileTest`（体积口径）经 `reopenDomain(DISCIPLE)` 覆盖钩子恢复通道传输前提，继续守护捕获→changed id→快照→信封各段机械；关闭语义新覆盖 = 红线翻转断言 + 检测门控用例（CloseoutTest 观察窗/关闭集合两用例置 `withMode(AUTHORITATIVE)`）。harness 三用例 Side B（flag-OFF 基准臂）显式 `withMode(OFF)` / Side A `withMode(AUTHORITATIVE)`——模式语义与生产对齐。

**⑥ 测试**: 新 GTest `mission_start_tx_test.cpp` **6 用例**（模板快照逐位/槽位清理+住所保留+状态重置/未知任务零写入/缺行队员/零抽取锚点/端口形状含 INVALID_PARAMS 与 MISSION_NOT_FOUND）+ 新 `MissionStartNativeTxGateTest` **2 用例**（回退臂逐位：activeMissions+槽位清理+住所保留+状态重置；两臂降级等价）。

**门禁实跑**: 桌面 C++ **1430/1430**（1424 + 6；ctest + 单进程直跑复核）｜`:core:engine` **3312 / 306 类 / 0 失败 / 0 跳过**（3310 + 2；47 `Diff*` 全绿含停发用例，重建桌面 JNI 后复跑）｜`:core:domain` **1758/0**（守卫红线翻转后 6/6 绿）｜`:app` **1003/0**｜`:feature:game` 零触碰（872/0 沿 §2.76 实测）｜六模块 detekt 全绿｜`:app:externalNativeBuildRelease` + `:app:lintRelease` 绿（C++ 触碰必跑）｜生成器幂等 + 生成物零漂移（**198 动作 / maxId=1861**）。

**遗留**: ① `aiSectDisciples` 段 + `worldMapSects` 关闭 = §2.75④ 第 3 项（存档自愈收口）；② retained 字段逐域判定（第 4 项）；③ 集合段九类与 `recruitList` 等运行态字段随第 4 项逐一裁决；④ 全部传输单元关闭后重启五步④⑤（通道删除）；`w4-rem/07` tag + bundle 已落盘。


### §2.78 W4-D/D4 续·存档自愈收口——aiSectDisciples 段 + worldMapSects 关闭（2026-09-15）

批次: handover §2.75④ **第 3 项**（存档自愈收口）| 性质: **零协议面、零 ActionId 变更、零 C++ 改动、零玩家可见变更**。

**① 写者穷尽审计（closure 集 = aiSectDisciples 段 + worldMapSects）**:
- worldMapSects AUTHORITATIVE 中途写者 = 存档自愈（`SaveFacadeImpl.regenerateSectsBeforeSave`——仅 world 为空/缺玩家宗门时触发）/ 攻宗占领（`GameEngineBattleOps.occupySectRewards`：占领 + garrison + aiSectDisciples 清池 + recruitList 俘虏 + sectDetails.isOwned）/ 宗门升级回退臂（`tryNativeSectLevelUpgrade` 失败分支——native 臂 batch-19 SECT_LEVEL_UPGRADE_TX 直写 C++）/ 完整性修复（`ensureGameDataIntegrity` 经 `upgradeSectLevel` 中途可达）；boot/读档归一化（`BootSequenceController`/`GameEngineLoadDataOps`/StorageEngine 存档路径 = LOAD_BOOT 族，首旬全量导入吸收）；`removeDeadDefenders` native 臂（batch-20b）；遭遇战（`EncounterBattleService:381`）与好感事件（`FavorEventProcessor:92`）写者挂月结子事件表 = flag-OFF 回退臂（检测 AUTHORITATIVE 门控不计数）；C++ AI 流写面经前向镜像。
- aiSectDisciples 段写者 = 上述并集 − 升级回退臂 + `GameEngineLifecycleOps:130` 池修复（integrity 内，被 ③ 覆盖）。
- **保留（不在本批关闭）**：`sectRelations`（遭遇战 `:261` + 好感事件 `:52` 活写者——外交域收口工作）；`activeSectId`（enterSect 净化条件写者 + LOAD_BOOT）/ `sectName`（LOAD_BOOT 族）——审计结论登记，下一轮清偿扫除随域判定转关闭。

**② re-baseline 原语（§2.75④ "自愈后全量重建基线"落地）**: `GameEngine.rebaselineNativeMirror(reason)`（`GameEngineCoreAuthoritativeOps`）——`importToNative(restoreRng=false)` 全量重建 C++ 基线（ADR 保留面 sanctioned 复用；restoreRng=false 保持 native RNG 真相源）；`GameCoreBridge` 未加载/未初始化时静默跳过（boot 基线由首旬 `ensureAuthoritativeNative` 全量导入吸收已修复状态）。事件均为低频用户/存档动作，O(状态) 一次性成本可接受（WS-1 实测口径 ≈100 弟子下数十 ms 级）。

**③ 四处接线**: ① `SaveFacadeImpl` 注入 `StateSyncService`（DI 构造注入，无手工构造面），自愈更新后重建；② `ensureGameDataIntegrity` 尾（覆盖 upgradeSectLevel 中途修复路径；boot 两处调用无害跳过）；③ `occupySectRewards` 尾（update 事务外）；④ 宗门升级回退臂分支（native 臂成功路径无需——C++ 已直写，避免以写入前镜像覆盖 C++ 产出）。

**④ 关闭登记**: `W4DChannelClosures` 新增 `topLevelSection(AI_SECT, SECTION_AI_SECT_DISCIPLES)` + `gameDataField(AI_SECT, "worldMapSects")`（retained 条目同批转出）；`ReverseChannelPolicyGuardTest.audit red line` 三连翻转——弟子通道 + aiSectDisciples 段 + worldMapSects **必须 closed**（穷尽分类守卫自动验证 retained 转出后无遗漏）。`sectRelations`/`activeSectId`/`sectName` 保留并在 retained 注释登记审计结论。

**测试**: 无新增用例（关闭语义由翻转后的守卫断言承载；re-baseline 为 native-only 路径——JVM 无 .so 不可达，桌面 JNI + 引擎全量回归守护；全量用例数与基线完全一致 = 测试面外零行为变更得证）。

**门禁实跑**: 桌面 C++ 全量**豁免**（零 C++ 改动）｜`:core:engine` **3312 / 306 类 / 0 失败 / 0 跳过**（§2.77 基线持平）｜`:core:domain` **1758/0**（守卫 6/6 绿，三连翻转断言）｜`:app` **1003/0**｜六模块 detekt 全绿 + `:app:lintRelease` 绿｜`:app:externalNativeBuildRelease` **豁免**（零 C++/资源/Manifest 改动）｜生成器幂等 + 零漂移（198 动作 / maxId=1861）。

**遗留**: ① `sectRelations` 关闭 = 外交域收口（遭遇战/好感事件写者下沉或 re-baseline 接线）；② `activeSectId`/`sectName` 下一轮清偿扫除随域判定转关闭；③ 9 类实体集合 + retained 字段族（钱包/年度收支账/recruitList/生产槽位等）= §2.75④ 第 4 项；④ 全部传输单元关闭后重启五步④⑤（通道删除）；`w4-rem/08` tag + bundle 已落盘。


### §2.79 W4-D/D4 续·retained 字段族逐域判定第一段 + 关闭域写者补漏（2026-09-15）

批次: handover §2.75④ **第 4 项第一段**（retained 字段逐域判定，D2 §2.73 同款"逐条判定"格式）+ **§2.77/§2.78 关闭域漏网写者补漏** | 性质: **零协议面、零 ActionId 变更、零 C++ 改动**；玩家可见语义零变更（写入语义逐字保形迁移 + 写入后基线重建）。

**① 审计先行（三路并行 `git grep` 穷尽 + 逐点核对，审计修正两处旧登记）**:
- **retained 字段族全量重审**：§2.78 收口时在册保留的 40 字段（W4-A 13 + W4-B 21 + W4-C 13 − 已转出）+ 9 类实体集合逐域穷尽写者审计，分类 = native 臂就位后的回退臂 / LOAD_BOOT 族 / flag-OFF 结算臂 / LIVE（阻断关闭）四类。
- **审计修正 1（§2.78 登记错误）**：`sectName` 原 retained 注记"LOAD_BOOT 族"**证伪**——`SectDelegate.kt:38 renameSect` 是稳态写者（UI 改名动作），且它同时写 `worldMapSects`（§2.78 已关闭）⇒ **§2.78 的 worldMapSects 关闭存在漏网写者**：AUTHORITATIVE 下改名会被关闭域写入检测计数、且经前向镜像被 C++ 侧旧值覆盖 = 改名静默丢失。
- **审计修正 2（§2.77 漏网写者）**：`resetAllDisciplesStatus`（设置"重置弟子"）写 `worldMapSects`/`activeMissions`（均已关闭）+ 弟子协议列（通道已关闭）；`releaseDiscipleFromAllSlotsAtomic`（长老分配前清理/释放流程）与 `cancelBloodRefinement` 写弟子协议列；赏赐/服药**偷盗判定钩子**（`applyTheftHookResidual`，native 臂成功后按"执法域不下沉"原序执行完整执法链）写弟子 `lastTheftJudgementYears`/`statuses` + 集合（偷盗取走）——这些在 §2.77 通道关闭后已构成真实回导缺口（写入被镜像覆盖 = 数据丢失面），同批补漏。
- **审计修正 3（域判定纠偏）**：`annualTheftCount`/`annualDesertedDisciples`/`theftJudgementsThisMonth`/`gameEventRecords`/`manualProficiencies` 的 LIVE 状态由偷盗钩子维持（执法域不下沉既有登记）——保持保留；`pendingPatrolBattleResults` 的迎战弹窗队列 append/drain 为稳态写者——保持保留。

**② 补漏接线（8 处，全部复用 §2.78 `rebaselineNativeMirror` 原语）**:
1. **宗门改名迁入引擎层**：新增 `GameEngine.renameSect`（`GameEngineLifecycleOps.kt`，写 `sectName` + `worldMapSects` 玩家宗门名后 rebaseline）；`SectDelegate.renameSect` 改为委派（UI 收尾不变）。
2. **仓库驻守卸任迁入引擎层**：新增 `GameEngine.removeWarehouseGarrison`（`GameEngineWarehouseOps.kt`，`updateGameDataAndSync` + gate 释放逐字保形 + rebaseline）；`ProductionViewModel.removeWarehouseGarrison` 改委派。
3. **enterSect 收敛**：`GameEngineLifecycleOps.kt` 归一化发生时（条件性幂等，未发生零成本）rebaseline——解除 `activeSectId`/`placedBuildings`/`spiritMineSlots` 的写者阻断。
4. **设置重置**：`DiscipleStatusService.resetAllDisciplesStatus` 事务改 `updateMirror`（**非捕获**——捕获侧 presence 检测对合法重置误报；弟子协议列属通道级关闭，值比较不可用）+ `GameEngineDiscipleOps.kt:22` 包装层 rebaseline。
5. **槽位释放**：`GameEngineDiscipleSlotOps.releaseDiscipleFromAllSlotsAtomic` 同款 `updateMirror` + rebaseline。
6. **取消血炼**：`GameEngineBloodRefinementOps.cancelBloodRefinement` 同款 `updateMirror` + rebaseline。
7. **偷盗判定钩子**：`DiscipleOpsNativeTxForward.applyTheftHookResidual` 事务改 `updateMirror` + `GameEngineCore.rebaselineNativeMirror`（新增 Core 侧变体，与 GameEngine 侧同一 StateSyncService 实例）——执法域写面（弟子列 + 集合 + 执法堂字段）整段经基线重建推入 C++。
8. **内存裁剪**：`GameEngineServiceOps.releaseMemory` CRITICAL 分支裁剪发生时 rebaseline（裁剪语义 = 双侧释放内存）。

**③ 关闭登记（29 项 retained → closedUnits，守卫穷尽分类自动验证）**:
- RECRUIT：`recruitList`（占领 rebaseline 吸收 + 回退臂/旗臂/LOAD_BOOT）。
- AI_SECT：`activeSectId`、`sectName`（enterSect/改名接线后余者 LOAD_BOOT）。
- DIPLOMACY：`shownWarningStageIds`（1843 回退臂-only）、`vassalContracts`、`suzerainSectId`（占领吸收 + VASSAL_TX 回退臂 + 旗臂 + `establishVassalage` 死 API）。
- INVENTORY：玉符四字段（`jadeSymbols`/`jadeSymbolsToday`/`jadeAccumMs`/`jadeDayAnchorMs`——JadeSymbolService 全部写入点 native 臂先行）。
- PATROL：`patrolConfigs`、`spiritMineSlots`、`patrolSlots`、`residenceSlots`（槽位事务族 native 臂在位 + ② 接线）。
- SAVE_LOAD：`terrainTiles`、`mapGenVersion`（唯一 Kotlin 写者 = boot 回填，仅 `mapSeed≠0` 触发且与 C++ `ensureTerrainGenerated` 同源恒等——W4-C 当初的"回填误报"担忧随 D3 等价性证明消解；`mapSeed=0` 老档两端恒空无丢失面）。
- SECRET_REALM：`secretRealmState/Session/AITeams/CooldownYear` + `caveExplorationTeams`、`aiCaveTeams`（1800/1801/1442 族回退臂-only + ② 接线）。
- DISCIPLE：`elderSlots`、`librarySlots`、`warehouseGarrisons`、`battleTeams`、`activeBloodRefinements`（槽位事务族 + ② 接线）。
- BUILDING：`placedBuildings`（enterSect 接线后余者回退臂/LOAD_BOOT）。PRODUCTION：`spiritFieldPlants`（灵田事务族全 native 臂/旗臂/LOAD_BOOT，零接线需求）。
- **第二段（保持保留，附守卫断言防抢跑）**：钱包三阶+灵草、年度收支账族、执法堂三项、`usedRedeemCodes`、`watchedItemIds`、`gameEventRecords`、`manualProficiencies`、`productionSlots`（align/placeSlots 两处 W4-A 登记偏差）、W4-C 战斗世界域四字段、`sectRelations`（遭遇战 + 宗门交易 LIVE）、`pendingPatrolBattleResults`、`autoBuyList`/`mailRecords`/`heavenlyTrialState`（登记不下沉族）与 9 类实体集合——写者下沉/接线后逐域转关闭。

**④ 死链登记（D5 清单）**: `BuildingService.assignDiscipleToBuilding` 链（`BuildingService.kt:111` ← `BuildingFacadeImpl.kt:138` ← `GameEngineBuildingOps.kt:18` ← `DiscipleDelegate.kt:94`）——末级 delegate 方法零界面调用方，整链死代码；`VassalService.establishVassalage`、`FavorServiceImpl.updateFavor/modifyFavor` 同批登记。

**测试**: 引擎新增 `GameEngineSectIdentityOpsTest` **3 用例**（改名只更新 sectName + 玩家宗门 / 同名仍写入 / 仓库卸任移除条目 + gate 释放——Robolectric JVM 面，rebaseline 走 GameCoreBridge 短路）；domain 新增守卫 2 用例（**第一段 29 项不得恢复传输** + **第二段保留项写者下沉前不得关闭**）；`ReverseChannelCloseoutTest` 样本字段 `jadeSymbols`（已关闭）→ `spiritStones`（第二段保留代表）。`GameViewModelTest.renameSect` 三用例改断言"委派引擎层"（语义守卫随迁引擎测试类）。

**门禁实跑**: 桌面 C++ 全量**豁免**（零 C++ 改动）｜`:core:engine` **3315 / 0 失败 / 0 错误 / 0 跳过**（3312 + 3；47 `Diff*` 全绿 270 用例，JNI 指向本树 desktop-jni）｜`:core:domain` **1760/0**（1758 + 2 守卫）｜`:feature:game` **872/0**｜`:app` **1003/0/0（2 既有跳过）**｜六模块 detekt 全绿（**顺带清偿两处前批漏网违规**：`ChannelClosureEntries.kt:40` MaxLineLength、`DiscipleDelegate.kt:18` UnusedImports——§2.76/§2.77 交付时 detekt 缓存未拦下）｜`:app:lintRelease` 绿｜`:app:externalNativeBuildRelease` **豁免**（零 C++/资源/Manifest 改动）｜生成器幂等 + 生成物零漂移（198 动作 / maxId=1861）。
...
### §2.81 W4-D/D4 续·retained 字段族逐域判定第三段（终段）——9 类实体集合转关闭，w3-13 删除步硬前置全量达成（2026-09-15）

批次: handover §2.80③ 遗留 **第 4 项第三段（终段）** | 性质: **零协议面、零 ActionId 变更、零 C++ 改动、玩家可见语义零变更**（宿主事务 updateMirror 化 + 尾部/双臂基线重建）。本批完成后 **w3-13 五步规程硬前置"关闭清单 = 全部传输单元"全量达成**，删除步（五步④归档 tag → ⑤删除）就绪。

**① 审计先行（三代理并行 + 逐点核对，约 90 调用点 / 25 文件穷尽）**: 集合通道与 gameData 字段的本质差异 = 捕获侧 presence 检测（`captureCollection` 引用变化即计数，§2.80 已定性）⇒ 值等值收敛不适用，**稳态写者的宿主事务必须先转非捕获**（尾部 rebaseline 对 gameData 字段够用、对集合通道不够用）。判定结果：AUTHORITATIVE 稳态**可达且捕获型**仅 6 簇 7 处；其余全部落六类合法形态——
1. **native 臂回退臂**（检测 AUTHORITATIVE 门控不计数）：库存 UI 操作面卖/用/锁/消耗/购买/合并/整理/充公（1010–1029/1520/1521/1525/1530/1531，门控在 `GameEngineInventoryOps` wrapper 或 facade 内）、弟子操作面赏赐消耗品/服药（1740–1759）、灵田种植扣种（spirit_field）、逐出 Kotlin 原路径（1590 回退臂）；
2. **flag-OFF 月年结臂**（C++ 直辖）：自动购买（C++ 子事件 10 `merchant_settle::executeAutoBuy`）、任务奖励发放（子事件 5/13）、生产完成 4a/4b（炼丹/锻造七系统扇出）、灵田收获、CultivationEvent 奖励、巡逻月度战利品；
3. **LOAD_BOOT 族**：新档初始储物袋（`GameEngineLoadDataOps.kt:435`）、读档收获（`checkAndCollectCompletedSlots`）；
4. **AUTHORITATIVE 防御性 no-op**：`GameEngineMissionOps.grantMissionInventoryRewards`（C++ 完成经前向镜像后无完成项可结算，§2.80 免接线判定 1 同款口径）；
5. **对拍基准专属**（零生产调用）：`PhaseSettlementExecutor` 族的 `CultivationEventAutoWarehouseOps`（:268–292）、`AutoPillService`（:93）、`BuildingService` 惰性收获 UI 入口（`HarvestProductionUseCase` 零生产调用方）；
6. **局部副本**（非全局集合写）：秘境背包（`SecretRealmRuinsResolver`/`SecretRealmBattleHelper`）、任务奖励生成（`MissionSystemRewardOps`）、AI 编队/敌人生成（`AISectTeamComposer`/`EnemyGenerator`）、开袋暂存批（`InventoryFacadeImplBagOps`）。

**② 接线（6 簇 7 处，宿主事务 updateMirror 化；集合写入经 addXxx 重入非捕获）**:
1. **攻宗占领**：`GameEngineBattleOps.occupySectRewards` `update`→`updateMirror`（rebaseline :378 既有）——事务内 `grantWarRewardsInside` 写 6 类集合；
2. **攻宗碾压**：`crushSectRewards` 同款（rebaseline 调用方 :111 既有）；败北分支无集合写入不动；
3. **世界关卡胜利奖励**：`GameEngineWorldBattleOps.applyVictoryRewards` 奖励段整体包裹 `updateMirror`（`handleBeastLevelVictory`/`handleCaveLevelVictory` 去 `suspend`——体内无挂起点；`addSpiritStones` 异步钱包写维持原样，钱包字段属值比较检测 §2.80 语义）；双臂尾部 rebaseline（:52/:84）既有；
4. **逐出袋物化残差**：`DiscipleLifecycleNativeTx.tryNativeExpelDisciple` 物化段包裹 `updateMirror`（rebaseline :101 既有）；
5. **仓库赏赐装备/功法**（唯一需新增 rebaseline 的簇）：`DiscipleFacadeImpl功法Ops1.rewardEquipment/rewardManual` `update`→`updateMirror` + **新增条件性 rebaseline**（`wrote` 标记，早退零成本）——本链**无 native 臂**（C++ `rewardItemTx` 仅支持 pill/material/herb/seed 四类，扩类型 = 协议改动），Kotlin 即 AUTHORITATIVE 正主；UI 入口 `WarehouseDiscipleSelectDialog:301`，与已下沉的消耗品四链形成的不对称以此收口；
6. **宗门贸易购买**：`DiplomacyService.buyFromSectTradeSync` `update`→`updateMirror`（原 :746 注释"集合……或经捕获"的过渡口径终结；rebaseline :748 既有）；
7. **妖兽迎战奖励**：`ExplorationServiceBeastRaidOps.resolveBeastAttackFight` `update`→`updateMirror`（战利品 `addMaterial` + 败北掠夺写面全入；`GameEngine.kt:272`"妖兽迎战" + `GameEngineExplorationNativeOps.kt:186`"遭遇战分支"双入口 rebaseline 既有；月度自动防守臂走月结事务不受影响）。

**③ 关闭登记（9 项 COLLECTION → closedUnits）**: `ChannelClosureEntries` 新增 `collectionUnit` 助手；`W4DChannelClosures.w4DClosedUnits` 追加 `collectionUnit(INVENTORY, …)` ×9（equipmentStacks/equipmentInstances/manualStacks/manualInstances/pills/materials/herbs/seeds/storageBags）+ INVENTORY 域证据一段。
- 🔴 **里程碑（终局）**：**w3-13 删除步硬前置"关闭清单 = 全部传输单元"全量达成**——gameData 字段面（§2.80 `transportedGameDataFields` = 空集）+ 顶层段（`lockedBeastIds` batch-23 / `aiSectDisciples` §2.78）+ 弟子通道（§2.77）+ 9 类实体集合（本段）；无任何在册保留传输面。`ReverseChannelPolicyGuardTest` 里程碑断言翻转为终局形态（`w3-13 hard precondition met - every transport unit is closed`：四类单元逐一断言关闭）+ 新增第三段红线（9 集合不得恢复传输）。**下一步 = 五步④归档 tag `w3-13-pre-delete` + bundle → ⑤通道删除**（删除面/保留面/防复发见 §2.75④ 第 5 项）。

**测试**: 守卫翻转 2 用例（domain 1761→1762）；`ReverseChannelCloseoutTest`"在册保留集合仍回导"用例翻转终局语义（`all collections closed - envelope carries no collection section`：关闭后捕获载荷为空 ⇒ 信封零发送 + AUTHORITATIVE 下内置审计检测登记——不经 override，实证内置审计结论）；机械面 fixture 补 `reopenDomain(INVENTORY)` 恢复传输前提 3 处（`ReverseChannelVolumeProfileTest`/`StateSyncServiceReverseTest`/`GameStateStoreReverseDirtyTest`）+ `GameStateStoreReverseDirtyTest` 的 override 型用例开头 `resetSwitches()`（消除夹具 reopen 对 override 的优先级覆盖）。
**运行时实证**: 引擎全量含 100 旬停发对拍（闸门②"关闭域写入检测零命中"）在 9 集合关闭后保持绿——AUTHORITATIVE 结算管线无集合写入，与穷尽审计互证。

**门禁实跑**: 桌面 C++ 全量**豁免**（零 C++ 改动）｜`:core:engine` **3315 / 0 失败 / 0 错误 / 0 跳过**（持平；47 `Diff*` 全绿 270 用例 + 停发 100 旬闸门绿，JNI 指向本树 desktop-jni）｜`:core:domain` **1762/0**（1761 + 1 净：里程碑翻转 + 第三段红线）｜`:feature:game` **872/0**｜`:app` **1003/0/0（2 既有跳过）**｜六模块 detekt 全绿｜`:app:lintRelease` 绿｜`:app:externalNativeBuildRelease` **豁免**（零 C++/资源/Manifest 改动）｜生成器幂等 + 生成物零漂移（198 动作 / maxId=1861）。

**遗留（w3-13 终局）**: ① 五步④——归档 tag `w3-13-pre-delete` + bundle（本批随 docs 提交落盘）；② 五步⑤——通道删除本体（`captureReverseDirty`/`ReverseDirtyAccumulator`/`applyDirtyToNative` + 反向信封构建/`consumeReverseDirty`/`GameCore::applyReverseDirty` + JNI 导出/`ReverseChannelPolicy` 本体；保留面 = 前向镜像 + 全量导入 + 各域回退臂；防复发 = 镜像只读契约 + 稳态零写入断言进长期 CI），属下一个大批次；③ D5 死链清册新增：`DiscipleFacade.assignManual/removeManual`（facade learnManual/forgetManual 复制链零生产调用方）、`EquipItemUseCase`、`HarvestProductionUseCase`（零生产调用方，仅测试引用）——随既有 D5 清单逐条删除。`w4-rem/11` tag + bundle 已落盘。

### §2.82 W4-D/D4 终局——w3-13 反向通道删除（五步⑤落地，通道不复存在）（2026-09-17）

批次: handover §2.75④ 第 5 项 = **五步⑤删除本体**（前置 §2.76–§2.81 写者收口全量达成 + `w3-13-pre-delete` 归档 tag 已落）| 性质: **纯删除批——零新增行为、零协议面、零 ActionId 变更**；玩家可见语义零变更（§2.80 已证 AUTHORITATIVE 稳态窗口恒空 = 通道零流量）。

**① 删除面（与 §2.75④ 第 5 项清单一一对应）**:
- **Kotlin 捕获链**：`GameStateStoreImpl.ReverseDirtyAccumulator` + `captureReverseDirty`/`captureCollection` + `consumeReverseDirty`/`resetReverseAccumulator` 两 override；`GameStateStore` 接口的 `ReverseDirtySnapshot`/`CollectionCapture`/两默认方法整段删除。
- **信封/发送链**：`StateSyncService` 删除约 470 行——`applyDirtyToNative`/`sendReverseEnvelope`/`buildReverseEnvelope`/`appendGameDataSections`/`appendAiSectDisciplesSection`/`appendDiscipleSection`/`appendCollectionSections`/`reverseGameDataPatch`/`anchorReverseGameData`/关闭字段检测族（`closedFieldKnown`/`detectClosedFieldWrites`/`refreshClosedFieldBaseline`/`updateClosedFieldBaseline`/`jsonValuesEquivalent`）/信封埋点 `logReverseEnvelopeProfile`（batch-22a 信封面）+ 构造器 `reverseSender` 参数 + `reverseVersion`/两个变化检测缓存/两个锚点字段；`syncFromNative`/`importToNative`/`applySnapshot` 的锚点与基线刷新副作用一并清除。
- **策略本体**：`ReverseChannelPolicy.kt` + `reversechannel/` 包五文件（`ChannelClosureEntries`/`W4{A,B,C,D}ChannelClosures`）**整包删除**——关闭清单/逐域回滚/关闭域写入检测/观察窗开关（`reverseTransportEnabled`）随通道失去存在意义。
- **tick 集成**：`GameEngineCoreAuthoritativeOps` 删步骤⑤（反向回导 + 全量降级 + `reverse` 分段）与 `ensureAuthoritativeNative` 的 `resetReverseAccumulator`；`GameEngineRoadOps` 删 `syncRoadChangeToNative`（AUTHORITATIVE 稳态道路走 native 事务臂 + 前向镜像回流，回退臂下 Kotlin 即真相源——回导本无必要）。
- **C++ 面**：`GameCore::applyReverseDirty`（`game_core.cpp`）+ 匿名命名空间辅助簇四函数（`upsertEntities`/`removeEntities`/`applyCollectionUpsert`/`applyCollectionRemove`——穷尽 `applyReverseDirty` 专属）+ `reverseVersion_` 成员与 `importStateInternal` 版本归零；JNI 双导出删（生产 `GameCoreBridge_nativeApplyReverseDirty` + 对拍 `DiffRngBridge_nativeCoreApplyReverseDirty`）+ Kotlin `external` 双声明删；GTest `apply_reverse_dirty_test.cpp`（13 用例）删 + `test/CMakeLists.txt` 摘除。
- **保留面（原样）**：前向镜像（`applyDirtyFromNative`/`applySnapshot`/`syncFromNative`）+ 全量导入（`importToNative(restoreRng)`）+ `rebaselineNativeMirror` 原语 + `updateMirror` 入口 + 各域 native 事务回退臂。

**② 防复发（§2.75④ 第 5 项两道门禁落地）**:
1. **镜像只读契约（符号面守卫）**：新增 `MirrorReadOnlyGuardTest`（core:engine 测试，`RngSourceGuardTest` 同款源码扫描题型）——六模块主源（含 `app` 的 C++ JNI 面）对通道符号族（`ReverseChannelPolicy`/`ReverseDirty`/`consumeReverseDirty`/`resetReverseAccumulator`/`applyDirtyToNative`/`applyReverseDirty`）**零命中**；命中即 = 删除步被局部回退，须按 ADR §4 重新立项。
2. **稳态零写入断言（行为面门禁，观察模式转正式硬门禁）**：`FakeGameStateStore` 新增 `nonMirrorWriteCount`（**非镜像入口** update/updateAndReturn/modifyState 提交的持久状态变更计数，AUTHORITATIVE 门控——flag-OFF 写入即真相不计数）；`DiffAuthoritativeTickTest` 100 旬 AUTHORITATIVE 全管线主用例尾部断言 `storeA.nonMirrorWriteCount == 0`。**变红处置 = 下沉 native 事务或改 updateMirror + rebaseline 接线，禁止删除/放宽断言**（与 detekt-baseline-count.guard 同纪律）。生产 `GameStateStoreImpl.updateMirror` 保留独立入口（委托 `update`，事务语义一致）——命名约定即契约载体（updateMirror = C++ 真相源投影，update = Kotlin 游戏写入）。

**③ 测试面**：删除 5 个反向测试类——`ReverseChannelPolicyGuardTest`（domain 10 用例；硬前置已达成、通道已删，审计清单完成历史使命）、`ReverseChannelCloseoutTest`（8）/`ReverseChannelVolumeProfileTest`（4）/`StateSyncServiceReverseTest`（11）（信封与窗口语义随通道消亡）、`GameStateStoreReverseDirtyTest`（8，app）；C++ `apply_reverse_dirty_test.cpp` 13 用例。`DiffAuthoritativeTickTest` 观察窗停发用例删除（停发仪器已随通道移除，其闸门②由稳态零写入断言接替）；`DiffStateSyncTest` 反向信封 aiSectDisciples 变化检测用例删除；`GameEngineRoadOpsTest` 瘦身为放置/删除/占格语义（回导断言面消亡）；`RoadFacadeImplTest` 构造去 `reverseSender` lambda。

**④ 门禁基线漂移登记（诚实口径）**: §3 基线表此前记"桌面 1417"为 §2.73 时点值，§2.76/§2.77（C++ 触碰批）实跑已达 **1424 → 1430** 而基线表未同步（两批各自小节有记录）——本批删除 13 用例后桌面实测 **1417**，与表值巧合相等，已按 1430 − 13 修注，消除"桌面数不变"的误读。

**门禁实跑**: 桌面 C++ **1417/1417**（= 1430（§2.77 实测）− 13 反向用例；ctest + 单进程直跑复核）｜`:core:engine` **3288 / 0 失败 / 0 错误 / 0 跳过**（= 3315 − 25 反向用例 − 观察窗用例 + DiffStateSyncTest 反向用例 + 1 守卫；47 `Diff*` 全绿 + **100 旬管线稳态零写入断言绿**——删除步后 AUTHORITATIVE 管线 Kotlin 非镜像持久写入为零的行为面实证）｜`:core:domain` **1752/0**（1762 − 10 守卫用例）｜`:app` **995/0（2 既有跳过）**（1003 − 8）｜`:feature:game` **872/0**｜`:core:data` **716/0/15 既有跳过**｜六模块 detekt 全绿（baseline 全 0；本批顺带清偿 3 处新增违规——UnusedPrivateProperty/UnusedImports/MaxLineLength 实修，未进 baseline）｜主源 + 测试源编译绿（六模块）｜`:app:externalNativeBuildRelease` + `:app:lintRelease` 绿（C++ 触碰必跑）｜生成器幂等 + 生成物零漂移（198 动作 / maxId=1861，本批零协议变更）｜JNI 导出 ↔ Kotlin external **38 ↔ 38**（`docs/cpp-engine.md` §9 计数已同步）。

**遗留**: ① D5 死代码清零（§2.D 清单 + §2.81③ 新增死链）——下一串行项；② D6 文档与版本收口（`version.properties` 由用户决定）；③ 真机 E3/N5（反向信封体积/耗时）观测面**随通道删除取消**（§4.1 相应注销——通道已不存在，无信封可采样）；④ WS-1 再评估阈值改为只看"真机每旬镜像 >100ms"（D1 镜像分段埋点保留，不受本批影响）。




### §2.83 W4-D/D5——死代码清零 + 守卫面收口（2026-09-17）

批次: [W4 剩余工作实施文档](parallel-batches-w4/remaining-work-implementation.md) §0 序 D5 + §2.D 全清单 | 性质: **零 C++ 改动、零协议面、零 ActionId 变更、玩家可见语义零变更**（纯删除 + 守卫数值只缩）。

**① 先补清单（D5 硬前置）**: handover §5④ 的"18+11+5 处零调用者站点"原始分项清单在仓库内不可考（§14 #15 已定性"无基准不可验收"）⇒ 本批以**全量重扫 + 逐点核实**建立新基准，产出三份可核对清单（每条带"生产调用 n / 测试引用 n"证据）：

**A. 已由前批清偿（本批逐项核实，免做）**:
| 项 | 清偿批次与证据 |
|---|---|
| 洞府探索生命周期入口死链 | W4-B/B4（`CaveExplorationProcessor` 构造面收敛注释 + 委托段已空；`processSectDisciplesAging`/`processAISectOperations`/`currentAiThermalBatchSize`/`processSectDisciplesYearlyRecruitment` 四活路逐一核实带生产调用方） |
| `GameEnginePatrolOps.updatePatrolConfig` 死 API | W4-B（现存仅复数版 `updatePatrolConfigs` 活版；`PatrolTowerViewModel.updatePatrolConfig` 为同名不同函数） |
| `InventorySystem.materializeDiscipleBagAndMarkDead` 同签名遮蔽 | W4-C 根治（`InventorySystem校验Ops3.kt:163` 根治注释） |
| `GameEngineSectLevelOps` `bloodMaterials.random()` 输入侧随机流入 C++ 事务（§2.D ⑧ / §12 债表） | W4-B/B0 已分区化（`:210` 现走 `RngPartition.MAIL`，注释在案；§12 债表行同步销账） |
| `CultivationEventAutoWarehouseOps` 悬空 KDoc（§2.D ②） | §2.68 已修 |
...
| B5 | `EquipItemUseCase` / `HarvestProductionUseCase`（`:app` usecase，零生产调用方） | 0 | `UseCaseInvocationTest` 两段（同步删） |
| B6 | `AISectPersonality.random()` + `randomDenounceInterval` + 失去唯一消费者的 `weightedPool` | 0 | `AISectPersonalityTest` 4 用例（同步删，余 8 个枚举/权重/门槛用例保留） |
| B7 | `PillGrade.random()` 无参重载（裸 `Random.Default`；生产炼丹品阶已走分区 RNG 重载） | 0 | `ProductionProcessorAlchemyTest` 概率分布用例（同步删） |
| B8 | `BaseTemplateRegistry.pickWeightedRandom`/`generateWeightedRarity`/`generateTieredRarity`（后者两个零生产调用方；前者的**唯一生产调用方** = B9 两函数，链条闭合后整体死） | 0 | `BaseTemplateRegistryTest` 4 用例 + 2 个暴露 protected 的测试辅助（同步删） |
| B9 | `BeastMaterialRegistry.getRandomMaterialByRealm`/`getRandomMaterialByBeastType`（实例类两随机函数，全仓零调用方——生产掉落走的是 `BeastMaterialDatabase` **object** 同名函数） | 0 | 0 |
| B10 | `BeastMaterialDatabase.getRandomMaterialByRealm`（裸 `Random.Default` 加权抽取） | 0 | `BeastMaterialDatabaseTest` 1 用例（同步删） |

**C. 核实后保留（诚实口径——A5 存量并非全死）**: `BeastMaterialDatabase.getRandomMaterialByBeastType`（裸 `Random.nextDouble()` 加权抽取）有 **3 个生产调用方**（`GameEngineWorldBattleOps:322` / `ExplorationService:364` / `PatrolBattleSystem:595`，妖兽材料掉落）⇒ **非死代码，本批不删**；其"全局随机不可复现"属 **RNG 阶段 3 分区化**范畴（与 `MaterialTemplateRegistry.generateRandom` 家族同批），已登记阶段 3 清单。A5 当初"8 处均仅测试调用方"的表述对其中 2 处（B8/B10 及其上游）不准确，以本清单为准。

**② 守卫面收口（守卫只缩不增 + 计数断言落地）**:
- `RngSourceGuardTest` core/domain BARE_DRAW 登记上限 **13 → 6**（= 原基线 5 + 存活 1；A5 显形 8 处中 7 处随 B6–B10 删除，余 1 处为 C 项活代码并附归属说明）；
- `RngEngineIsolationGuardTest` 白名单 **4 → 1**（删 3 条陈旧"同族遗留"豁免——三处 `var xxxRngManager` 已随 W4-C C-③ 改形参必传，生产面 0 命中核实）+ **新增"白名单条目数必须等于登记值"计数断言**（`expectedExclusionCount = 1`；§12 债表"白名单无计数断言"项同步销账——新增豁免从此机器可拦）。

**③ 门禁实跑**: 桌面 C++ 全量**豁免**（零 C++ 改动）｜`:core:engine` **3288 / 0 / 0 / 0**（= 3288 − 1 炼丹用例 + 1 白名单计数断言；47 `Diff*` 全绿 0 跳过，JNI 指向本树 desktop-jni）｜`:core:domain` **1743/0**（= 1752 − 9：settings 1 + with* 3 + registry 4 + byRealm 1）｜`:core:data` **716/0/15 既有跳过**｜`:feature:game` **872/0**｜`:app` **986/0/2 既有跳过**（= 995 − 9：personality 4 + usecase 5）｜六模块 detekt 全绿（3 处 UnusedImports 删除批残留实修，未进 baseline）｜NDK + lint **豁免**（零 C++/资源/Manifest 改动）｜生成器幂等 + 生成物零漂移（198 动作 / maxId=1861）。

**遗留**: ① D6 文档与版本收口（`version.properties` 由用户决定）；② RNG 阶段 3 的 `BeastMaterialDatabase.getRandomMaterialByBeastType` 分区化已登记阶段 3 清单（唯一存活裸抽取）。
...
- **§7.2/§7.3 勘误补齐**（[W4 README §1.1](parallel-batches-w4/README.md) 移交收口人的欠账）：6 项存量口径按"勘误标注不重写历史"格式在原位追加（`VulkanBackend.cpp` 清屏色现值 `:2733`；`IslandCliffTextureLoader` SwallowedException 现值 `:177` 复核一致；`§2.58.2`"六个扩展函数"实为 8 个；§2.51 `kOpenYears` 双常量复核**仍并存**——值相等无行为差，收敛维持低危技术债登记未实施；§2.53 46/82 站点明细权威落档处 = ui-read-surface §4.4；`rng_manager.h` 已迁 `rng/rng_manager.h`，`exportStates` 现值 **:88**）+ §7.3 三处未登记稳态写者全部标记已收口（`replaceManual` → §2.62 / `approveMarriageProposal` → §2.62.2 / `forceSettleDisciplesBeforeBattle` → §2.64.4）+ §7.4 ②⑤⑥ 三项标注已取消/已裁决/已定论。
- **[cpp-engine.md §9](cpp-engine.md)**：计数表权威指针从"169 动作 / maxId=1734"更正为 **198 动作 / maxId=1861**（生成器实跑口径；§2.82 删除批零协议变更）。
...
**遗留**: ① `version.properties` 是否递增——**用户决定**（本批未动）；② W4 波收官后可派工清单**为空**，剩余全部属非并行轨（真机验证批需物理设备 / WS-4 需玩法设计文档 / WS-1 已决策不做——见 §4.1/§5）；③ RNG 阶段 3 余量（注册表随机家族 + 挂钟播种两处）与 §7.4 ①③④ 两项未复核登记维持原状。

## 3. 验证结果（当前门禁基线 + 各批数值；未达项与归属见本节末）
**当前门禁基线（2026-09-17，§2.83 D5 死代码清零后）**：

| 验证 | 结果 |
|---|---|
| 桌面 C++ 全量单测 | **1417/1417 全绿**（§2.82 = 1430（§2.77 实测真值）− 13 反向用例；此前本表记 1417 为 §2.73 时点漂移值——§2.76/§2.77 实为 1424/1430 而表未同步，已修注；含单进程直跑复核）；运行需 `llvm-mingw-*-ucrt-x86_64\bin` 在 PATH |
| 引擎全量单测 `:core:engine` | **3288 用例 / 0 失败 / 0 错误 / 0 跳过**（§2.83 = −1 炼丹死函数用例 + 1 白名单计数断言，净持平；47 个 `Diff*` 对拍全绿 0 跳过 + **100 旬管线稳态零写入断言绿**——AUTHORITATIVE 管线 Kotlin 非镜像持久写入为零） |
| `:core:domain` 单测 | **1743 用例 / 0 失败**（§2.83 = 1752 − 9：GameSettingsData 1 + GameData with* 3 + BaseTemplateRegistry 4 + BeastMaterialDatabase 1，随死函数删除） |
| `:core:data` 单测 | **716 用例 / 0 失败 / 0 错误 / 15 跳过（既有）**（§2.83 复跑，持平） |
| `:core:ui` 单测 | **146 用例 / 0 失败**（W4-00 实测值；三批零触碰 `:core:ui`，本波未复跑） |
| `:feature:game` 单测 | **872 用例 / 0 失败 / 0 错误**（§2.83 零触碰持平） |
| `:app` 单测 | **986 用例 / 0 失败 / 0 错误 / 2 跳过（既有）**（§2.83 = 995 − 9：AISectPersonality 4 + UseCase 5，随死代码删除） |
| detekt | ✅ **六模块 `detekt` 全绿，baseline 全 0**（§2.83 复跑；3 处 UnusedImports 删除批残留实修，未进 baseline） |
| 动作计数 | **198 动作，maxId=1861**（`gen-action-ids.mjs` 实跑口径；§2.82 零协议变更） |
...
| **反向信封体积构成** | ~~单元 harness 观测面~~ **已随 §2.82 通道删除取消**（无信封可采样；D1 的每旬镜像分段计时埋点保留，服务 WS-1 再评估阈值） |
...
|---|---|---|
| **§2.83 D5 死代码清零 + 守卫面收口（09-17）** | —（零 C++ 改动，豁免） | **3288 / 0 / 0 / 0**（−1 +1 净持平；47 `Diff*` 全绿 0 跳过；`:core:domain` 1743/0、`:core:data` 716/0/15、`:feature:game` 872/0、`:app` 986/0/2 复跑） |
| **§2.82 w3-13 反向通道删除（终局，09-17）** | **1417 / 1417**（= 1430 − 13 反向用例；ctest + 单进程直跑复核） | **3288 / 0 / 0 / 0**（= 3315 − 26 + 1 守卫；47 `Diff*` 全绿 + 稳态零写入断言绿；`:core:domain` 1752/0、`:core:data` 716/0/15、`:feature:game` 872/0、`:app` 995/0/2 复跑；NDK + lint 绿） |
| **§2.81 retained 字段族第三段（终段）——硬前置全量达成（09-15）** | —（零 C++ 改动，豁免） | **3315 / 0 / 0 / 0**（持平；47 `Diff*` 全绿 + 停发 100 旬闸门绿；`:core:domain` 1762/0、`:feature:game` 872/0、`:app` 1003/0 复跑） |
...
| **§2.76 W4-D/D4 续·交谈写面下沉（09-15）** | **1424/1424**（+7） | **3310 / 305 类 / 0 / 0**（+3；47 `Diff*`） |
| **§2.75 W4-D/D4 阶段 A+B 观察窗（09-15）** | —（零 C++ 改动，豁免） | **3307 / 304 类 / 0 / 0**（+2；47 `Diff*` + 停发 100 旬用例） |
...
| **真机（物理设备）验证残留** | 模拟器会话未覆盖 10 项：A2 ASTC 缺失机 RGBA 回退 / A4 旋屏 / C1 偷盗钩子自然触发 + TapDB 上报 / C3 S5 战斗任务 / C4 S6 秘境全链 / C6 ThermalMonitor 真实热档 / D2 放置确认步 / D3 道路装配 / E2 云存档 / ~~E3 WS-1 绝对值（需先补 debug 埋点小批）~~（**E3 的反向信封体积/耗时观测面已随 §2.82 通道删除取消**——WS-1 再评估阈值只剩"每旬镜像 >100ms"，由 D1 保留的镜像分段埋点承担） |
...
| `GameSettingsData.autoSave` | ✅ **已删除（2026-09-17，§2.83 D5）**——孤儿模型 + `EnumConverters` 悬空 TypeConverter 对 + `AudioConfig.updateFromSettings` 一并清理 |
...
验收门禁 = **Diff 对拍全绿**（~~4 字段被 `DiffAuthoritativeTickTest` 覆写保留传输——该 harness 把 Kotlin 月/年完整编排纳入 AUTHORITATIVE 稳态，为生产超集；**可选清偿**：把 harness 对齐生产（C++ 月结 + Kotlin 残差），届时这 4 字段可重评~~ → **✅ 已清偿（2026-09-15，§2.74 W4-D/D3）**：harness 已对齐生产口径（C++ 月/年结算 + Kotlin 残差），4 个覆写字段（`spiritMineLastSettledMonth`/`annualAlchemyCount`/`availableMissions`/`yearlyReports`）重评后全部转入关闭）。**✅ 主轴完结（2026-09-17，§2.82 `w4-rem/12`）**：写者收口四段（§2.76–§2.81）达成硬前置 → 五步④归档 tag `w3-13-pre-delete` → ⑤通道删除本体交付（捕获/信封/策略本体/C++ `applyReverseDirty`/双 JNI 导出/反向测试族全删；保留前向镜像 + 全量导入 + 回退臂；防复发 = `MirrorReadOnlyGuardTest` 符号面守卫 + 100 旬管线稳态零写入硬断言）。**AUTHORITATIVE 稳态 Kotlin 对 C++ 只读**——后续任何"Kotlin 需变更 C++ 状态"的玩法一律走 native 事务（参照 §2.76 交谈写面下沉定式）。

**② 真机（物理设备）验证批**——§4.1 登记 10 项残留；真机不可得时的替代口径见 §2.39（模拟器 + 产物字符串核验）。

**③ 待拍板 / 待立项**——WS-4 NPC 移动（需玩法设计文档）；~~`TimeSystem.onPhaseTick`（§2.C）与 `PresentationRandom` 一致性口径（§2.B）~~ **两项已实施（2026-09-15，§2.69）**；~~`GameSettingsData.autoSave` 删除~~ **✅ 已实施（§2.83 D5）**。**已决策不做** = WS-1 阶段 3 数据导向存储（2026-09-15 按桌面 Release 实测决策：真实规模 ≈100 弟子下占旬间隔 0.8% 且不占渲染线程 ⇒ 无瓶颈；再评估阈值 >400 弟子或真机每旬镜像 >100ms，见 §4.1）。**已拍板并全部实施** = W4-D 汇流波（`docs/parallel-batches-w4/README.md` §8：~~D1 埋点 → D2 `w3-11` → D3 harness 对齐 → D4 反向通道删除 → D5 死代码清零 → D6 文档收口~~ **D1–D6 已实施（2026-09-15～09-17，§2.72–§2.84，`w4-rem/02`–`14`）——W4 波可派工项全部收官**；`version.properties` 未递增，由用户决定）；WS-5b 地图冻结批 **✅ 已随 W4-C 落地（§2.64.2）**。
...
**④ 死代码滚动清零**（§2.53 审计登记，w3 各批顺手删除或单列清理批）——`GameEngine.updatePatrolConfig` 死 API、洞府探索整族死链、`InventorySystem.materializeDiscipleBagAndMarkDead` 成员/扩展同签名遮蔽、18+11+5 处零调用者站点、`GameData` 四个零调用方辅助函数。**✅ 全部清偿（2026-09-17，§2.83 D5）**——原"18+11+5"聚合数仓库内不可考，全量重扫立新基准（三份带证据清单见 §2.83①）：前批已清偿 5 项核实免做 + 本批删 10 组（含 `GameData` 死函数族与 `updatePatrolConfig`/洞府死链/遮蔽三者的前批清偿核实）；唯一存活裸抽取 `BeastMaterialDatabase.getRandomMaterialByBeastType` 经核实有 3 个生产调用方**非死代码**，归 RNG 阶段 3 分区化清单（[rng-source-inventory §8](rng-source-inventory.md)）。

> **批次文档索引**：[parallel-batches-w2/](parallel-batches-w2/README.md)（batch-11～21，**已全部交付**；除 batch-12/20 保留为设计记录外实施文档已删）｜[parallel-batches-w3/](parallel-batches-w3/README.md)（**已全部交付**：w3-01～w3-12 随 W4-A/B/C 三批落地，w3-13 反向通道删除随 W4-D/D4 终局收官——§2.82）｜[parallel-batches-w4/](parallel-batches-w4/README.md)（**已全部交付**：W4-00 前置批 + W4-A/B/C 三批 + W4-D 汇流波 D1–D6）｜[non-parallel-work.md](parallel-batches-w2/non-parallel-work.md)（真机验证批 / 待拍板 / 立项项——**唯一未收官轨**）。
...
> **剩余**：① ~~`DiffYearSettlementTest` 1 例~~（**✅ §2.59.1 清偿**——夹具快照 9 号键垃圾值触发 C++ 存档续接覆盖，测试侧修复）；② **阶段 3**（决策类按域分批下沉——**弟子侧与战斗侧已交付**：§2.62.1 W4-A/A5 `CHAT` 分区（`DiscipleChatDialog` 5 决策点改引擎侧签发 + 守卫 ② 正则扩面）/ §2.64.1 W4-C/C7 `BattleDescriptionGenerator` 14 处 `.random()` 归零（ID 散列确定性选词，零抽取零分区影响）；**余量登记** = 注册表随机家族（`MaterialTemplateRegistry.generateRandom` 家族 + §2.83 核实存活的 `BeastMaterialDatabase.getRandomMaterialByBeastType`）与 `WorldMapGenerator`/`CaveExplorationSystem` 挂钟播种两处，见 [rng-source-inventory §8](rng-source-inventory.md)）；③ ~~`:feature:game` 两族预存夹具失败~~（**✅ §2.59.2 清偿**）。
...
> ✅ **补齐状态（2026-09-17，W4-D/D6 §2.84）**：6 项全部按"勘误标注不重写历史"处置完毕——
> ① §2.57 原位已有更正后结论（"键 6 不写入导出面、AI 流态载体是 9 号镜像键"）；② §2.51 原位标注
> **双常量至今仍并存**（复核 `secret_realm_settlement.h:53` × `secret_realm.h:81` 均 =5，值相等无行为差，
> 收敛维持低危技术债登记未实施）；③ §2.58.2 原位标注"六个"实为 8 个；④ 46/82 站点明细的权威落档处
> = [ui-read-surface §4.4](ui-read-surface.md)（域级结论表 + 站点级滚动更新，§2.53 原文保留）；
> ⑤ 两处 file:line 已在原位标注现值（`VulkanBackend.cpp` **:2733**、`IslandCliffTextureLoader` **:177** 复核一致）；
> ⑥ `rng_manager.h` 已迁 **`rng/rng_manager.h`**，`exportStates` 现值 **:88**（原文 :61/:86 均已过期）。
> **本表自此关闭**——后续新勘误直接在原位按同一格式标注，不再另立登记表。
...
> ✅ **清偿状态（2026-09-17，W4-D/D6 §2.84）**：三处未登记稳态写者已全部随 W4 三批收口——
> `replaceManual` → **§2.62**（`DISCIPLE_OP_REPLACE_MANUAL` native 臂 `GameEngineManualOps.kt:141`）；
> `approveMarriageProposal` → **§2.62.2**（1592 批准接线 + 1750 拒绝事务）；
> `forceSettleDisciplesBeforeBattle` → **§2.64.4**（战前结算 `1782` 下沉）。**本表无未清偿项。**
...
> **状态追记（2026-09-17，W4-D/D6 §2.84）**：② **已随 §2.82 通道删除取消**（反向信封不复存在，无可采样面；
> WS-1 再评估阈值只剩"每旬镜像 >100ms"，由 D1 保留的镜像分段埋点承担）；⑤ **已裁决（§2.74）**——harness
> 对齐生产后 4 个覆写字段重评并全部转入关闭，"超集"论证随之失效；⑥ **已定论（§2.63.B4 + §2.83① A）**——
> 入口死链属实且已删除（`CaveExplorationRewardOps.kt` 整文件），月结活路（`processSectDisciplesAging`/
> `processAISectOperations`/`currentAiThermalBatchSize`/`processSectDisciplesYearlyRecruitment` 四活路）逐一核实
> 带生产调用方保留。**仍开放**：①③④（均不阻塞派工，维持登记）。
