...
**当前门禁基线（2026-09-15，§2.81 retained 字段族第三段后）**：
...
| `:core:domain` 单测 | **1762 用例 / 0 失败**（§2.81 = 1761 + 1 净（里程碑翻转终局形态 + 第三段 9 集合红线）；`ReverseChannelPolicyGuardTest` 10 用例，含 🔴 **"every transport unit is closed"终局断言**——w3-13 删除步硬前置全量达成） |
...
| 动作计数 | **198 动作，maxId=1861**（`gen-action-ids.mjs` 实跑口径；196 → §2.76 +1860 `DISCIPLE_CHAT_EFFECT_TX` → §2.77 +1861 `MISSION_START_TX`） |
