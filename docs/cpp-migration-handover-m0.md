...
| **WS-1 残留口径 / 阶段 3 立项** | 列级 delta / 二进制通道 + `dirty_tracker` 列级写屏障随**计划 v2 阶段 3 数据导向存储**落地（约 145+ 列写点回归风险）；协议形状变更会影响 47 个 `Diff*` 对拍场景与存档格式。**2026-09-15 桌面实测（§2.68，`dirty_tracker_bench_test.cpp` 实跑）**：每旬全脏 `diffToJson` = **7.9ms@100 / 100.9ms@1000 / 523ms@5000** 弟子；**空闲零变更仍 328ms@5000**（实现恒对全状态做一次全量序列化 + 缓存树深比较 ⇒ 脏标记只减"发出多少"不减"算多少"）；上限口径 `maxDisciples=1000` + 2x 速 1s/旬 ⇒ 满编 ≈ 101ms/秒（≈6 帧）。**建议**：先随 W4-D/D1 拿到真机埋点绝对值，再按"每旬导出 >16ms 且落引擎线程"这一阈值决定是否立项协议 v2；未达阈值前优先考虑"按 UI 读面裁剪镜像载荷 / 廉价指纹预筛"等**非协议**优化 |
...
**③ 待拍板 / 待立项**——WS-4 NPC 移动（需玩法设计文档）；`TimeSystem.onPhaseTick` 与 `GameSettingsData.autoSave` 删除（§4.2）；`PresentationRandom` 一致性口径（§4.2，**含新发现的"文档与实现不符"**）；WS-1 阶段 3 数据导向存储（含 `dirty_tracker` 列级写屏障；**实测数据与决策阈值见 §4.1**）。**已拍板待实施** = W4-D 汇流波（`docs/parallel-batches-w4/README.md` §8：D1 埋点 → D2 `w3-11` → D3 harness 对齐（**含地形 2 字段退出对拍排除面**）→ D4 反向通道删除 → D5 死代码清零 → D6 文档收口）；WS-5b 地图冻结批 **✅ 已随 W4-C 落地（§2.64.2）**。
