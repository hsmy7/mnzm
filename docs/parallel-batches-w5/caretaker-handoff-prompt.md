...
2. **B11 正在施工中**（09:21 经 Qoder 派发，批次文件 `batch-R3B.md` = R3.3 overlay 几何 C++ 生成消 258 drawRect + R3.4 脏更新协议；会话标题"读取 docs/parallel-batches-w5/batch-R3B.md…"）。**11:11 轮实况**：已入库三笔本批代码提交——`fb465f0b1`（前置缺陷 B：`-ffp-contract=off` 补 native-renderer，仅 CMakeLists +15）、`5b3a19dd5`（**R3.3 主体**，13 文件 +1,888/-75，自陈 GTest **1490/1490**、G4 叠加层 draw call **276→3**、渲染面 JNI 端口 44→47、NDK arm64 编译过）、`574378424`（**前置缺陷 A 真修复**，4 文件 +95/-36，见第 3 条）。此刻工作区 **5 处 = R3.4 脏更新协议收尾（11:11：上轮缺的行为级守卫已补）**：新建未跟踪 `feature/game/.../sect/SceneUpdateChannel.kt`（15,725B，**11:10:18 仍在写**，**已成形为独立类**：十路端口×判据×频率表、稳态触线 0 次、每帧 JNI 4 次、`push(): Int` 返回跨线数）+ **新建未跟踪 `feature/game/src/test/.../SceneUpdateChannelTest.kt`（14,893B，11:08:58）＝ 带 `CountingSink` 计数替身的行为级守卫，9 用例**（首帧十路按协议序各一次 / **稳态 `push()==0` 且零调用** / 相机独变零导入 / 逐路变化恰一次且只触该路 / 同引用改 count 仍推 / 预览 16 浮点逐项值比较 / null 路全程静默 / **`FrameSkipPolicy.shouldSkipFrame(frameChanged=true)==false` 锁住早退不退化** / **G4 每帧 JNI：新路径固定四端口 + `push()` 实测，稳态·相机·拖拽三档均 <10，旧路径按调用点算术 >250 并 println 前后对照**）+ `M VulkanRenderBackend.kt`（**+20/-161**，**11:10:27**，把 R3.2/R3.3 内联的九个 `pushed*` 基线字段与 `pushSceneUpdates`/`pushOverlayUpdates`/`countOf`/四常量**整体抽入通道**）+ `M RenderMetrics.kt`（+28/-3，10:48:40，`sceneUpdatePushes`/`sceneUpdateFrames` 遥测）+ `M SceneOverlayProtocolGuardTest.kt`（+10/-8，11:03:09：`PREVIEW_DATA_STRIDE` 的 Kotlin 端解析源随抽通道迁到 `SceneUpdateChannel`，C++ 端仍锁 `kPreviewStride` ⇒ **迁移解析面而非放松门禁**；另把 `throw AssertionError` 改 `error(...)`，异常类型变但测试仍红，判绿语义不变）。**已亲自读码确认灰度红线未破**：`sceneStoreRender` 分叉(:113)、回滚臂 `renderLegacyOverlayPath`(:142)、`drawAllTiles`(:261)、四段逐 rect helper 全在位，`drawRect` 27 处仅存于回滚臂。GUI：步骤 **3/6**、"16 个文件已改动 +2,609 -160"（上轮 15/+2,203/-149，增量即新测试）、上下文 **51%**（上轮 48%）、输入框运行态、模型 Qwen3.8-Flash/完全访问/main 未变。**并发判定（本轮起子会话正在跑 Gradle，看护必须让路）**：`Get-Process java` 见 08:02:17（看护 Gradle）/08:40:55（Kotlin 守护）两常驻实例 + **11:10:34、11:11:05 连续新短命 JVM（Gradle 客户端特征）**，右侧后台进程面板三条运行中；cmake/ninja/g++/clang++/ctest 全空。已见其跑 `:feature:game:detekt :core:engine:detekt` 并自陈 "Need the failure detail"，随后定位到 `AtomicLong.get()` 返回 Long 与 `assertEquals(10, …)` 的 Int/Long 重载歧义 ⇒ **自查类型而非放宽断言**。**看护轮仍勿并发 cmake/ctest/Gradle**；`build/desktop-test/test/game-core-tests.exe` 仍 **10:41:50**（R3.4 纯 Kotlin 面未变，符合预期），`CTestCostData.txt` 仍 **10:22:14** ⇒ 全量门 1 未复跑。每轮按手册监控该 Qoder 会话（免 GUI 旁证优先：`git status` / 文件 mtime / `Testing/Temporary/CTestCostData.txt` / `Get-Process`）。
...
4. B11 验收要点：C++ 批口径须先重建对拍桥再跑门 1（**`libgamecorejni.so` 仍是 08:38:58 的 B10 期产物，本批未重建**；基线预期 **1490**，以当批报告为准）；**R3.4 新增 JNI 端口 3 个**（`sceneSetSelection` / `sceneSetDemolishMarkers` / `sceneSetPreview`，渲染面 44→47）⇒ 验收核其报告与 §7.2 是否逐个登记豁免理由；**待交**：R3.4 脏更新协议独立 commit（须含"稳态相机静止帧零跨线"与 `frameDirty` 早退语义不退化）、文档三件套（§7.2 B11 行 / CHANGELOG 4.01.15 / `docs/cpp-engine.md` 进展行）、G4 的**每帧 JNI 次数**数字。**11:02 轮新增必查 → 11:11 已闭合**（`SceneUpdateChannelTest.kt` 已建，9 用例含稳态 `push()==0` 且零调用、逐路变化恰一次、`FrameSkipPolicy` 早退不退化、G4 每帧 JNI 三档 <10 断言 ⇒ **验收改为核其在组合门内实跑非 skip**）；**原判断记录**：`SceneUpdateChannel` 目前只被 4 个文件引用（本体 / `VulkanRenderBackend.kt` / `RenderMetrics.kt` 注释 / `SceneOverlayProtocolGuardTest.kt` 的**静态源码比对**），**尚无带计数 Sink 的行为级"稳态零跨线"单测**（`SceneUpdateChannelTest` 不存在）；而 `push(inputs): Int` 已返回跨线次数，做该守卫成本极低 ⇒ 验收须见"稳态 0 次 / 每帧 4 次"从**文档声明 + 遥测口径**升级为**实证**（若其选择以遥测 + 静态门禁交付，须在 §7.2 说明为何不给行为守卫）。**判据修正（勿再误判）**：前几轮以"`git diff` 被删 `drawRect` 行数 = 0"当作"258 drawRect 未替换"的证据是**错的**——`drawRect` 在 helper 内部，本批是**调用点重组**：`renderFrame` 按 `NativeEngineFlag.sceneStoreRender`（默认 true）分叉，新路径仅 `sceneUpdates.push(...)`（R3.4 抽通道后）+ overlayFlags 装配 + `drawFrame`，五段逐 rect 路径整体移入 `renderLegacyOverlayPath` 回退臂（红线要求"旧路径代码不得删除"），另有 `SceneOverlayProtocolGuardTest`（现 5 用例，含 `per-rect draw calls exist only in the legacy rollback arm` 静态门禁）⇒ 验收按"结构 + 静态门禁"复核，不再要求 drawRect 删除负增量。
