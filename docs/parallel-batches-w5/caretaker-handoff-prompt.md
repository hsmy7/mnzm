...
### 当前待办（接手后立即）
0. **用户在场约束（2026-09-19 11:38 起，优先级高于手册第 3 条的"截屏观察"）**：**本轮起停用一切 windows-mcp GUI 旁证**（不 `App switch`、不截屏、不点击、不取 UI 树），改用免 GUI 只读旁证判活（`git status`/`git log`/`git diff --numstat`、文件与产物 mtime、`/tmp/b11_gate2.log` tail + BUILD 行 grep、`LastTest.log` 计数、JUnit XML 解析、`Get-CimInstance Win32_Process`）；"连续 3 轮无变化即停滞"改以 mtime/日志增长为判据。**唯一例外**：`dispatch`/`fix_needed` 需开新会话时须占 Qoder 焦点 30–60 秒，**先向用户请求让出焦点**，未获同意则不派发（台账保持 `dispatch` 等待）。验收（`verifying`）是 CLI 亲跑两门、与 GUI 无关，照常进行，但**须先确认子会话组合门已结束**再起看护构建以免争用 Gradle 守护。解除以用户明示为准。详见台账《当前状态》"用户在场约束"条。
