...
2. **B11 正在施工中**（09:21 经 Qoder 派发，批次文件 `batch-R3B.md` = R3.3 overlay 几何 C++ 生成消 258 drawRect + R3.4 脏更新协议；会话标题"读取 docs/parallel-batches-w5/batch-R3B.md…"）。**11:22 轮实况：本批四笔代码提交已齐，只剩文档 1/3 + 两道门绿证**。四笔逐子项独立入库：`fb465f0b1`（前置缺陷 B：`-ffp-contract=off` 补 native-renderer，仅 CMakeLists +15）→ `5b3a19dd5`（**R3.3 主体**，13 文件 +1,888/-75）→ `574378424`（**前置缺陷 A 真修复**，4 文件 +95/-36，见第 3 条）→ **`e4ccade12`（R3.4 脏更新协议，5 文件 +782/-175：`SceneUpdateChannel.kt` +353 新建、`SceneUpdateChannelTest.kt` +367 新建、`VulkanRenderBackend.kt` 净删约 170 行、`RenderMetrics.kt`、`SceneOverlayProtocolGuardTest.kt`）**。R3.4 交付质量已内容级复核：`push(): Int` 返回跨线数 + `CountingSink` 计数替身 9 用例（稳态 `push()==0` 且零调用、逐路变化恰一次、`FrameSkipPolicy.shouldSkipFrame(frameChanged=true)==false` 锁早退不退化、**G4 每帧 JNI 新路径三档 <10 / 旧路径 >250 并 println 对照**）。**两道门的本批首份实证已出现（我 11:02/11:11 轮列的缺口已闭合）**：① 桌面全量门 **11:14:58 复跑，我亲自 grep `LastTest.log` 得 1490 Passed / 0 Failed**（exe 10:41:50 携 R3.3 + 缺陷 A；R3.4 纯 Kotlin 未变）；② **对拍桥已于 11:19:50 重建**（`libgamecorejni.so`），组合门 `/tmp/b11_gate2.log` **此刻正在跑**（11:20→11:21 持续增长，尾部见 `:core:engine:kspReleaseKotlin`；`java` 新增 11:19:51 客户端 JVM；子会话自陈 **"ETA 11:40+"**）⇒ **看护轮在 11:30 前勿并发 cmake/ctest/Gradle**。③ 文档面：`git status` 仅 3 个未提交文档——方案 **§7.2 B11 整块已成形**（+18 行六行状态表）、**CHANGELOG +14 行**（含 G4 每帧 JNI 283→4/5/5、draw call 276→3、缺陷 A 对照表 33→39/64→77/127→153、3 新端口逐个豁免、十路判据表、"途中发现不顺手改"三项）、`android/docs/renderer-feature-checklist.md` +8/-7（新增 `overlay_geometry_cpp` 行）；**唯一未落笔 = `docs/cpp-engine.md` 进展行（仍 07:26:19，其已自陈正在补）**；两文档的 GTest/JUnit 数字均写"见完成报告口径"⇒ **验收以我亲跑为准**。GUI：步骤 **5/6**、"19 个文件已改动 +2,757 -176"、上下文 **55%**（上轮 51%）、输入框运行态、模型 Qwen3.8-Flash/完全访问/main 未变。**一处自纠待确认**：见其思考 "introduced junk `op.flagUnusedPlaceHolder = 0;`（field doesn't exist）" 正在清 `scene_overlay_equivalence_test.cpp` 的垃圾代码 ⇒ 若该 C++ 文件再改，exe 须重建、1490 那轮失效，验收门 1 须在其重建后复跑。每轮按手册监控该 Qoder 会话（免 GUI 旁证优先：`git status` / 文件 mtime / `Testing/Temporary/LastTest.log` 计数 / `/tmp/b11_gate2.log` tail / `Get-Process`）。
...
4. B11 验收要点：**（11:22 更新）对拍桥已于 11:19:50 重建、桌面全量已于 11:14:58 复跑 1490/0（子会话自跑，仍须我亲跑为准）；R3.4 已独立入库 `e4ccade12`；§7.2 与 CHANGELOG 已成形、仅剩 `docs/cpp-engine.md` 进展行 + 文档 commit + 组合门绿证** ⇒ 我复跑门 2 前**无需再重建桥**（除非其又改 C++ 面；若改了 C++ 则须先 `cmake --build` desktop-test 再 `ctest`）。C++ 批口径：门 1 基线预期 **1490**，以当批报告为准；**R3.4 新增 JNI 端口 3 个**（`sceneSetSelection` / `sceneSetDemolishMarkers` / `sceneSetPreview`，渲染面 44→47）⇒ 验收核其报告与 §7.2 是否逐个登记豁免理由（**11:22 已见 §7.2/CHANGELOG 逐端口写明豁免理由，须复核措辞是否成立**）。**判据修正（勿再误判）**：前几轮以"`git diff` 被删 `drawRect` 行数 = 0"当作"258 drawRect 未替换"的证据是**错的**——`drawRect` 在 helper 内部，本批是**调用点重组**：`renderFrame` 按 `NativeEngineFlag.sceneStoreRender`（默认 true）分叉，新路径仅 `sceneUpdates.push(...)`（R3.4 抽通道后）+ overlayFlags 装配 + `drawFrame`，五段逐 rect 路径整体移入 `renderLegacyOverlayPath` 回滚臂（红线要求"旧路径代码不得删除"），另有 `SceneOverlayProtocolGuardTest`（现 5 用例，含 `per-rect draw calls exist only in the legacy rollback arm` 静态门禁）⇒ 验收按"结构 + 静态门禁"复核，不再要求 drawRect 删除负增量。**验收必查（11:22 定稿）**：四笔提交逐子项独立、`SceneUpdateChannelTest` 9 用例 + `SceneOverlayProtocolGuardTest` 5 用例在组合门内**实跑非 skip**（XML 时间戳实证）、§7.2 三项登记（3 端口豁免 / 缺陷 A 行为变更 + 对照表 / 十路脏更新语义）、`docs/cpp-engine.md` 进展行、Canvas 零改动、`op.flagUnusedPlaceHolder` 类垃圾代码已清、"固定结构 nameIdx=19 高亮框 2×2 偏小"既有缺陷已登记不顺手改。
