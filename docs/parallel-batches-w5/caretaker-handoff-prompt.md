...
### 当前待办（接手后立即）
0. **【最高优先·看护已移交用户】2026-09-19 11:41 起本编排停止自动看护**：用户明示"你停止看护吧，我来负责看护并提醒你进行验收和给出下一轮任务的提示词"。⇒ **任何定时轮读到本条立即结束本轮**：不占锁、不截屏、不追加监控日志、不提交台账、不改状态、不派工、不起任何验收门。台账状态已置 `paused_handover`。**验收与派发只在用户开口时执行**：用户说"验收 Bxx"→ 按手册第 4 条 + 下方第 4 项清单亲跑全部门（不采信子会话自述）；用户给"下一轮任务提示词"→ 按手册第 5 条写批次文件并派发。**定时任务 id `a0d2f496-565a-40a6-9400-25e9d89032a0` 须由用户自行删除**（看护会话工具集内无 `CronList/CronDelete`，MCP 查 cron/schedule 为空，无法自删）。**另仍生效：用户在场约束＝停用一切 windows-mcp GUI 旁证**（不 `App switch`/不截屏/不点击/不取 UI 树），判活改用 `git status`/`git log`/文件与产物 mtime/`/tmp/b11_gate2.log` tail + BUILD 行 grep/`LastTest.log` 计数/JUnit XML/`Get-CimInstance Win32_Process`；"连续 3 轮无变化即停滞"改以 mtime 与日志增长为判据。派工如需 GUI 开新会话，**先请用户让出焦点**；候选免 GUI 通道 `mcp__builtin__create_chat_session` + `read_chat_session` + `wait_chat_sessions` **尚未实测**（新建会话的模型/完全访问权限/工作区绑定是否等价未知），如需采用先拿 B12 试一次。详见台账《当前状态》"看护已移交用户"与"用户在场约束"两条。
...
2. **B11 正在施工中**（09:21 经 Qoder 派发，批次文件 `batch-R3B.md` = R3.3 overlay 几何 C++ 生成消 258 drawRect + R3.4 脏更新协议；会话标题"读取 docs/parallel-batches-w5/batch-R3B.md…"）。**11:33 轮实况：本批四笔代码提交已齐，文档三件套 3/3 已全部落笔（cpp-engine.md 补齐），只剩文档 commit + 组合门绿证 + 完成报告**。四笔逐子项独立入库：`fb465f0b1`（前置缺陷 B：`-ffp-contract=off` 补 native-renderer，仅 CMakeLists +15）→ `5b3a19dd5`（**R3.3 主体**，13 文件 +1,888/-75）→ `574378424`（**前置缺陷 A 真修复**，4 文件 +95/-36，见第 3 条）→ **`e4ccade12`（R3.4 脏更新协议，5 文件 +782/-175：`SceneUpdateChannel.kt` +353 新建、`SceneUpdateChannelTest.kt` +367 新建、`VulkanRenderBackend.kt` 净删约 170 行、`RenderMetrics.kt`、`SceneOverlayProtocolGuardTest.kt`）**。R3.4 交付质量已内容级复核：`push(): Int` 返回跨线数 + `CountingSink` 计数替身 9 用例（稳态 `push()==0` 且零调用、逐路变化恰一次、`FrameSkipPolicy.shouldSkipFrame(frameChanged=true)==false` 锁早退不退化、**G4 每帧 JNI 新路径三档 <10 / 旧路径 >250 并 println 对照**）。**两道门的本批首份实证已出现（我 11:02/11:11 轮列的缺口已闭合）**：① 桌面全量门 **11:14:58 复跑，我亲自 grep `LastTest.log` 得 1490 Passed / 0 Failed**（exe 10:41:50 携 R3.3 + 缺陷 A；R3.4 纯 Kotlin 未变；**注：该轮随后因 11:23:35 的 C++ 测试源新增而作废，见第 4 条**）；② **对拍桥已于 11:19:50 重建**（`libgamecorejni.so`）；组合门 `/tmp/b11_gate2.log` **此刻仍在跑但已过测试段**（11:33 轮实况：251KB/273 任务、全文 `FAILED` 计数 **0**，已进入 `writeReleaseLintModelMetadata` + `:app:generateReleaseUnitTestLintModel`，**ETA ≈11:35–11:40**）⇒ **看护轮在 11:41 前勿并发 cmake/ctest/Gradle**。③ **本批两份新守卫首份实跑绿证已由看护亲解析 XML 到手**（11:31:57 落盘）：`SceneUpdateChannelTest` **9/0/0/0skip**、`SceneOverlayProtocolGuardTest` **5/0/0/0skip**；全量 XML 当前 tally 6836/0/0/15skip（`:app` 未落盘）+ **Diff\* 273 用例 0 skip**。④ **文档三件套 3/3 齐**：`docs/cpp-engine.md` **+1 行 B11 进展行已落笔**（11:25:32）、方案 §7.2 +24、CHANGELOG +14、`renderer-feature-checklist.md` +8/-7，**四件均未提交（等门数字回填后独立 commit）**。⑤ **代码面新增两笔经逐条读 diff 判定为纯注释、零语义**（`VulkanRenderBackend.kt` +3/-2 补等价证明来源、`gamecore/test/CMakeLists.txt` +2/-1 补守卫注释）。⑥ **11:27 轮盯的两处已定案**：C++ 侧 +121 = 第 13 条守卫 `PlacementModeFullFrameDrawCallBudget`（整帧 draw call 13<15）；`SceneUpdateChannelTest` +6/-3 = `LEGACY_FIXED_PORTS=6` 自纠算术（`>250` 断言未动，且 cpp-engine 写"旧 282"正是 283−1 的互证）。**垃圾代码 `op.flagUnusedPlaceHolder` 已清**（11:23:35 后该文件未再改）。GUI：其自陈下一步 **"等 JUnit 门补，补上门禁数字后生成完成报告"**、上下文 **58%**（上轮 55%）、输入框运行态、模型 Qwen3.8-Flash/完全访问/main 未变 ⇒ **有推进非停滞、尚未出最终完成报告**。每轮按手册监控该 Qoder 会话（免 GUI 旁证优先：`git status` / 文件 mtime / `Testing/Temporary/LastTest.log` 计数 / `/tmp/b11_gate2.log` tail / `Get-CimInstance Win32_Process` 判 java）。
...
4. B11 验收要点：**（11:33 更新）四笔代码提交齐 + 文档三件套 3/3 已落笔（含 `docs/cpp-engine.md` B11 进展行）；对拍桥 11:19:50 已重建且对生产面即时（未提交 C++ 全在 `gamecore/test/`，生产源最新 `scene_draw.h` 10:33:26 < 桥）⇒ 我复跑门 2 前无需再重建桥（除非其再改生产 C++）**。**门 1 口径已变（重要）**：`game-core-tests.exe` 仍 **10:41:50**，而 `scene_overlay_equivalence_test.cpp` 于 **11:23:35** 新增第 13 条整帧守卫 ⇒ **11:14:58 那轮 1490/0 作废**，验收时**一律由我 `cmake --build` 重建后复跑 `ctest`，基线预期 1491**（cpp-engine 预写 1476 + SceneOverlayEquivalenceTest 13 + SceneStoreTest 2 = 1491，与看护预测 1490+1 一致；文档数字属预写，以我实跑为准）。**R3.4 新增 JNI 端口 3 个**（`sceneSetSelection` / `sceneSetDemolishMarkers` / `sceneSetPreview`，渲染面 44→47）⇒ 验收核其报告与 §7.2 是否逐个登记豁免理由（**11:22 已见 §7.2/CHANGELOG 逐端口写明豁免理由，须复核措辞是否成立**）。**判据修正（勿再误判）**：前几轮以"`git diff` 被删 `drawRect` 行数 = 0"当作"258 drawRect 未替换"的证据是**错的**——`drawRect` 在 helper 内部，本批是**调用点重组**：`renderFrame` 按 `NativeEngineFlag.sceneStoreRender`（默认 true）分叉，新路径仅 `sceneUpdates.push(...)`（R3.4 抽通道后）+ overlayFlags 装配 + `drawFrame`，五段逐 rect 路径整体移入 `renderLegacyOverlayPath` 回滚臂（红线要求"旧路径代码不得删除"），另有 `SceneOverlayProtocolGuardTest`（现 5 用例，含 `per-rect draw calls exist only in the legacy rollback arm` 静态门禁）⇒ 验收按"结构 + 静态门禁"复核，不再要求 drawRect 删除负增量。**验收必查（11:33 定稿）**：四笔提交逐子项独立、`SceneUpdateChannelTest` 9 用例 + `SceneOverlayProtocolGuardTest` 5 用例在组合门内**实跑非 skip**（11:31:57 XML 已见首份实证，验收仍以我亲跑那轮为准）、§7.2 三项登记（3 端口豁免 / 缺陷 A 行为变更 + 对照表 / 十路脏更新语义）、整帧 draw call 守卫**实跑 stdout 打印值**（文档写 13<15，CHANGELOG 一处写崖壁 ≤7 而守卫按 8 构造上界 ⇒ 以实跑为准核一次）、`docs/cpp-engine.md` 进展行、Canvas 零改动、`op.flagUnusedPlaceHolder` 类垃圾代码已清（11:23:35 后未再改，判已清）、"固定结构 nameIdx=19 高亮框 2×2 偏小"既有缺陷已登记不顺手改、**文档三件套 + 两份测试源改动最终如何入库**（`SceneUpdateChannelTest`/`VulkanRenderBackend`/`test CMakeLists` 的自纠发生在 `e4ccade12` 之后 ⇒ 核它是新笔还是 amend；若 amend 则 11:15 轮记录的 +782/-175 以最终谱系为准）。
