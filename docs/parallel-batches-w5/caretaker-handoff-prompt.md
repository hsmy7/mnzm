...
2. **B11 正在施工中**（09:21 经 Qoder 派发，批次文件 `batch-R3B.md` = R3.3 overlay 几何 C++ 生成消 258 drawRect + R3.4 脏更新协议；会话标题"读取 docs/parallel-batches-w5/batch-R3B.md…"）。**10:22 轮实况**：步骤 2/6、上下文 43%、改动面 **13 文件 +1,847/-75**（C++ 生成面 5 文件 + Kotlin 消费面 `NativeBridge.kt +47/-2`、`NativeEngineFlag.kt +12/-6`、`VulkanRenderBackend.kt +186/-58` + 未跟踪 `gamecore/test/scene_overlay_equivalence_test.cpp`（10:10:31）与新出现的 **R3.4 Kotlin 守卫 `feature/game/src/test/.../SceneOverlayProtocolGuardTest.kt`（10:15:17）** + `SceneUvTablesMirrorGuardTest.kt +69/-2`（10:18:51 实时在写））；`game-core-tests.exe` 10:12:25 重建，**10:22 截屏见其正启动全量桌面 ctest**（终端运行态）；`:feature:game:compileReleaseKotlin` / engine 测试编译分别于 09:56、10:16 exit 0（**复用看护 Gradle/Kotlin 守护进程 session 25/27**，未另起 JVM）。当前 `cmake/ninja/ctest` 进程未入列（ctest 刚启动）、java 仅 08:02/08:40 两实例 ⇒ 无并发构建，但**看护轮勿并发 cmake/ctest/Gradle**。每轮按手册监控该 Qoder 会话（免 GUI 旁证优先：`git status` / 文件 mtime / `/tmp/*.log` / `Get-Process`）。
3. B11 批次文件内置两处**前置缺陷红线**（验收时须盯其交付）：① `VulkanRenderBackend.kt:552/555` 网格线 Y 轴漏乘 `TOPDOWN_Y_SCALE` —— 修复须独立 commit 或逐位保留现状，禁止混入等价重构。**证据测试已就位但修复 commit 尚未出现**：其"defect A evidence test"量化对照表（差 6173/26 条 @ scale 0.34/0.5/0.75）已在 overlay 守卫内 12/12 绿，而 HEAD 至今只有前置缺陷 B 的独立单笔 `fb465f0b1`，`VulkanRenderBackend.kt` 自 09:56:01 未再改 ⇒ 验收必查：修复是否独立 commit、量化表是否进 §7.2、等价重构 commit 内是否零行为变更；② `-ffp-contract=off` 未覆盖 native-renderer —— **已按红线独立提交 `fb465f0b1`（仅 `cpp/CMakeLists.txt` +15 行）**，验收时核其提交仍为独立单笔即可。
4. B11 验收要点：C++ 批口径须先重建对拍桥再跑门 1（**当前 `libgamecorejni.so` 仍是 08:38:58 的 B10 期产物，本批未重建**；基线预期 ≥1476，本批 overlay 守卫 12 条 + scene_store 新增 3 条会再上移，**以当批报告为准**）；**R3.4 新增 JNI 端口已实证为 3 个**（`sceneSetSelection` / `sceneSetDemolishMarkers` / `sceneSetPreview`，`NativeBridge.kt` 声明数 44→47，`GameCoreBridge` 4 未变；子会话已自数并自纠过一次计数歧义）⇒ 验收核其是否在报告与 §7.2 登记豁免；消费面须见**负增量**（258 drawRect 真被替换，而非只加 C++ 生成面）。**10:22 实况补充**：`git diff` 被删的 `drawRect|drawLine|drawText` 行计数当前仍为 **0**（新增 2 行，-75 全为 KDoc/结构），其形态是"先建 overlay 双路共存 + 标注回退臂"（与 B10 灰度口径一致、方向正确）——验收时须确认最终确有真替换负增量，或由其明确登记"双路保留、替换落在后续批"。
