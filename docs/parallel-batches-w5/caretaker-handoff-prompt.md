...
2. **B11 正在施工中**（09:21 经 Qoder 派发，批次文件 `batch-R3B.md` = R3.3 overlay 几何 C++ 生成消 258 drawRect + R3.4 脏更新协议；会话标题"读取 docs/parallel-batches-w5/batch-R3B.md…"）。**10:34 轮实况**：**R3.3 主体已独立提交 `5b3a19dd5`（10:32:39，13 文件 +1,888/-75）**，自陈 GTest **1490/1490**（1476+14）、G4 叠加层 draw call **276→3**、渲染面 JNI 端口 44→47（生产总数 85→88）、NDK arm64 编译过且 `compile_commands` 核到 `-ffp-contract=off`、detekt + compileReleaseKotlin 绿；提交后工作区一度干净（10:32:54），此后增量至 10:41 的 **4 文件 +62/-34**（`scene_overlay_equivalence_test.cpp` 10:39:17 / `scene_draw.h` 10:33:26 / `VulkanRenderBackend.kt` 10:35:30 / `SceneOverlayProtocolGuardTest.kt`）＝**前置缺陷 A 修复 + R3.4 面在写**（见第 3 条）。**10:41 轮实况**：步骤 3/6、"13 个文件已改动 +1,984 -80"、上下文 **46%**、输入框运行态、此刻 `gradlew :feature:game:compileReleaseKotlin` 运行中，其前一条自陈 "12/12 pass with the before/after evidence table"；桌面全量 ctest 上一轮完成于 **10:22:14**（`game-core-tests.exe` 仍 10:12:25 ⇒ 缺陷 A 修复后的 C++ 测试走筛选跑）；java 仍仅 08:02/08:40 两实例（看护 Gradle/Kotlin 守护，cmake/ninja/ctest 全空）⇒ 复用守护进程、无 Gradle 争用。**看护轮仍勿并发 cmake/ctest/Gradle**。每轮按手册监控该 Qoder 会话（免 GUI 旁证优先：`git status` / 文件 mtime / `Testing/Temporary/CTestCostData.txt` / `Get-Process`）。
3. B11 批次文件内置两处**前置缺陷红线**：**② `-ffp-contract=off` 覆盖 native-renderer —— 已按红线独立提交 `fb465f0b1`（仅 `cpp/CMakeLists.txt` +15 行）**，验收核其仍为独立单笔即可。**① 网格线 Y 轴漏乘 `TOPDOWN_Y_SCALE` —— 交付形态已于 10:41 由选项②转为选项①"真修复"**：`5b3a19dd5` 入库时确为"逐位复刻现状"（守卫用例 `GridRowRangeReplicatesLegacyUncompressedFormula`），但**当前工作区**（4 文件 +62/-34）已把 `scene_draw.h::buildOverlayLayers` 的 `lastRow` 改为 `viewportH/(scaleSafe * kTopdownYScale)`、并同步给 **Kotlin 回退臂** `drawGridOverlay` 补 `SpriteAtlasDef.TOPDOWN_Y_SCALE`；守卫用例改名 `GridRowRangeFollowsProjectedViewportBand`，`SceneOverlayProtocolGuardTest` 新增"三端口径防复发锁"（同时锁 Vulkan 旧臂 / Canvas / C++ 三处公式）。截屏自陈 "12/12 pass with the before/after evidence table"。⇒ **验收必查（取代 10:34 轮的"选项②单列登记"判据）**：a) 该修复须为**独立 commit**（R3.3 已先行入库，故须核实它没有 amend/混入 `5b3a19dd5`）；b) **回退臂行为变更须显式登记**——B10 起"回退臂行为 = 开工前现状"的不变量被有意打破，§7.2/CHANGELOG 须写为**行为变更**（视口底部补齐横线、与 Canvas 对齐）并重述灰度回退语义；c) 改动面须仅限该公式，不得顺带其他行为。
