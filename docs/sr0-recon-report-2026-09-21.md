...
### 6.1 总判定：**Go**（No-Go 条件未触发）

| 任务 | 判定 | 一句话结论 |
|---|---|---|
| T1 payload 实测 | ✅ Go | 最大档（200 年极限档）LZ4 后 **0.29MB** = TapTap 硬上限 2.8%；裁剪非必要（战斗日志维持 1000 封顶）、heavy 分 key 无必要 |
| T2 TapTap 限额 | ✅ Go | 100 档/玩家/游戏 + 100MB 总量 + 10MB 单档；限频最保守口径 1 次/分钟（创建+更新共享冷却）；6 槽方案全额可行 |
| T3 云多档语义 | ✅ 代码层可行 | `CloudSaveApi` 全名参数化 + SDK 4.10.5 签名核验匹配；6 项待真机/沙盒实测（§3.3，不阻塞 SR-2 开工） |
| T4 冲突剧本 | ✅ 交付 | SR-2 单测清单（11+10 例）+ 双设备剧本矩阵 S1-S10（真机部分归 SR-3 硬门） |
| T5 嵌套事务 | ✅ 存疑解除 | 并入同一事务（4/4 实测 + Room 2.7.0 SAVEPOINT 源码证据）；审计 §15 已补注 |

方案 §4 的 No-Go 定义 = "payload 无法压到红线内且分 key 不可行"——实测 payload 距红线
两个数量级，No-Go 不成立，**SR-1 起按既定依赖链推进**。

### 6.2 IN5 红线建议（SR-0 定值，待用户/验收确认后固化 CI）

**红线 = 2,000,000 字节（LZ4 后云档 payload，构造 200 年极限档样本断言）**。
依据：最坏实测 0.29MB ⇒ ~7× 余量；对 10MB 硬上限留 5×；覆盖 SR-1 邮件并入、
内容密度修正 3× 与未来功能漂移。CI 断言种子已就位（`CloudPayloadSizeBenchTest`，
当前 10MB 宽松断言，红线确认后收紧为 2MB）。

### 6.3 需用户/后续批拍板的事项（如实上报，不夹带决策）

1. **AI 宗弟子换设备不保真（现役云链路既有语义）**：`aiSectDisciples` @Transient 不进云档，
   换设备由 `ensureGameDataIntegrity` 重生成 ~初始态（50/宗）。SR-3 槽位云化前须拍板：
   (a) 接受重生成（零工作量）；(b) heavy 域并入云档——体积上现实态可行（~1450 弟子 ≈ 0.5MB
   LZ4 量级），但 1000/宗 满编上限会逼近 10MB（~10MB 量级），并入则必须配套裁剪策略。
   本报告不替用户选，仅给数据。
2. **限频口径冲突**（§2.3）：按 1 次/分钟保守设计，真机实测后可放宽——UploadQueue 冷却参数化即可，无需预拍板。
3. **IN5 红线 2MB** 本报告为建议值，固化进 CI 前由用户/验收确认。

### 6.4 交付物索引

| 交付物 | 路径 |
|---|---|
| 施工卡 | `docs/parallel-batches-w5/batch-SR0.md` |
| 本报告 | `docs/sr0-recon-report-2026-09-21.md` |
| 完成报告 | `docs/parallel-batches-w5/report-SR0-completion-2026-09-21.md` |
| 台架测试（保留，IN5 种子 + Room 升级守卫） | `android/core/data/src/test/.../CloudPayloadSizeBenchTest.kt`、`RoomNestedTransactionSemanticsTest.kt` |
| 审计补注 | `docs/save-system-audit-2026-09-21.md` §15 存疑 1 后【SR-0 补注】 |

### 6.5 纪律声明

零产品代码改动（本批全部提交 = docs/ + core:data 测试目录 2 文件）；SR 方案文档保持
untracked 未提交；台账仅动 SR 相关行；真机/沙盒不可测项全部如实登记未虚报；
`accepted` 留待用户。
