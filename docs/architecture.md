...
| C-07 | **批次 10：彻底单引擎**（原"职责边界固化"二次重定义）——C++ 唯一真相源（结算/实体存储/~~引擎循环~~/渲染），Kotlin 降级纯平台层（UI/平台能力/存档编码）；阶段化退役见 docs/cpp-engine.md 第 7 节。**2026-08-27 阶段 5 更新**：引擎循环（帧累积/逻辑步进/墙钟消费/tick 计数/心跳/看门狗判据）已入 C++，批次 10 剩余阶段 6（渲染 RHI）与阶段 7（Kotlin 退役 + 存档决策） | 计划 v2 阶段 1-7 逐阶段（阶段 5 ✅、阶段 6 ✅ 已完成——阶段 6 渲染 RHI + 合成器统一：道路合成器单一权威 `road_compositor.h` 物理下沉（批次 R 剩余清零）+ `Rhi.h` 形式化（Metal/iOS 预留）；剩余阶段 7） |
