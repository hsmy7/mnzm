...
| # | 待办内容 | 触发/计划 |
|---|---|---|
| C-06 | **批次 9：转发层收尾** ✅ 基础设施已完成（feature flag/StateSyncService 宽松合并/tick 桥/转发辅助/性能基准/增量变更集通道）；批 8-1~8-3 完成监控器接口化（engine `import android.*` 清零）+ 库存家族 10 动作生产接线；**批 8-4（2026-08-29）接线面收口判定**：87 个 ActionId 全量清点六类裁决（已接线 10 / 结算内部路径 16 / 纯函数影子基准 46 / 查询留守 4 / 钱包族事务内原语留守 3 / 无独立生产调用点 8）——可接线面已穷尽，转发接线阶段终结；**退役专项批 9-1/9-2（2026-08-29）完成**：SHADOW 对拍态 + 纯 Kotlin 旬结算路径删除，tick 结算恒走 native（单引擎终态），对拍框架转长期回归基线（engine JUnit 2925/2925 全绿含桌面 JNI 对拍 0 skip）；逐动作转发降级契约保留（判定详见 docs/cpp-engine.md §7.1 批 8-4 + §7.2）；**增量迁移批 10-1/10-2/10-3/10-4（2026-08-29~30）**：月变 S8 侦察过期清理 + 月度叛逃检测 + 月度偷盗兜底 + 附庸脱离检查下沉 C++（含宗门详情域协议扩容、执法堂配置/职务加成辅助、aiSectDisciples @Transient 顶层协议承载，cpp-engine.md §7.3）；**批 10-5（2026-08-30）**：S-15 清偿（aiSectDisciples 反向回导 + 镜像 @Transient 保留修复）；**批 11-1~11-3（2026-08-30）**：S8 自动招募 + 秘境到期关闭/AI 队伍派遣 + 十二月自动购买下沉（GTest 603/603 + DiffMonthSettlementTest 三场景对拍，S-16~S-19 登记，cpp-engine.md §7.3）——用户指示收窄范围，任务/洞天/AI 兽战/购买/任务刷新保持未下沉并登记批次；**批 11-4（2026-08-30，途中问题清偿）**：FakeGameStateStore 嵌套事务对齐生产重入语义（S-14 家族，12 月对拍库存集合+年度 by-source 纳入 diff）+ 任务 RNG 确定性化（RngPartition 新增 MISSION(8)，MissionSystem 收敛 GameRngManager 分区，S-19 清偿）+ 商人回退分支确定性散列选池（S-18 清偿） | 接线与退役完成；月/年残留执行器余量随 §7.3 增量批次逐批下沉 |
