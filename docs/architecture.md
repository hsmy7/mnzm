...
| # | 待办内容 | 触发/计划 |
|---|---|---|
| C-06 | **批次 9：转发层收尾** 🟡 基础设施已完成（feature flag/StateSyncService 宽松合并/tick 桥/转发辅助/性能基准/增量变更集通道）；剩余 Kotlin GameEngine ~289 方法逐一转发（84 动作协议已建未接线，其余 ~200 操作待 C++ 化）——**阶段 7 已完成 AUTHORITATIVE 默认切换与存档决策（cpp-engine.md §7），C-06 成为 Kotlin 引擎全量退役的最后前置**（完成后删除 Kotlin 双实现 + shadow 对拍转回归基线） | 超大工作量批次（阶段 7 批 7-4 依赖）；全量退役为终态收尾 |
