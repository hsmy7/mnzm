...
> **执行状态（2026-09-17 收官）**：W4-00 前置批 ✅ → 三批并行 ✅（W4-A `w4a/01`–`05` / W4-B `w4b/01`–`05` / W4-C `w4c/01`–`08`）→ **集成收口 ✅**（合并提交 `7c89deb19`，全部门禁绿；独立复验抓出并根因修复 3 处缺陷——A 测试源编译、C 迁移链 12 处、B 硬编码 ActionId，详见 [handover §2.68](../cpp-migration-handover-m0.md)）。**汇流波 W4-D（串行）：D1 埋点 ✅（§2.72，`w4-rem/02`）→ D2 `w3-11` 残差 ✅（§2.73，`w4-rem/03`）→ D3 harness 对齐 ✅（§2.74，`w4-rem/04`）→ D4 通道删除 ✅ **终局交付（§2.75–§2.82，`w4-rem/05`–`12`）：写者收口四段达成硬前置 → 五步④归档 tag `w3-13-pre-delete` → ⑤通道删除本体落地——反向通道不复存在，AUTHORITATIVE 稳态 Kotlin 对 C++ 只读（防复发 = `MirrorReadOnlyGuardTest` + 稳态零写入断言）→ D5 死代码清零 + 守卫面收口 ✅（§2.83，`w4-rem/13`：全量重扫立清单 + 10 组死链删除 + 守卫计数断言落地）→ D6 文档与版本收口 ✅（§2.84，`w4-rem/14`：全文档口径对齐 + §7.2/§7.3 勘误补齐 + cpp-engine §9 / CODE_WIKI / 双日志同步；`version.properties` 留用户决定）——🔴 W4 波可派工项全部收官****；§9 非并行轨（真机 / WS-4）需用户决策或硬件。
>
...
| D2 | **w3-11 月年编排残差** | W4-B（w3-05）、W4-C（w3-06） | 扇出项逐条判定：状态写 → C++；通知/日志 → Kotlin；引导领奖 `GameEngineGuideOps.claimGuideReward`（`:57`）下沉；兑换码 `RedeemCodeService`（`:153/:402`）按既有结论**登记不下沉**（C++ 无物品随机生成器，RNG 红线）。**本批同时消化 §2.3 冻结的宿主文件族调用点清理**。**✅ 已实施（2026-09-15，§2.73，tag `w4-rem/03`）**——1830 `GUIDE_REWARD_CLAIM_TX` + `guide_reward_tx.h`（注册表复刻 + 可行性预检 + 抽取位四象限对齐）；附庸年贡/脱离实裁"C++ 已在位、不占号"；宿主族解冻核对零死调用点；`guideClaimedRewardIds` 转入关闭；桌面 1417/1417、引擎 3305/304/0/0 |
| D3 | **`DiffAuthoritativeTickTest` harness 对齐生产** | D2 | ADR §8 债：把 harness 的 AUTHORITATIVE 管线从"Kotlin 月/年完整编排"改为"C++ 月结 + Kotlin 残差"，使测试面 = 生产面；随后**重评** 4 个被覆写字段（`spiritMineLastSettledMonth` / `annualAlchemyCount` / `availableMissions` / `yearlyReports`）的关闭结论。**＋ 并入项（2026-09-15 拍板，§2.68 复验发现）**：把 WS-5b 的 `terrainTiles` / `mapGenVersion` 从 `DiffSurfaceAssertion.diffIsMirrorGeneratedField` 的**对拍排除面**中移除——做法 = 让 harness 的 Kotlin 侧也跑与生产同款的 boot 回填（当前 harness 不跑 ⇒ Kotlin 期望快照恒无该段 ⇒ 当初只能排除），随后两字段按普通字段参与全状态逐字段对拍。验收 = 排除面删除后 47 个 `Diff*` 全绿。**✅ 已实施（2026-09-15，§2.74，tag `w4-rem/04`）**——桌面 JNI + `DiffRngBridge` 新增 settleMonth/settleYear 通道；Side A 边界改生产同款（C++ 结算 + 残留执行器）；对齐红点归因**全部为 harness 缺装配**（真实 DiscipleLifecycleProcessor / MerchantAndRecruitService + ManualDatabase / cave Processor Provider / 收购 id 排除面），零两端真分叉；4 字段全部转入 W4-D closedUnits；地形 2 字段参与全状态对拍。门禁：桌面 1417/1417、引擎 3305/304/0/0（持平）、`:core:domain` 1758/0 |
| D4 | **w3-13 反向通道删除批（终局）** | D1–D3 + W4-A/B/C 全部合入 | 按 ADR §4 五步：**先禁用（信封体积=0 但代码在）→ 跑完整业务周期（≥1 游戏年 + 离线结算 + 跨旬月 + 存档写读，期间关闭域写入检测零命中）→ 状态指纹零差异（同档同输入 ≥100 旬）→ 删前打归档 tag → 再删除**。删除面：`captureReverseDirty` / `ReverseDirtyAccumulator` / `applyDirtyToNative` + 反向信封构建 / `consumeReverseDirty` / `GameCore::applyReverseDirty` + JNI 导出 / `ReverseChannelPolicy` 本体。保留面：前向镜像 + 全量导入（读档/新档基线）+ 各域回退臂。**防复发**：镜像只读契约（结构性/编译期约束优先）+ "稳态零写入断言"进长期 CI。**⚠️ 阶段 A+B 已实施（2026-09-15，§2.75，tag `w4-rem/05`）**——观察窗仪器（`ReverseChannelPolicy.reverseTransportEnabled` 停发开关）落地并进 CI（100 旬停发对拍用例：零信封发送 + 关闭域写入检测零命中 + 终态全量对拍逐位一致，全绿）；**删除步判定 = 阻断**：AUTHORITATIVE 稳态 Kotlin 写者未清零（交谈效果 `DiscipleDelegate.kt:238` / 任务派遣 `GameEngineMissionOps.kt:26` / 存档前自愈 `SaveFacadeImpl.kt:56` 等；守卫红线"弟子通道与 aiSectDisciples 段必须保持传输"仍在位），硬前置"关闭清单 = 全部传输单元"不成立——逐写者完成路径见 handover §2.75，写者收口完成后再入删除步。**✅ 完成路径第 1 项已实施（§2.76，`w4-rem/06`）：交谈效果写面下沉**（`DISCIPLE_CHAT_EFFECT_TX=1860`，段 1860–1869 首个实裁，事务零 RNG 双臂抽取恒 0）；审计修正：通道整体关闭还依赖任务域收口（`startMission` 写槽位协议列）+ lifeEvents 投影检测裁决（§2.76 审计先行节）。**✅ 上述两项已全部实施（§2.77，`w4-rem/07`）：`MISSION_START_TX=1861` 任务域收口 + `DISCIPLE_CHANNEL` 关闭（守卫红线翻转）+ lifeEvents 投影转非捕获路径 + 捕获侧检测 AUTHORITATIVE 门控**——删除步剩余开放面 = `aiSectDisciples` 段（存档自愈）、9 类集合、retained 字段族（§2.75④ 第 3/4 项）。**✅ 第 3 项已实施（§2.78，`w4-rem/08`）：`aiSectDisciples` 段 + `worldMapSects` 关闭**——写者穷尽审计 + `rebaselineNativeMirror` 四处接线（存档自愈/完整性修复/攻宗占领/升级回退臂）；`sectRelations`/`activeSectId`/`sectName` 保留并登记审计结论。**✅ 第 4 项第一段已实施（§2.79，`w4-rem/09`）：retained 字段族逐域判定——29 项转关闭 + §2.77/§2.78 漏网写者补漏 8 处接线**（宗门改名/仓库卸任迁入引擎层；enterSect/设置重置/槽位释放/取消血炼/偷盗钩子/内存裁剪 rebaseline 或 updateMirror+rebaseline）；第二段保留面（钱包/年度账/执法堂/兑换码/战斗世界域/`sectRelations`/9 类集合）落守卫断言防抢跑。**✅ 第 4 项第二段已实施（§2.80，`w4-rem/10`）：gameData 字段面关闭清单 = 全部传输单元**——30 项转关闭 + 12 处逐动作接线（世界胜利/侦查/攻宗碾压败北/贸易购买懒刷新/邮件领取/试炼通关领取/兑换码/逐出物化/开袋/弹窗清空 updateMirror/自动购买与生产槽迁引擎层）。**✅ 第 4 项第三段（终段）已实施（§2.81，`w4-rem/11`）：9 类实体集合转关闭——🔴 删除步硬前置"关闭清单 = 全部传输单元"全量达成**（写者穷尽审计约 90 调用点/25 文件；AUTHORITATIVE 稳态可达且捕获型仅 6 簇 7 处，宿主事务 updateMirror 化 + 基线重建：攻宗占领/碾压、世界关卡胜利奖励、逐出袋物化、仓库赏赐装备/功法（无 native 臂，新增条件性 rebaseline）、宗门贸易购买、妖兽迎战；余者回退臂/flag-OFF 月年结臂/LOAD_BOOT/no-op/对拍基准专属/局部副本六类合法形态；守卫终局断言锁死）——**✅ 第 5 步（终局）已实施（2026-09-17，§2.82，`w4-rem/12`）：反向通道删除本体落地**。删除面全数执行——Kotlin 捕获链（`ReverseDirtyAccumulator`/`captureReverseDirty`/`consumeReverseDirty`/`resetReverseAccumulator`）+ 信封发送链（`applyDirtyToNative`/`buildReverseEnvelope` 族/关闭字段检测族/信封体积埋点）+ `ReverseChannelPolicy` 本体与 `reversechannel/` 包五文件 + C++ `GameCore::applyReverseDirty` 与匿名辅助簇四函数 + `reverseVersion_` + 双 JNI 导出（生产 + 对拍）+ GTest `apply_reverse_dirty_test.cpp`（13 用例）+ 5 个 Kotlin 反向测试类；保留面原样（前向镜像/`importToNative` 全量导入/`rebaselineNativeMirror`/各域回退臂）；防复发两道门禁落地——`MirrorReadOnlyGuardTest`（六模块主源通道符号零命中）+ 100 旬 AUTHORITATIVE 管线稳态零写入断言（`FakeGameStateStore.nonMirrorWriteCount == 0`，观察模式转正式硬门禁）。门禁：桌面 **1417/1417**（= 1430（§2.77 实测真值）− 13；基线表 1417 系 §2.73 漂移值已修注）｜引擎 **3288/0/0/0**（47 `Diff*` 绿 + 稳态零写入断言绿）｜domain 1752/0｜data 716/0/15｜game 872/0｜app 995/0/2｜detekt 六绿｜NDK+lint 绿｜生成物零漂移（198 动作/maxId=1861）｜JNI 导出 ↔ external **38 ↔ 38**。**真机 E3/N5（信封体积/耗时）观测面随通道删除取消** |
| D5 | **死代码清零 + 守卫面收口** | D4 | ①🔴 先补清单（原"18+11+5"聚合数不可考→全量重扫立新基准，三份"生产 0/测试 n"清单进 handover §2.83）；②洞府死链；③GameData 六死函数；④GameSettingsData 族（已拍板清理）；⑤守卫收缩。**✅ 已实施（2026-09-17，§2.83，`w4-rem/13`）**：前批已清偿核实 5 项（洞府死链 W4-B/B4 / updatePatrolConfig W4-B / 遮蔽 W4-C / bloodMaterials 分区化 W4-B/B0 / 悬空 KDoc §2.68）；本批删除 10 组（maxDisciples / GameSettingsData 族 / GameData 六函数 / assign·removeManual 三层链 / 两 UseCase / AISectPersonality 双函数+weightedPool / PillGrade.random 无参 / BaseTemplateRegistry 三函数 / BeastMaterialRegistry 双函数 / BeastMaterialDatabase.getByRealm）——**诚实口径**：A5 存量 8 处核实仅 7 处死，`getRandomMaterialByBeastType` 有 3 生产调用方保留并登记阶段 3；守卫面 = RngSource core/domain 13→6 + RngIsolation 白名单 4→1 + **计数断言落地**（§12 债表两行销账）。门禁：domain 1743/0｜engine 3288/0/0/0（−1+1 持平，47 `Diff*` 0 跳过）｜game 872/0｜app 986/0/2｜data 716/0/15｜detekt 六绿｜桌面/NDK/lint 豁免（零 C++）｜生成物零漂移 |
| D6 | **文档与版本收口** | D1–D5 | handover §3 验证表 / §4.1 勾销 / §5 下轮建议 / §6 主轴剩余 / `ui-read-surface.md` §4.4 / `docs/cpp-engine.md` §9 计数表 / `CODE_WIKI.md` / 双更新日志（`CHANGELOG.md` + `changelog_entries.json`，**必须一起**；同版本条目追加到 `changes` 数组末尾，禁新建同版本第二条目）；`version.properties` 由**用户**决定是否递增。**✅ 已实施（2026-09-17，§2.84，`w4-rem/14`）**——handover §2.84 新小节 + §4.1 索引补齐（§2.69–§2.84）+ §5③④/批次索引收官改写 + §6 接手必读数值刷新（3288/1743/716/1417 + 守卫 13→6、4→1）+ **§7.2/§7.3 勘误补齐**（6 项原位标注 + §7.3 三写者已收口 + §7.4 ②⑤⑥ 已取消/已裁决/已定论）+ cpp-engine §9 权威指针更正（**198 动作 / maxId=1861**）+ CODE_WIKI 补 `updateMirror` 行与镜像只读契约块 + ui-read-surface §4.3 终局标注与 §4.4 两处误引勘误 + rng-source-inventory §8 / rng-remediation-status 追记；双日志 = CHANGELOG.md 追加 D6 条目（游戏内 json 未追加——零玩家可见变更）；`version.properties` **未递增（留用户决定）**。**W4 波可派工项至此全部收官** |
...
| ~~**`RngEngineIsolationGuardTest` 白名单无计数断言**~~ | ✅ **已清偿（2026-09-17，§2.83 D5）**：白名单缩至 1 条（3 条陈旧"同族遗留"豁免随 W4-C 收敛删除）+ 新增"白名单条目数必须等于登记值"计数断言（`expectedExclusionCount = 1`）——新增豁免从此机器可拦 | — |
...
| ~~**`GameEngineSectLevelOps.kt:210` 的输入侧随机流入 C++ 事务**~~ | ✅ **已清偿（W4-B/B0；§2.83 D5 核实销账）**：模板抽取改走 `RngPartition.MAIL`，`SECT_LEVEL_CLAIM_TX` 发放内容可复现 | — |
