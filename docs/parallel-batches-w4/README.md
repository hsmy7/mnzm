...
| **WS-1 阶段 3 数据导向存储** | 需**单独立项**：涉及约 145+ 列写点的回归面，协议形状变更会影响全部 47 个 `Diff*` 对拍与存档格式 | 设计文档（协议 v2：列级增量/二进制编码 + 版本协商 + 前后兼容 + 回滚方案）→ 评审 → 分批实施。**⚠️ 2026-09-15 桌面实测已补齐触发条件的数据面**（§11 债项表：7.9ms@100 / 100.9ms@1000 弟子每旬，满编 2x 下每秒一次 101ms）⇒ **先取真机埋点绝对值（W4-D/D1）再决定**，阈值建议 >16ms 且落引擎线程 |
...
| WS-1 阶段 3（列级 delta / 二进制通道 + `dirty_tracker` 列级写屏障） | 145+ 列写点回归面，需协议 v2 设计文档先行 | 单独立项（§9）；**触发条件 = 真机实测证明全量实体 JSON 序列化为瓶颈**。**⚠️ 2026-09-15 桌面实测（§2.68，`dirty_tracker_bench_test.cpp` 实跑）**：`diffToJson` 每旬全脏 **7.9ms@100 弟子 / 100.9ms@1000 / 523ms@5000**；而**空闲零变更仍 328ms@5000**（`diffToJson` 恒对全状态做一次全量序列化 + 缓存树深比较，脏标记只减少"发出多少"不减少"算多少"）。**上限口径**：`GameConfigData.kt:50 maxDisciples = 1000`、旬间隔 `GameTimeClock.kt:224 MS_PER_PHASE_1X = 2000`（2x = 1s/旬）⇒ 满编存档每旬导出 ≈ **101ms，2x 下每秒一次**（≈6 帧）。**决策阈值建议**：真机埋点（D1）若测得每旬导出 >16ms 且落在引擎线程 ⇒ 立项做协议 v2；否则先做"UI 读面裁剪/廉价指纹预筛"等非协议优化 |
