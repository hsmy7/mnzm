...
| **D4** | `w3-13` 反向通道删除（终局） | 串行 | D1–D3、B、C | `captureReverseDirty`、`ReverseDirtyAccumulator`、`applyDirtyToNative`、信封构建、`GameCore::applyReverseDirty`、`ReverseChannelPolicy` | 五步流程走完；关闭域写入检测零命中；状态指纹零差异 —— **⚠️ 阶段 A+B 已实施（2026-09-15，§2.75，tag `w4-rem/05`）：观察窗仪器落地 + 100 旬停发对拍绿；删除步判定 = 阻断（稳态写者未清零，完成路径见 handover §2.75）；完成路径 1–3 项已实施（§2.76–§2.78，`w4-rem/06`–`08`）；第 4 项第一段已实施（§2.79，`w4-rem/09`）：29 项转关闭 + 漏网写者补漏 8 处接线；第二段已实施（§2.80，`w4-rem/10`）：30 项转关闭 + 12 处逐动作接线——**gameData 字段面关闭清单 = 全部传输单元**，仅余 9 类集合（第三段终段专项）**；**✅ 终局已实施（2026-09-17，§2.82，`w4-rem/12`）：第三段 9 类集合转关闭达成硬前置 → 五步④归档 tag → ⑤通道删除本体落地——反向通道不复存在**（防复发 = `MirrorReadOnlyGuardTest` + 100 旬管线稳态零写入断言；真机 E3/N5 观测面随通道删除取消）** |
| **D5** | 死代码清零 + 守卫面收口 | 串行 | D4（地形字段落定） | 见 §2.D 清单 | 三份零调用者清单产出并逐条删除；守卫只缩不增 —— **✅ 已实施（2026-09-17，§2.83，`w4-rem/13`）**：全量重扫立新基准（原 18+11+5 不可考）；前批已清偿核实 5 项免做 + 本批删 10 组 + 活代码 1 项诚实保留（登记阶段 3）；RngSource core/domain 13→6 + RngIsolation 白名单 4→1 + 计数断言落地 |
| **D6** | 文档与版本收口 | 串行（末项） | D1–D5、B、C | handover §3/§4.1/§5/§6、`ui-read-surface.md` §4.4、`cpp-engine.md` §9、`CODE_WIKI.md`、双更新日志 | 全文档口径与实测一致；双日志同批更新 —— **✅ 已实施（2026-09-17，§2.84，`w4-rem/14`）**：§7.2/§7.3 勘误补齐 + cpp-engine §9（198/maxId=1861）+ CODE_WIKI 镜像只读契约 + rng 双文档同步 + 双日志；`version.properties` 留用户决定（未递增）。**W4 可派工清单自此为空** |
...
| D4 | 严格按 ADR §4 五步：**先禁用（信封体积=0 但代码在）→ 跑完整业务周期（≥1 游戏年 + 离线结算 + 跨旬月 + 存档写读，期间关闭域写入检测零命中）→ 状态指纹零差异（同档同输入 ≥100 旬）→ 删前打归档 tag → 再删除**。删除面/保留面见 §8；**防复发**：镜像只读契约（结构性/编译期优先）+ "稳态零写入断言"进长期 CI —— **⚠️ 阶段 A+B 已实施（2026-09-15，§2.75，tag `w4-rem/05`）**：停发仪器（`ReverseChannelPolicy.reverseTransportEnabled`）+ 100 旬停发对拍用例进 CI（零发送/检测零命中/指纹零差异全绿）；**删除步判定 = 阻断**（稳态 Kotlin 写者未清零：交谈效果/任务派遣/存档前自愈等——硬前置"关闭清单 = 全部传输单元"不成立），逐写者完成路径见 handover §2.75④，收口后重启五步④⑤ —— **✅ 第 1 项已实施（2026-09-15，§2.76，tag `w4-rem/06`）：交谈效果写面下沉（`DISCIPLE_CHAT_EFFECT_TX=1860`，段 1860–1869 首用，零 RNG 参数化事务）；审计修正：通道关闭还依赖任务域收口（`startMission` 写槽位协议列）与 lifeEvents 投影检测裁决（§2.76 审计先行）——**✅ 均已实施（2026-09-15，§2.77，tag `w4-rem/07`）：`MISSION_START_TX=1861` + `DISCIPLE_CHANNEL` 关闭（守卫红线翻转）+ 投影转非捕获路径 + 检测 AUTHORITATIVE 门控；剩余开放面 = aiSectDisciples 段、9 类集合、retained 字段族（第 3/4 项）。**✅ 第 3 项已实施（§2.78，`w4-rem/08`）：aiSectDisciples 段 + worldMapSects 关闭**（写者穷尽审计 + rebaselineNativeMirror 四处接线）；sectRelations/activeSectId/sectName 保留并登记。**✅ 第 4 项第一段已实施（2026-09-15，§2.79，`w4-rem/09`）：retained 字段族逐域判定——29 项转关闭（recruitList/activeSectId/sectName/预警去重/附庸契约/宗主/玉符×4/巡逻灵矿住所槽位×4/地形×2/秘境×4/洞府队×2/弟子槽位×5/placedBuildings/spiritFieldPlants）+ §2.77/§2.78 漏网写者补漏 8 处接线（宗门改名 sectName+worldMapSects、仓库卸任迁入引擎层；enterSect/设置重置/槽位释放/取消血炼/偷盗钩子/内存裁剪 rebaseline 或 updateMirror+rebaseline）；审计修正：sectName 原"LOAD_BOOT 族"登记证伪（SectDelegate 改名 = 稳态写者，且其 worldMapSects 写入构成 §2.78 关闭的漏网写者）；第二段保留面（钱包/年度账/执法堂/兑换码/事件日志/生产槽/战斗世界域/sectRelations/9 类集合）落守卫断言防抢跑，死链 3 处登记 D5 清单。门禁：引擎 3315/0/0/0（+3）｜domain 1760/0（+2 守卫）｜**✅ 第 4 项第二段已实施（§2.80，`w4-rem/10`）：30 项转关闭 + 12 处逐动作接线（邮件/试炼 `Provider<GameEngineCore>` 惰性注入；弹窗清空 updateMirror + 消费重建；自动购买/生产槽迁引擎层 wrapper）。**✅ 第三段（终段）已实施（2026-09-15，§2.81，`w4-rem/11`）：9 类实体集合转关闭——🔴 删除步硬前置"关闭清单 = 全部传输单元"全量达成**（写者穷尽审计约 90 调用点/25 文件；稳态可达且捕获型 6 簇 7 处宿主事务 updateMirror 化 + 基线重建——攻宗占领/碾压、世界关卡胜利奖励、逐出袋物化、仓库赏赐装备/功法（无 native 臂，新增条件性 rebaseline）、贸易购买、妖兽迎战；余者六类合法形态；守卫终局断言锁死）。门禁：引擎 3315/0（持平，47 `Diff*` 绿 + 停发 100 旬闸门绿）｜domain 1762/0（+1 净）｜game 872/0｜app 1003/0｜detekt 六绿｜lint 绿｜零 C++ 改动桌面豁免——**✅ 五步④⑤已实施（2026-09-17，§2.82，`w4-rem/12`）：通道删除本体落地**。删除面：Kotlin 捕获链 + 信封发送链 + `ReverseChannelPolicy` 本体与 reversechannel 包五文件 + C++ `GameCore::applyReverseDirty` 与辅助簇四函数 + 双 JNI 导出 + GTest 13 用例 + 5 个 Kotlin 反向测试类；保留面：前向镜像/`importToNative`/`rebaselineNativeMirror`/回退臂；防复发：`MirrorReadOnlyGuardTest` 符号面守卫 + `FakeGameStateStore.nonMirrorWriteCount` 稳态零写入硬断言（进 `DiffAuthoritativeTickTest` 100 旬管线）。门禁：桌面 **1417/1417**（= 1430（§2.77 实测真值）− 13；基线表 1417 系漂移值已修注）｜引擎 **3288/0/0/0**｜domain 1752/0｜data 716/0/15｜game 872/0｜app 995/0/2｜detekt 六绿｜NDK+lint 绿｜生成物零漂移（198/maxId=1861）——**D4 终局完成，下一项 = D5** |
| D5 | 第一件事是**先补清单**（handover §5④ 的"18+11+5 处零调用者站点"仓库内**无分项清单**，无基准不可验收）：用 `git grep` 产出"生产调用 0 / 测试引用 n"三份清单再逐条删除；其余见 §2.D |
| D6 | 双更新日志**必须一起**更新（`CHANGELOG.md` + `changelog_entries.json`；同版本条目追加到 `changes` 末尾，**禁新建同版本第二条目**）；`version.properties` 是否递增**由用户决定** —— **✅ 已实施（2026-09-17，§2.84，`w4-rem/14`）**：handover §4.1 索引/§5③④/批次索引/§6 接手必读数值 + **§7.2/§7.3 勘误补齐**（6 项原位标注 + 三写者已收口 + §7.4 ②⑤⑥ 结案）+ cpp-engine §9（169→**198 动作 / maxId=1861**）+ CODE_WIKI `updateMirror` 行与 AUTHORITATIVE 只读契约块 + ui-read-surface §4.3 终局标注/§4.4 误引勘误 + rng-source-inventory §8 / rng-remediation-status 追记；`CHANGELOG.md` 增 D6 条目、游戏内 json 未追加（零玩家可见变更）；`version.properties` **未递增（留用户决定）** |
