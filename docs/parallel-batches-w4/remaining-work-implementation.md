...
### 2.B ③ `PresentationRandom` 按场景派生（跨会话一致）——**本次未实施，交他人实施**

> **状态声明**：本轮仅完成**设计与接线点实测**（含一次探针式改动，已 `git checkout` 回退干净，当前树与 `HEAD=b961a356c` 一致）。实施从零开始。
...
### 2.C ② `TimeSystem.onPhaseTick` 迁入测试源集 —— **本次未实施，交他人实施**

> **状态声明**：本轮仅完成实测与方案，**未改任何代码**。
...
| `PresentationRandom` 一致性口径 | 需拍板后实施 | 本文件 §2.B 已给出完整方案（推荐"按场景派生"）；拍板即派工 |
| `TimeSystem.onPhaseTick` 删除 | 需拍板后实施 | 本文件 §2.C 已给出完整方案（推荐"移入测试源集"而非删除） |
