...
> - **基线**：GTest **717/717**（批 Y-4a/Y-4b +5/+4 用例）· engine JUnit **3057/3057**（桌面 JNI 对拍 0 skip；DiffYearSettlementTest 换装真实 MerchantAndRecruitService + 快照功法表注入，收购 SYSTEM 消费对拍逐位一致）· NDK externalNativeBuildRelease · engine detekt 全绿 · app compileReleaseKotlin 通过——随批 Y-4 提交（git log 最新含「批 Y-4」）
> - **已完成批次**（目标核心全部达成）：批 M-1 月变真相源切换（nativeSettleMonth + 残留执行器 + S-14/16/17/20 清偿，§7.6）→ 批 Y-1 年变零 RNG 小件 11 件（§7.7）→ 批 Y-2 3/5（T1-⑨/⑩ + T2-⑪，§7.8）→ 批 Y-switch 年变真相源切换（nativeSettleYear + YearSettlementResidualExecutor，§7.9）→ 批 Y-3 部分（T1-④ 招募刷新 + T1-⑪ autoBuy 接线 + 名字生成分区化，§7.10 状态行）→ 批 Y-3 T1-③（f90c8e77，T1 11/11 全部下沉）→ **批 Y-4a T2-④ 交易刷新下沉**（generateSectTradeItems/refreshAllSectTrades 局部种子 + 丹药 price 补全 recipe_db.h + GTest 717/717 + DiffYearSettlementTest 零回归）→ **批 Y-4b T2-③ 商人收购下沉**（buildMerchantItemPools/refreshMerchantAcquisition + S-22 部分清偿（收购价格收敛 SYSTEM 分区）+ DiffYearSettlementTest 换装真实 MerchantAndRecruitService + 快照功法表注入）；目标④（executeResidual 自动丹药/突破）确认已有 C++ 等价 + DiffPhaseSettlementTest 守护
> - **剩余工作（1 件下沉 + 收尾；不阻塞——切换已达成，残留执行器运行正常）**：② T2-② AI 宗门招募（AI 独立 RNG systemSeed+6*31337 + generateRandomDisciple AI 版 + 占领路由；C++ 需补 AI 独立 RNG 播种点）⑤ 年变对拍扩展余项（Kotlin 臂换装真实 CaveExplorationProcessor/DiplomacyService——MerchantAndRecruitService 已换装）⑥ CHANGELOG 追加批 Y-4a/Y-4b 条目 + 最终验证提交（diff 面登记：收购/交易 MerchantItem.id/itemId 镜像生成排除）
> - **恢复指引**：从 §7.10「目标③完成状态」继续 T2-②；每批 = Kotlin 源码审计 → C++ 移植（year_settlement.h detail）→ GTest 黄金序列 → 桌面对拍桥（scripts/build-desktop-jni.ps1）→ engine JUnit 强制重跑（--rerun-tasks）→ NDK/detekt；工作区另有 feature/game 4 项**预存非本任务改动**（建造栏石板路置灰）未提交，勿混入迁移提交
...
- **剩余**：T2-② AI 宗门招募（AI 独立 RNG systemSeed+6*31337 + generateRandomDisciple AI 版 + 占领路由——C++ 需补 AI 独立 RNG 播种点）；年变对拍扩展余项（Kotlin 臂换装真实 CaveExplorationProcessor/DiplomacyService）；CHANGELOG 条目 + 最终验证提交
...
| ~~S-22~~ ✅ | **商人 `createMerchantItem` 价格波动非托管 RNG**（批 Y-4b 对拍途中登记）：`GameUtils.applyPriceFluctuation(adjustedPrice)` 未传 rng → JVM 全局 Random（非分区、不入 rngStates）——收购/旅行商人价格生成期随机（结果落库后固定，对存档确定性无影响），但跨语言 SYSTEM 流分叉（C++ 若消费分区价格则每 item 多 1 次 nextDouble，RNG 终态永不对齐） | `MerchantAndRecruitService.kt` | 确定性缺口（S-19 同族） | ✅ **部分清偿（批 Y-4b）**：`createMerchantItem` 加 `random: kotlin.random.Random = Random.Default` 参数——**收购路径**（refreshMerchantAcquisition，年变 T2-③ 已下沉 C++）传 `rng.asKotlinRandom()`（SYSTEM 分区确定性，跨语言逐位可对拍）；**旅行商人路径**（refreshTravelingMerchant/addGuaranteedTopRarityItem）保持默认 Random.Default（JVM 全局随机）——剩余面登记：旅行商人价格仍非托管（玩家手动刷新路径，不进结算对拍面，可随商人批次下沉时清偿） |
