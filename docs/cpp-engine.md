...
> - **引擎重构 R1 进展（2026-09-18，B02 批）**：同方案 §3 R1 第二批落地——R1.2 `committedDisciples` 去物化（突破概率唯一消费长老位 ≤2 名弟子悟性 ⇒ 入口全量 D 弟子物化快照退役，每旬 D 次深拷贝 → 0 次，`committedElderComprehensionOf` SoA 列直算入口时点值）+ R1.3 字符串键 → dense 索引（DiscipleStore `numericIds`/`numericIdToRow` 数值列与索引、热路径逐点免重解析；equipment/manual 步骤入口映射改 owner 行索引桶式存储 `instance_buckets.h`——每步 E 次实例深拷贝 + 字符串键节点退役；`idToRow` string 键仅保留协议边界）；只改形状不改结果（桌面 GTest 1425/1425 + JUnit 六模块 7777/0 含 Diff* 268 用例 0 skip），协议/JNI/存档零变更；登记见方案 §7.2
> - **引擎重构 R1 收官（2026-09-18，B03 批）**：同方案 §3 R1 第三批落地——**R1 全部条目收官**。R1.4 DirtyTracker 列级写屏障（`column_dirty.h` `ColumnDirtyTracker`：SoA 协议列枚举 109 列与 Disciple to_json 双射 + 行主序脏位图 + 集合 tombstone + gameData 域标脏；导出仅序列化脏列，协议形状与版本语义同 diffToJson；挂点 = DiscipleStore 协议边界变更原语，**生产 exportDirtyJson 仍走全量树 diff 零漂移**，热路径写点标脏随 R2）+ R1.6 `destroyDiscipleEntities` 去 O(D²)（`eraseEntityUnordered` swap-and-pop 变体，仅用于重建/批量删除等顺序无观察点场景，保序路径不变）+ R1 收官 bench（`GAMECORE_BUILD_BENCH` kover 模式门禁：全局 operator new 计数替换，**每旬结算 D=5000 修炼热路径 17 次 malloc / ~1.8ms——G1 < 1 万达成**；带实例清单残差 85017 为桶节点+pending 提交管线的数据驱动分配，登记观察项；R1.3 收尾——每旬入口 4 处 `indexById` 全量重建逐点替换为直用 `numericIdToRow` 免重建缓存）；只改形状不改结果（桌面 GTest 1443/1443 + JUnit 六模块 7777/0 含 Diff* 47 类 268 用例 0 skip 实跑），协议/JNI/存档零变更；登记见方案 §7.2
> - **引擎重构 R4.2（2026-09-18，B04 批）**：同方案 §3 R4 表秘境战斗行落地——`SecretRealmService` 战斗执行段（妖兽战 PvE / AI 宗门 PvP 两分支）经统一入口 `executeRouted` 切 `BattleExecutionRouter` → `nativeBattleExecute`（与 R4.1 同路由与灰度契约：flag 关 / native 未加载 / 失败信封回退 Kotlin `executeBattleWithTimeout` 臂，臂保留一个版本周期；灰度 flag = 既有 `NativeEngineFlag.authoritative`，零新增）；会话管理/UI/邮件/暂停租约留 Kotlin 平台域，秘境会话既有 native 事务通道不动；RNG 委托式约定零调整（分区独立属 R4.4）。零协议/JNI/存档变更、战斗结果逐位一致（桌面 GTest 1443/1443 + JUnit 六模块 7780/0 含 Diff* 47 类 268 用例 0 skip 实跑）；登记见方案 §7.2
> - **引擎重构 R4.3（2026-09-18，B05 批）**：同方案 §3 R4 表探索/巡逻生产行落地——探索/巡逻**结果生产评估**切 `BattleExecutionRouter` → `nativeBattleExecute`（参照系 = AI 兽战处理器既有 native 优先模式；R4.1/R4.2 同路由与灰度契约）：`ExplorationService.createBeastBattle`（妖兽防守战生产）+ `PatrolBattleSystem` 三处（巡逻楼普通 PvE / 冲突战 Phase 1 PvP / Phase 2 PvE）；flag 关 / native 未加载 / 失败信封回退 Kotlin `executeBattle` 臂，臂保留一个版本周期；灰度 flag = 既有 `NativeEngineFlag.authoritative`，零新增。探索会话管理/UI/通知/奖励生成/伤亡写回/关卡刷新/攻击检测留 Kotlin 平台域；洞府探索评估结论 = 留 Kotlin（会话管理 + `System.nanoTime` 非分区随机域）；RNG 委托式约定零调整（分区独立属 R4.4）。零协议/JNI/存档变更、探索/巡逻结果逐位一致（桌面 GTest 1443/1443 + JUnit 六模块 7785/0 含 Diff* 47 类 268 用例 0 skip 实跑）；登记见方案 §7.2
> - **引擎重构 R2.1 + R2.2（2026-09-18，B06 批）**：同方案 §3 R2 表 GameView proto 定义 + 镜像通道换 protobuf 行落地（前置 = B03 R1.4 列级写屏障）——① R2.1 定义 `GameView` proto（`core/engine/src/main/proto/game_view.proto`，选型 protobuf 不引 flatbuffers）：镜像信封四块结构 resourcesHeader / discipleListDelta（typed `DiscipleRow` 109 字段行级增量 + removed）/ eventFeed（本批 schema 预留，R2.4 接线）/ configEcho，其余集合与 gameData 字段经 collectionChange/gameDataChange 扩展区（v1 JSON 载荷过渡）承载；文件头声明 **schema 演进纪律「字段只增不改」**（方案 §5 风险条款）+ proto3 optional 显式 presence + 集合恒 repeated 不用 map（§7.3）；gamecore 手写零依赖 wire 编码器 `gameview_encode.h/.cpp`（表驱动 kDiscipleRowFields 与 proto 双向锁定、字段号升序 + nlohmann 键有序遍历确定性逐字节相同），DirtyTracker 抽 `diffToTree` 使 JSON 导出与 protobuf 信封共用同一树源（双格式逐值等价由构造保证）；② R2.2 镜像通道换轨：生产 JNI `nativeExportDirty` 改走 `GameCore::exportDirty()` 分发（**返回签名不变、仅字节载荷编码换轨**），Kotlin `StateSyncService.applyDirtyProto` 经 `GameViewMirrorCodec` 还原同形变更集树复用同一 applier，**存档格式不动**（`nativeExportState` 全量 JSON 仅留存档/rebaseline），**UI 消费面不动**（镜像仍全量，R2.3 才瘦身）。**灰度开关 `NativeEngineFlag.mirrorProtobufTransport`（默认 true=换轨生效；false=旧 JSON 回滚臂，新旧共存一个版本周期）**；**JNI 面豁免登记**：新增 `nativeSetDirtyExportProtobuf` 引擎线程控制 setter（与 `nativeSetAiThermalBatchSize` 同族，沿 R0.2 探针先例）。守卫：`gameview_encode_test.cpp`（9 用例，最小 wire 解码器按字段/值锁定 + 证明产出合法 protobuf）+ `DiffDirtyEnvelopeEquivalenceTest`（3 用例，编码面富树 protobuf 往返逐值 deep-equal + 应用面真实 native 双解码对照，桌面 JNI 0 skip）+ `DirtyTrackerBench.MirrorTransportJsonVsProtobuf`（观测：每旬全脏 5000 弟子 JNI 传输 protobuf 2.26MB vs JSON 12.3MB 缩至 1/5.5 + 消费者省全量 JSON parse，净 mirror 段非劣化、趋势向 G2；**G2 <10ms 终态由 R2.3 瘦身达成**）。桌面 GTest 1453/1453 + JUnit 六模块 7788/0（engine 3307 含 Diff* 48 类 271 用例 0 skip，桥携本批 C++ 重建）+ detekt/lint/compile 绿；协议 JSON 面/存档格式零变更、镜像逐值等价；登记见方案 §7.2

> - **引擎重构 R2.3 第一波（2026-09-18，B07 批）**：同方案 §3 R2 表 R2.3 行第一波落地——**UI 消费面完成二进制传输切换（镜像仍全量），本批生产代码零变更**（切换实质由 B06 完成，本批补齐守卫与证明）。① **消费面审计**（`docs/mirror-consumer-audit-2026-09-18.md` 馈送点清单 F1–F7）：热路径稳态馈送（每旬 tick / `tryExecuteNative`+`executeRaw` 动作后 / 月变 / 年变后，13 个调用文件同走 `StateSyncService.applyDirtyFromNative`）已 100% 由 GameView protobuf 信封馈送；**唯一残余 JSON 点 = F3 全量快照兜底臂**（`syncFromNative`→`applySnapshot`，仅增量臂返回 null 时触发，实测 0/12 旬），判为设计上保留（R2.2 声明 `nativeExportState` 全量 JSON 留低频路径；切它需 C++ 全量视图编码器 + 应与第二波 GameViewStore 投影域同定），故本批**不切不瘦身**，`GameStateStore` 全量 replaceAll 一字未改；F4 月年信封属 R2.4、F5 动作结果信封非馈送点、F6 反向回导、F7 渲染属 R3。② **守卫三件**：`MirrorProtoFeedEquivalenceTest`（Robolectric + `MirrorProtoFeedFixture`/`MirrorDiscipleRowFixture` 同事实源夹具）锁 **proto 信封 → 解码 → GameStateStore 馈送**与旧 JSON 路径逐字段同形同值（弟子整行 109 协议字段恒设 = proto 契约覆盖度钉、集合 upsert/remove、gameData 标量+容器、`DirtyApplyResult` 计数、单事务原子 + 防两臂同错的期望值断言）；`DiffMirrorArmConvergenceTest`（桌面 JNI 0 skip）锁 12 旬真实结算下 F1/F2/F3 **三臂全等** + 版本号序列单调 + 信封非 JSON 文本；`MirrorConsumerSurfaceGuardTest` 把审计结论静态化（`nativeExport*` 拉取点唯一 = StateSyncService、`proto.gameview` import 唯一 = GameViewMirrorCodec、UI 模块对镜像符号零命中、灰度双分支与旗标默认值保留）。③ **观测固化**：传输字节比 0.20（proto 18493B vs JSON 91480B，与 B06 bench 5000 弟子 1/5.5 同向）、增量臂 mirror 稳态中位 2.6ms/旬（首封 194ms 为 protobuf 运行时一次性初始化）、兜底臂 3.2ms/旬，两臂断言 <100ms 告警线。**诚实边界**：`collectionChange.upsertsJson`/`gameDataChange.valueJson`/`storageBagItemsJson` 仍是"二进制信封内嵌 JSON 原文"（R2.1 v1 过渡编码），typed 化与 replaceAll 退役同属第二波，**G2 <10ms 终态待 R2.3 第二波**。桌面 GTest 1453/1453（零 C++ 变更、基线持平）+ JUnit 六模块 7794/0（engine 3313 含 Diff* 49 类 272 用例 0 skip）+ detekt/lint/compile 绿；JNI 面/协议 JSON 面/存档格式/灰度旗标语义零变更；登记见方案 §7.2

> - **引擎重构 R2.3 第二波（2026-09-18，B08 批）**：同方案 §3 R2 表 R2.3 行**第二波**落地——**镜像瘦身：GameViewStore 投影态 + 每旬级全量重建退场 + UI 消费块逐块迁移**（批次文件 `docs/parallel-batches-w5/batch-R2C.md`）。① **每旬级全量重建退场**：gameData 应用由「整份 GameData JSON encode→覆盖变更键→整份 decode」（137 个序列化字段含 `gameEventRecords`/`recruitList`/`worldMapSects` 等巨型容器，每旬整树往返两次，只为改 3~5 个标量）换成 **`GameDataFieldPatch` 字段级应用**（一次 `copy()` 浅拷贝 + 本封变更字段逐个解码；表体 = 序列化字段全集，逐字段同一 `Json` 实例 + 同一 serializer ⇒ 与整份解码逐值等价；未知键宽松忽略、任一在册字段解码失败整组丢弃，两语义同旧臂）；弟子行由 **`GameViewDiscipleRows` typed 直读**取代「每行 109 键 JsonObject 造树 + kotlinx 结构解码」；`replaceAll` 全表替换经静态门禁锁定为**只存在于低频全量兜底臂 F3**（10 个实体集合；每旬增量臂出现任一 `.replaceAll(` 即红）。② **GameViewStore 投影态**（core/engine 新 `gameview` 包，Hilt 单例）替代 GameStateStore 全量快照的"UI 唯一入口"地位：块①资源头部（三阶灵石 + 年/月/旬）/ 块②配置回声（政策·年俸(+开关)·长老槽·放置建筑·招募灵根）/ 块③事件流当前载体（`gameEventRecords`）；proto 块 3 `eventFeed` 仍待 R2.4（`PROTO_EVENT_FEED_BLOCK = reserved-not-produced` 显式登记，不长出消费面）。镜像提交后**只重投本封触及的块**；馈送点全在镜像链单一入口 `StateSyncService`（增量 `project` / 兜底 `reprojectAll` / 基线建立点 `reprojectAll`——换档走 `loadFromSnapshot` 不发事务，故必须在 `importToNative` 收敛，否则暂停态读档后 HUD 停在上一档头部值）；注册 `TransactionObserver` 做**非镜像写入对账**（提交世代 ≠ 最近镜像世代即按 store 快照重投，`reconciliationCount` 稳态恒 0）。③ **ViewModel 逐块迁移**（三块三 commit 各自可回滚）：`spiritStoneTotals`←`gameEngine.resourcesHeader`、`configState`←`gameEngine.configEcho`、`gameEventRecords`←`gameEngine.eventLog`，切换点全在 GameEngine 转发面（UI 代码零语义改动），回滚臂经**同一纯函数**从整份快照派生同一视图 ⇒ 只换来源不换值。④ **fail-fast 红线（方案 §5）三处落地**：行必在标量 93 项 ↔ 编码表标量面逐项双射（缺即抛错点名缺失字段）、gameData 写入器键集 ↔ `GameData.serializer().descriptor.elementNames` 双射、投影块声明字段掉出镜像协议面即抛错——禁止以默认值掩盖；抛出经降级契约转为"本封增量失败 → 全量兜底"，不污染镜像。⑤ **G2 趋势实测**（`MirrorSegmentProjectionBenchTest` = B06 `DirtyTrackerBench` 的 Kotlin 消费侧对应面，同一封每旬全脏信封两臂分阶段计时）：**D=1000 旧臂 117.81ms → 新臂 47.99ms（−59%）；D=5000（信封 2,177,797B）旧臂 343.13ms → 新臂 132.68ms（−61%）**；**G2 <10ms@5000 未达成**——残余成本中心 = 每旬全脏 5000 行 `upsertMirrorRow`（109 列写屏障）+ store 侧 O(D) 列表组装 + C++ 侧全量序列化基线（R1.4 列级导出未接生产），终态判定与 WS-1 关闭留 R2 收官。灰度：**新增 `NativeEngineFlag.gameViewProjection`（默认 true = 第二波形态；false = 第一波形态回滚臂，共存一个版本周期）**，`mirrorProtobufTransport` 语义与默认值零变更、两旗标正交。**登记缺陷（按红线原样复刻、不顺手改行为）**：旧整份 GameData 解码必然把 5 个 @Transient 运行态字段（`slotId`/`autoSaveIntervalMonths`/`aiBeastEncounterTargets`/`battleTeam`/`aiBattleTeams`）**每旬打回声明默认值**，修复须另批裁决并跑全量对拍。守卫：`GameDataFieldPatchGuardTest` 5 + `GameViewDiscipleProjectionTest` 4（含真实 C++ 结算信封逐行 presence 实证）+ `GameViewStoreGuardTest` 6 + bench 内两臂落库全等；B06/B07 馈送等价与三臂收敛守卫全绿未改语义（仅把"三键稀疏新弟子"夹具随 emit-always 契约收敛为全字段两臂同源）。桌面 GTest **1453/1453**（`ninja: no work to do` 证零 C++ 变更、基线持平）+ JUnit 六模块 **7812 / 0 失败 / 17 既有跳过**（engine 3331 含 Diff* 49 类 272 用例 0 skip；组合门 339 任务全 executed 24m09s）+ detekt/lint/compile 绿；**JNI 面 / 协议 JSON 面 / 存档格式零变更**；登记见方案 §7.2
> - **引擎重构 R2.4 + R2 收官（2026-09-18，B09 批）**：同方案 §3 R2 表 R2.4 行 + R2 验收行落地（批次文件 `docs/parallel-batches-w5/batch-R2D.md`；前置 = B06/B07/B08）。① **eventFeed 转正（月/年 JSON 信封并入事件流）**：C++ `GameCore` 事件队列——月结 MONTH_SETTLED（policyCosts.disabledPolicies + 征伐环 seizedSectBuildings）/ PURCHASE×N / SECRET_REALM_CLOSED，年结 YEAR_SETTLED（丧亲草稿）/ DEATH×N（死亡链草稿全字段含 storageBagItems 完整协议），突破经 `gameEventRecords.sequenceId` 水位增量收割（phase 结算钩子，导入推到最大防旧档重放）；`encodeGameView` 增补 eventFeed 参数（field 4，缺省 nullptr 不产出 = R2.1/R2.2 历史字节不变）；**导出即消费**（编码成功清空、异常保留重试）；JSON 回滚臂不入队（信封 JSON 面零变更红线）。Kotlin：codec 解 eventFeed 为 typed `GameViewStreamEvent`（detailJson v1 过渡编码在 codec 一处解析）→ `GameViewStore.recordEvents`（应用成败与否均先入队）→ `buildMonth/YearEnvelopeFromEvents` **零 JSON 解析**组装执行器信封——以 MONTH/YEAR_SETTLED 为在场证明，无证明（回滚臂/事件丢失）回退旧 `parseXxxEnvelope`（登记保留，删除随回滚臂批次）；执行器输入形状逐字段等价，**执行器源零 JSON 解析由 `ResidualExecutorPurityGuardTest` 静态固化**（信封解析面收敛登记边界）。② **G2 列级导出接生产**：`DirtyTracker` 抽 `diffTreeSegments`/`stateWithoutDisciplesToJson` 共享段（gameData/集合域与全量 diff 同一循环体 = 构造等价）；`ColumnDirtyTracker` 整树导出 `exportDirtyTree(GameState)`（弟子域仅脏行×脏列；位图升级 std::atomic——并行核心批次多线程置位丢位修复，扩容仅串行段、resetBaseline 清零保留容量防并行扩容竞争）；**结算热路径写点 markCol 精确标脏**（修炼累积/HP·MP 恢复/自动装备写回/亲属赠送/偷盗链）+ 月/年边界审计列集粗粒度标脏（44 列并集全行）+ **异构路径锁存**（execute 分发/战斗/招募下一封回退全量）；`exportDirtyProto` 混合分发（列级开且无锁存走列级树，否则全量树 diff）；灰度 `NativeEngineFlag.dirtyColumnExport`（默认 true）经 **nativeSetDirtyExportColumn**（JNI 豁免登记，沿 nativeSetDirtyExportProtobuf 先例）推送。Kotlin 列级消费：弟子行以 `DiscipleRowPatch` 补丁交付，`applyDisciplePatches` 以 store 既有行 assemble 为基线 proto mergeFrom 覆盖（repeated 先清后并）、全行走同一 toDisciple 投影。③ **守卫**：C++ `ColumnExportEquivalenceTest`（core 模式 300 弟子 × 40 旬跨月/年真实结算逐封列级↔全量双臂对照：无漏报 + 多报同值 + gameData/集合域逐位相等）+ `DiffColumnExportMergeConvergenceTest`（Kotlin 12 旬双 store 收敛）+ `GameViewEncodeTest.EventFeedEmittedWhenProvided` + bench 列级臂。**G2 判定：未达标（诚实登记）**——列级信封 5000 弟子 123,906B vs 全脏 2,177,797B（**−94%**）、decode 段 27.87→1.37ms（**−95%**）、mirror 段合计 139.30ms vs B08 132.68ms（apply 段波动带内持平），**<10ms 未达成**；残余成本中心 = ① Kotlin `upsertMirrorRow` 全组列写（writeAllFields 109 列/行未随列级收窄）② store 侧 O(D) assembleAll（测试替身常数）③ C++ rest 域（gameData+集合）每封序列化仍在 ④ 弟子列表块未迁投影（后两项及 ① 属 R2.3 第二波既登记残余的延续，写点级标脏 C++ 侧已就位、收益兑现待 Kotlin 列级落表 + 弟子块投影迁移）；**WS-1 保持悬置**（<10ms 未达成不虚报）。桌面 GTest **1453/1453**（携 -ffp-contract=off；本构建目录基线 1450 + 新增 3：encode 事件 1 + 列级等价 2）+ JUnit 六模块全绿（engine 含新增 ResidualExecutorPurityGuardTest/EventEnvelopeAssemblyTest/DiffColumnExportMergeConvergenceTest/bench 列级臂；组合门 --rerun-tasks 全 executed）+ detekt/lint/compile 绿；JNI 面仅新增 1 控制端口 + 桌面对拍桥 1 守卫入口（登记豁免）；**存档格式/协议 JSON 面/存档兼容零变更**；登记见方案 §7.2
> - **引擎重构 R3.1 + R3.2（2026-09-19，B10 批）**：同方案 §3 R3 表前两行落地（批次文件 `docs/parallel-batches-w5/batch-R3A.md`）——**R3 渲染场景所有权下沉的结构性开端**。① R3.1 新建 `app/src/main/cpp/scene/scene_store.h`（native-renderer 内新模块，零 Android 依赖桌面可测）：持有 terrain（与 gamecore 地形单一权威同值一次性导入）/road mask/建筑集/作物集/崖壁布局/云实例六类场景状态，协议步长与旧 drawAllTiles 17 参数面逐一同形，整表替换 + 防御清空 + `reset()` 纪元复位；**只做逐值存储，绘制判定集中单份核心**。② R3.2 旧 drawAllTiles 函数体逐段收敛 `scene/scene_draw.h` 单份绘制核心（buildMapBatch + buildCliffLayer）——旧路径（JNI 数组装配）与新路径（SceneStore + 生成表装配）消费同一构建逻辑，像素等价由构造保证；JNI 面 8 端口（sceneSetTerrain 一次性 / sceneUpdateBuildings/Crops/Roads/Clouds / sceneSetCliffLayout 变化驱动导入 / sceneSetAtlasTexture / drawFrame(相机 5 标量+overlayFlags+2 alpha)——**每帧 ≈36B，G3 <200B 达成**；8 端口统一登记豁免沿 R0.2/B06 先例；drawAllTiles 标记 deprecated 保留一个版本周期）；UV 常量表由 build-atlas.mjs 同源生成进 C++（`scene/scene_uv_tables.h` 仓库内生成物，五张 UV 表+占地表+渲染常量，Kotlin 不再每帧传 SpriteAtlasDef 数组）；Kotlin `NativeEngineFlag.sceneStoreRender` 灰度旗标（默认 true，false = 旧路径回滚臂共存一个版本周期；Vulkan/GLES 双 GPU 后端切新路径，**Canvas 兜底路径不动 = R3.6 红线**）。③ 等价性守卫：C++ `SceneEquivalenceTest` 10 用例（新 vs 旧**顶点流逐位对照**：六要素单要素 × 相机三档位 + 全要素两帧 + 降级路径 + overlay 位 + 要素在场证明 + 生成表↔Kotlin 公式夹具对照）+ Kotlin `SceneUvTablesMirrorGuardTest` 5 用例（生成头 ↔ SpriteAtlasDef toRawBits 逐位对照，单侧生成物漏提交即红）。桌面 GTest **1476/1476**（携 -ffp-contract=off；基线 1456 + 20）+ 组合门全绿 + NDK arm64 重建；JNI 面新增 8 端口登记豁免；存档格式/协议 JSON 面零变更；登记见方案 §7.2
...
JNI 桥 GameCoreBridge (通用入口：init/advance/execute/export/import/poll + Clock 注入)
  ↓
★ C++ GameCore（纯 C++20、零 Android 依赖、桌面可编译；Android 为主，核心保持可移植）—— 确定性计算真相源
  ↓  版本号 + 变更集（增量：**GameView protobuf 二进制信封**，R2.1/R2.2；灰度旗标
  ↓    `mirrorProtobufTransport=false` 时回退旧 JSON 文本，新旧共存一版本）
  ↓  或全量快照（JSON，仅兜底/存档/rebaseline 低频路径）
Kotlin StateSyncService → GameStateStore（镜像写入，接口/StateFlow 不变）
  ├─ R2.3 第二波（B08）消费侧形状：gameData **字段级**应用（一次浅拷贝 + 本封变更
  │   字段逐个解码，整份 GameData JSON 往返退场）+ 弟子行 **typed 直读**
  │   （DiscipleRow → Disciple，每行 109 键 JsonObject 造树退场）；
  │   全表 replaceAll 只留在低频全量兜底臂 F3（静态门禁锁死每旬增量臂零命中）
  └─ → GameViewStore 投影态（按 UI 消费块拆分：资源头部/配置回声/事件流；proto
      块 3 eventFeed R2.4 转正 → 已迁 UI 块经 GameEngine 转发读投影，未迁块仍读
      GameStateStore 全量流；灰度旗标 `gameViewProjection`（默认 true，false =
      第一波形态回滚臂，共存一个版本周期）
  ↓
...
| 11-1b ✅（2026-09，手动招募失效修复批 R-1） | **手动招募/一键招募下沉 C++ 单真相源**（修复"招募界面点同意无效果"——与批 11-1 自动招募同侧）：`system/recruit_settlement.h` 新增 `manualRecruitFromList`（上限/按 id 查找/损坏条目同事务移除/allocateAndInsert（资质散列补算）/materializeCaptiveGear/按 id + 同人签名净化列表/计数与年报 +1，`ManualRecruitReason` 结果枚举 + `minReasonableAge` 软校验）+ `manualRecruitAll`（sanitizeRecruitList 等价：损坏移除/三级去重/跨表已入宗残留（isSamePerson 分组）+ 上限 + 逐个招募）；`GameCore::manualRecruitFromList/manualRecruitAll`（JSON 信封 ok/newId/age/name/reason）；Kotlin `DiscipleFacadeImpl.recruitDiscipleFromList`/`GameEngine.recruitAllFromList` 改 AUTHORITATIVE JNI 转发（信封解析 `ManualRecruitEnvelope.kt`/`RecruitAllEnvelope.kt`）+ native 不可用/UNKNOWN 回退 Kotlin 原实现（双实现并行契约）+ 镜像 lifeEvents 补写；`DiscipleDelegate` 防抖占位移入协程实际执行处（engineScope 取消窗口零残留——旧实现点击时占位永久残留致后续点击全部静默拦截）+ 拦截经 `onRecruitBlocked` 可见化；**反向通道加固**：C++ `applyReverseDirty` 版本推进后移（失败不推进——旧实现失败时版本已前进致增量回导永久 version mismatch）+ C++ `importStateInternal`/Kotlin `importToNative` 成功后双方版本归零（全量导入=重同步基线，防跨会话错位） | GTest +17（`manual_recruit_test.cpp` 15 用例：单招成功/上限/不存在/损坏 purge/同人双胞胎/一键净化+配额截断/信封/零 RNG 契约 + `apply_reverse_dirty_test.cpp` 2 版本时序守卫）· **DiffAuthoritativeTickTest 新增手动招募双臂对拍场景**（native 招募→前向镜像→一旬 ①-⑤→C++ 导出 vs Kotlin 基准逐字段一致）· engine JUnit 新增 ManualRecruitEnvelopeTest/RecruitAllEnvelopeTest/DiscipleDelegateRecruitGuardTest + GameEngineCoordinationTest 回退契约用例 · compileReleaseKotlin 通过 · 兼容性：无 Entity/Migration/存档变更；lifeEvents 仍为 Kotlin 瞬态域（镜像侧补写） |
...
| 批次 | 域 | ActionId 段 | C++ 事务头 | GTest |
|---|---|---|---|---|
| S4/S7 | 生产排程（手动排班/重置 + 完成结算/自动排班） | 1443–1444 | `system/production.h` | `production_test.cpp` |
| S6 | 秘境交互会话 | 1440–1442 | `system/secret_realm_session.h` | `secret_realm_session_test.cpp` |
| batch-06 | 建筑放置/迁移/升级/拆除 | 1450–1454 | `system/building_tx.h` | `building_tx_test.cpp`（18） |
| batch-07 | 道路放置/拆除 | 1470–1471 | `system/road_tx.h` | `road_tx_test.cpp`（16） |
