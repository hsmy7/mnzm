...
**目标③完成状态**：年变真相源切换达成（C++ runYearSettlement 为生产真相源 + Kotlin 残留执行器互插）；**批 Y-3（2026-09-01，T1-④/⑪ 完成）**：T1-⑪ autoBuy 接线（merchant_settlement.h 批 11-3 executeAutoBuy 接入年变 T1 #11 位）+ T1-④ 招募刷新下沉（`detail::processRefreshRecruitList`——等级招募范围 + 长老魅力加成（含职务加成乘算）+ 广纳门徒政策 +50% + 逐弟子生成链（SYSTEM 分区串行：性别/名字 generateName（FULL 姓氏 nextInt + 给定名 nextDouble+nextInt）/灵根 SpiritRootGenerator/年龄/DiscipleFactory 固定序）+ recruitList 追加 + lastRecruitYear + 惰性重置 + processAutoRecruit；差值判据 3 年内部）；**Kotlin 分区化（S-19 同族清偿）**：NameService.pickSurname 加 rng 参数（原 pool.random() JVM 全局 Random 非确定性）+ generateName 传 rng + refreshRecruitList 传 SYSTEM 分区适配器（`rng.asKotlinRandom()`，与给定名/弟子生成同源）——招募名字从非确定变确定性（存档可复现）；GTest 704/704（+4 T1-④ 黄金序列）+ engine JUnit 全量（桌面 JNI 0 skip）+ NDK + detekt + app compileReleaseKotlin；**T1 下沉 10/11**（除 T1-③ 死亡链大件）+ T2 6/11（除 T2-② AI 招募 / T2-③ 商人收购 / T2-④ 交易刷新——后两者需静态数据补全：丹药价格/普通材料表）；对拍扩展（年变 Kotlin 臂换装真实服务）随续作合并。
