...
| 12-6 ✅ | **对拍框架长期化**：`scripts/build-desktop-jni-linux.sh`（Linux g++ 构建桌面对拍桥，镜像 Windows 版源列表）+ CI 新增 `cpp-diff-jni-test` job（构建 JNI 桥 → `-Dgamecore.jni.path` 实跑 engine 全量含 Diff*Test 0 skip）——对拍框架从"本地手动"升级为 CI 长期回归基线 | CI job 配置落地（随 push 首跑验证）；本地等价命令已实测通过 |
