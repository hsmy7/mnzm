...
> - **基线**：GTest **722/722**（批 Y-4a/b/c +9+5 用例）· engine JUnit **3057/3057**（桌面 JNI 对拍 0 skip；DiffYearSettlementTest 换装真实 MerchantAndRecruitService + CaveExplorationProcessor，收购/AI 招募 SYSTEM·AI-RNG 消费对拍逐位一致）· NDK externalNativeBuildRelease · engine detekt 全绿 · app compileReleaseKotlin 通过——随批 Y-4 提交（git log 最新含「批 Y-4」）
> - **已完成批次**（目标核心全部达成）：批 M-1 月变真相源切换（nativeSettleMonth + 残留执行器 + S-14/16/17/20 清偿，§7.6）→ 批 Y-1 年变零 RNG 小件 11 件（§7.7）→ 批 Y-2 3/5（T1-⑨/⑩ + T2-⑪，§7.8）→ 批 Y-switch 年变真相源切换（nativeSettleYear + YearSettlementResidualExecutor，§7.9）→ 批 Y-3 部分（T1-④ 招募刷新 + T1-⑪ autoBuy 接线 + 名字生成分区化，§7.10 状态行）→ 批 Y-3 T1-③（f90c8e77，T1 11/11 全部下沉）→ 批 Y-4a T2-④ 交易刷新下沉 → 批 Y-4b T2-③ 商人收购下沉（S-22 部分清偿 + DiffYearSettlementTest 换装真实 MerchantAndRecruitService）→ **批 Y-4c T2-② AI 宗门招募下沉**（AI 独立 RNG + generateRandomAiDisciple + applyGearToAiDisciple + 占领路由 + S-19 同族清偿（AI 名字分区化）+ S-22 完全清偿（旅行商人价格收敛 SYSTEM）+ DiffYearSettlementTest AI 招募对拍场景——**年变残留执行器扇出清零**）；目标④（executeResidual 自动丹药/突破）确认已有 C++ 等价 + DiffPhaseSettlementTest 守护
> - **剩余工作（收尾；不阻塞——年变 T1/T2 全量下沉达成，残留执行器仅剩 T1-③ 死亡链平台效应）**：CHANGELOG 追加批 Y-4a/b/c 条目 + 最终验证提交（diff 面登记：收购/交易/AI 弟子 id 镜像生成字段排除；AI 独立 RNG 不入 rngStates）
> - **恢复指引**：无未下沉年变编排扇出；后续增量 = 战斗批次 D 评估续作/年变对拍 Kotlin 臂换装余项（DiplomacyService 真实——场景 sectDetails 空规避已零回归，纯增强可选）；每批 = Kotlin 源码审计 → C++ 移植（year_settlement.h detail）→ GTest 黄金序列 → 桌面对拍桥（scripts/build-desktop-jni.ps1）→ engine JUnit 强制重跑（--rerun-tasks）→ NDK/detekt；工作区另有 feature/game 4 项**预存非本任务改动**（建造栏石板路置灰）未提交，勿混入迁移提交
...
- **剩余**：~~T2-② AI 宗门招募~~（✅ **批 Y-4c 完成，见下**）；年变对拍扩展余项（Kotlin 臂换装真实 DiplomacyService——MerchantAndRecruitService/CaveExplorationProcessor 已换装）；CHANGELOG 条目 + 最终验证提交
