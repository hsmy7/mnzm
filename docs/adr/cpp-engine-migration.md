...
> 日期：2026-08-16。状态：已批准实施（批次 0-9 核心完成；计划 v2 阶段 0-7 已完成——AUTHORITATIVE 生产默认 + 引擎循环入 C++ + 渲染 RHI；**迁移主线（确定性逻辑核心）全部收口——注意：非"自研跨平台引擎整体目标"完成，引擎级现状与后续工作计划见 docs/cpp-engine.md §0**；执行路径见 docs/cpp-engine.md 第 7 节）。
...
7. **双实现并行 + 差分对拍 + feature flag 切换**：~~任何时刻可回退。Kotlin 引擎退役修订为职责边界固化（2026-08-25）~~ → **🔴 二次修订（2026-08-25 选项 A，权威口径；本条下三行旧表述已作废，2026-09-15 事实核查更正文内互斥）**：**终态 = C++ 单一真相源（AUTHORITATIVE 生产默认）+ Kotlin 引擎逻辑退役**；Kotlin 侧最终保留面 = UI/Compose + 平台能力（支付/广告/通知/网络/墙钟读数）+ 存档编码（kotlinx-proto，T-CPP-1）+ 输入桥 + **native 不可用时的降级回退臂**
   - C++ 接管：时间推进/结算引擎（确定性核心）、业务操作事务（玩家行为入口，逐域下沉中）、静态数据单一源
   - Kotlin 保留（**过渡期**）：未下沉域的写路径（**当前事实**：288 个写入点经反向通道回导，见 [reverse-channel-elimination](reverse-channel-elimination.md)）、UI 链路、存档编码
   - ~~不做 Kotlin 引擎全量删除——职责边界冻结后双端并行成为**最终架构**而非过渡态~~ → **作废**：SHADOW 对拍态与纯 Kotlin 旬结算路径已删除（批 9-1/9-2），对拍框架转为长期回归基线；**双实现并行是过渡态，退役对象 = 反向同步通道（w3 计划）**
...
- ~~双实现并行成为最终架构~~ → **终态为 C++ 单一真相源 + Kotlin 引擎逻辑退役**（二次修订，2026-08-25 选项 A）；双实现并行与反向同步通道为**过渡态**，退役计划见 [reverse-channel-elimination](reverse-channel-elimination.md)

负面/成本：
- C++ 引擎已交付 ~7 万行 + 测试（**桌面 GTest 1322 + JUnit 对拍 47 类**，2026-09-15 实测）
- 静态数据双份（Kotlin Registry + C++ 表，双端守卫防漂移；T-CPP-2 决定单一源时机）
- ~~未迁移系统保持 Kotlin 实现（职责边界显式登记）~~ → 未下沉域仍由 Kotlin 写状态并依赖**反向同步通道**回导（w3 计划逐域收尾中）
...
## 追加决策：UI 层（含文本输入框）维持 Compose/KMP（2026-09）

决策：文本输入框/对话框/导航等 **UI 层不迁移 C++**，维持 Compose（Kotlin）；跨平台（iOS）路径为 Compose Multiplatform/KMP 已预留。

理由（与既有边界规则一致，rules/cpp-priority.md「UI 层（Compose）只能用 Kotlin……不属于规则例外」）：
- C++ 侧无字体/排版/光标选区系统，「C++ 输入框」= 为输入重造整套 C++ 文本栈 + 平台 IME 通道（Android AGDK GameTextInput / iOS UITextInput 会话），而输入框的键盘交互本质仍是平台 IME——与语言无关；
- game-core「零 Android 依赖、桌面可编译、iOS 可复用」的立身约束不容平台 UI 栈污染；
- 本次 IME 异常（界面闪烁/键盘反复弹收）的根因是窗口架构（游戏 Surface 与输入共窗 + adjustResize + 跨窗口状态污染），与输入框实现语言无关，已于窗口层根治。

C++ 侧输入框仅在以下触发条件成立时引入（当前均不成立）：
- **触发 A**：架构转舵为「引擎自绘全套 UI」（放弃 Compose 层，UI 全部下沉 Vulkan/Canvas 渲染管线）；
- **触发 B**：出现与游戏场景同空间的 3D/世界内 UI 且规模超混合层承载力（仅部分 UI 下沉，全屏对话框仍留平台层）；
- **触发 C**：明确放弃 Compose Multiplatform 跨平台 UI 路线。
