    ...
    companion object {
        fun fromAppError(error: AppError): UiError {
            val (message, severity, recoverable) = when (error) {
                is AppError.Domain.Storage -> mapStorageError(error)
                is AppError.Domain.Network -> mapNetworkError(error)
                is AppError.Domain.Production -> mapProductionError(error)
                is AppError.Domain.Validation -> mapValidationError(error)
                is AppError.Domain.GameState -> mapGameStateError(error)
                is AppError.Domain.GameLoop -> mapGameLoopError(error)
                is AppError.Domain.Disciple -> mapDiscipleError(error)
                is AppError.Domain.Inventory -> mapInventoryError(error)
                is AppError.Domain.Building -> mapBuildingError(error)
...
private fun mapInventoryError(
    ...
    is AppError.Domain.Inventory.Insufficient ->
        Triple("物品不足：需${error.need}，有${error.have}", UiErrorSeverity.INFO, false)
