...
@Dao
interface GameDataCoreDao {
    @Query("SELECT * FROM game_data_core WHERE slot_id = :slotId LIMIT 1")
    fun getBySlotId(slotId: Int): Flow<GameDataCore?>

    @Query("SELECT * FROM game_data_core WHERE slot_id = :slotId LIMIT 1")
    suspend fun getBySlotIdSync(slotId: Int): GameDataCore?

    @Query("SELECT slot_id, sectName, gameYear, gameMonth, gameDay, spiritStones, spiritHerbs, sectCultivation, isGameStarted, lastSaveTime FROM game_data_core WHERE slot_id = :slotId LIMIT 1")
    suspend fun getMetadataBySlot(slotId: Int): GameDataMetadataProjection?

    @Query("SELECT slot_id, sectName, gameYear, gameMonth, gameDay, spiritStones, spiritHerbs, sectCultivation, isGameStarted, lastSaveTime FROM game_data_core ORDER BY slot_id ASC")
    suspend fun getAllMetadata(): List<GameDataMetadataProjection>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(core: GameDataCore)

    @Update
    suspend fun update(core: GameDataCore)

    @Query("DELETE FROM game_data_core WHERE slot_id = :slotId")
    suspend fun deleteAll(slotId: Int)

    @Query("DELETE FROM game_data_core")
    suspend fun deleteAllGlobal()
}

@Dao
interface GameDataWorldMapDao {
    @Query("SELECT * FROM game_data_world_map WHERE slot_id = :slotId LIMIT 1")
    suspend fun getBySlotId(slotId: Int): GameDataWorldMap?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(worldMap: GameDataWorldMap)

    @Update
    suspend fun update(worldMap: GameDataWorldMap)

    @Query("DELETE FROM game_data_world_map WHERE slot_id = :slotId")
    suspend fun deleteAll(slotId: Int)

    @Query("DELETE FROM game_data_world_map")
    suspend fun deleteAllGlobal()
}

@Dao
interface GameDataBuildingsDao {
    @Query("SELECT * FROM game_data_buildings WHERE slot_id = :slotId LIMIT 1")
    suspend fun getBySlotId(slotId: Int): GameDataBuildings?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(buildings: GameDataBuildings)

    @Update
    suspend fun update(buildings: GameDataBuildings)

    @Query("DELETE FROM game_data_buildings WHERE slot_id = :slotId")
    suspend fun deleteAll(slotId: Int)

    @Query("DELETE FROM game_data_buildings")
    suspend fun deleteAllGlobal()
}

@Dao
interface GameDataEconomyDao {
    @Query("SELECT * FROM game_data_economy WHERE slot_id = :slotId LIMIT 1")
    suspend fun getBySlotId(slotId: Int): GameDataEconomy?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(economy: GameDataEconomy)

    @Update
    suspend fun update(economy: GameDataEconomy)

    @Query("DELETE FROM game_data_economy WHERE slot_id = :slotId")
    suspend fun deleteAll(slotId: Int)

    @Query("DELETE FROM game_data_economy")
    suspend fun deleteAllGlobal()
}

@Dao
interface GameDataOrganizationDao {
    @Query("SELECT * FROM game_data_organization WHERE slot_id = :slotId LIMIT 1")
    suspend fun getBySlotId(slotId: Int): GameDataOrganization?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(organization: GameDataOrganization)

    @Update
    suspend fun update(organization: GameDataOrganization)

    @Query("DELETE FROM game_data_organization WHERE slot_id = :slotId")
    suspend fun deleteAll(slotId: Int)

    @Query("DELETE FROM game_data_organization")
    suspend fun deleteAllGlobal()
}

@Dao
interface GameDataExplorationDao {
    @Query("SELECT * FROM game_data_exploration WHERE slot_id = :slotId LIMIT 1")
    suspend fun getBySlotId(slotId: Int): GameDataExploration?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(exploration: GameDataExploration)

    @Update
    suspend fun update(exploration: GameDataExploration)

    @Query("DELETE FROM game_data_exploration WHERE slot_id = :slotId")
    suspend fun deleteAll(slotId: Int)

    @Query("DELETE FROM game_data_exploration")
    suspend fun deleteAllGlobal()
}
