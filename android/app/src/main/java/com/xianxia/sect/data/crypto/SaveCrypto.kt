 ...
 * | 优先级 | 问题 | 修复方案 |
 * |--------|------|----------|
 * | P0 | 双缓存不一致风险 (derivedKeyCache + precomputedKeys) | 合并为单一 unifiedKeyCache |
 * | P0 | Argon2id 使用错误的 PBEKeySpec 参数 | 改用 Argon2ParameterSpec (RFC 9106) |
 * | P1 | 缓存 TTL(2min) 与清理间隔(5min) 不一致 | 统一为 5min |
 * | P1 | 冗余 @JvmStatic (object 单例中无意义) | 移除 |
 * | P2 | init 块启动协程影响测试 | 改为懒初始化 (首次使用时启动) |
 * | NEW | 密钥轮换无版本化支持 | 引入 KeyVersion 枚举和版本路由 |
 * | NEW | 无现代 AEAD 备选方案 | 预留 XChaCha20-Poly1305 接口 |
 *
 * ## PBKDF2 / Argon2id 迭代次数选择说明
 *
 * ### 迭代次数设计原则
 * - **安全性**: 迭代次数越高，暴力破解成本越高（线性关系）
 * - **性能**: 迭代次数直接影响密钥派生耗时，需在安全与用户体验间平衡
 * - **当前值**: 310,000 次（OWASP 2023 推荐最低值，主流设备上约 200-350ms）
 *
 * ### 各版本迭代次数
 *
 * | 常量名 | 值 | 用途 |
 ...
 * ### 迭代次数版本化策略
 *
 * 当未来需要再次调整 PBKDF2_ITERATIONS 时，应遵循以下流程：
...
object SaveCrypto {
     ...
     *
     * 优势（相比 AES-GCM）：
     * - 24 字节 nonce（vs GCM 的 12 字节），随机生成即可，无需计数器
     * - 无 IV 重用风险（nonce 空间 2^96，远超 GCM 的 2^32 安全限制）
     * - 纯软件实现，无 AES-NI 硬件依赖（ARM 设备性能稳定）
     * - IETF RFC 8439 标准化，2025 年推荐用于移动端
     *
     * 可用性：Android API 26+ (BouncyCastle 或 Conscrypt 实现)
     */
    ...
    /** 密钥长度：256位（AES-256 / ChaCha20 均为 256 位） */
    private const val KEY_SIZE = 256

    /** GCM 认证标签长度：128位（最大强度） */
    private const val GCM_TAG_LENGTH = 128

    /** GCM 初始化向量长度：12字节（96位，GCM推荐值） */
    private const val GCM_IV_LENGTH = 12

    /**
     * XChaCha20-Poly1305 nonce 长度：24字节（192位）
     *
     * XChaCha20 使用扩展 nonce（XNonce），由 16 字节密钥派生的子密钥 + 8 字节随机 nonce 组成。
     * 随机生成的 24 字节 nonce 空间极大（2^192），无需担心重用问题。
     */
    ...
    /**
     * 当前标准 PBKDF2 迭代次数：310,000（OWASP 2023 推荐值）
     *
     * 选择依据：
     * - OWASP 2023 密码存储指南推荐最低 310,000 次（HMAC-SHA256）
     * - OWASP 2021 推荐最低 60,000 次（已过时）
     * - 在中端 Android 设备（骁龙865级别）上约 200-350ms
     * - 在低端设备上约 500-800ms（可接受，PBKDF2 仅作为 Argon2id 的 fallback）
     ...
     * - >= 4GB RAM: 64MB（默认高安全性）
     *
     * 变更记录 (v2->v3): 从 16MB 提升至 64MB 默认值，
    ...
    /** Argon2id 迭代次数：3（配合高内存参数，符合 RFC 9106 推荐） */
    private const val ARGON2ID_ITERATIONS = 3

    /** Argon2id 输出长度：32 字节（256 位密钥） */
    ...
    // ==================== 统一缓存架构 (v3 重构) ====================
    //
    // v2 问题：derivedKeyCache 和 precomputedKeys 是两个独立的 ConcurrentHashMap，
    //         存在状态不一致风险 -- 一个有数据另一个没有，或 TTL 过期时间不同步。
    //
     ...
     * 统一缓存数据结构 (v3)
     *
     * 合并了原 CachedDerivedKey 和 PrecomputedKeyEntry 的功能，
     ...
     * - cacheKey 由 password + salt 的哈希构成
     * - 包含派生密钥、创建时间戳、原始盐值、来源标识
     * - TTL = 5 分钟（与清理间隔一致）
     *
     * 线程安全保证：
     * - ConcurrentHashMap 保证单个操作的原子性
     * - 复合操作（检查-然后-操作）通过函数式方法保证一致性
     */
     ...
     * 3. 使用 HKDF 将两个密钥合并为最终加密密钥
     * 4. 执行 AES-GCM 加密
     *
     * @param data 待加密数据
     * @param password 用户密码
     * @param context Android Context（用于访问 Keystore）
     * @return 加密后的数据（格式：[version(1)] [salt(32)] [iv(12)] [ciphertext] [hmac(32)]）
     */
    @Throws(CryptoException::class)
     ...
     * 异步加密 - 使用密码
     *
     * 将 Argon2id 密钥派生操作移至 Dispatchers.Default（IO优化线程池），
     ...
     *
     * v3 重构变更：
     * - 不再维护独立的 precomputedKeys 缓存
     * - 直接写入 unifiedKeyCache，source 标记为 PRECOMPUTED
     * - 消除了双缓存状态不一致的风险
     *
     * 使用示例：
     * ```kotlin
     ...
     * 清除所有预计算密钥 (v3: 操作统一缓存)
     *
     * 仅清除 source=PRECOMPUTED 的条目，保留 DERIVED 条目。
     */
     ...
     * 由 ensureCleanupInitialized() 按需触发。
     */
     ...
     * 清理过期的缓存条目 (v3: 操作统一缓存)
     *
     * @param ttlMs 自定义TTL（毫秒），默认使用统一的 5 分钟 TTL
     * @return 清除的过期条目数
     */
     ...
     *
     * 查找顺序（单一缓存源）：
     * 1. 统一缓存 unifiedKeyCache（包含 PRECOMPUTED 和 DERIVED 来源）
     * 2. 未命中则实时 Argon2id 派生
     * 3. 派生结果存入统一缓存（source=DERIVED）
     *
     * v3 重构消除了原来先查 precomputedKeys 再查 derivedKeyCache 的两阶段查找，
     * 减少了一次 ConcurrentHashMap.get() 开销，同时消除了状态不一致的可能。
     */
    ...
    /**
     * 使用指定迭代次数派生密钥（支持 legacy 格式缓存，v3: 统一缓存）
     */
     ...
     *
     * 安全注意事项：
     * - PBEKeySpec 包含敏感信息（密码字符数组），必须在使用后立即清除
     * - 使用 clear() 方法将字符数组归零
     */
    ...
    /**
     * Argon2id 密钥派生 (v3: 修复参数规格，使用 Argon2ParameterSpec)
     *
     * ## v3 关键修复
     *
     * **P0 问题**：v2 版本在 API >= 30 时使用 `SecretKeyFactory.getInstance("Argon2id")`
     * 配合 `PBEKeySpec` 传递参数。但 `PBEKeySpec` **不包含 memoryKiB 和 parallelism 参数**，
     * 导致 Argon2id 实际运行时可能使用了不安全的默认参数（如 memory=16MB 而非预期的 64MB），
     * 或者完全忽略了这些关键的安全参数。
     *
     * **v3 修复**：改用 `Argon2ParameterSpec`（RFC 9106 标准），正确传递所有参数：
     * - `memoryKiB`: 内存成本（根据设备自适应）
     * - `parallelism`: 并行度（固定为 2）
     * - `iterations`: 迭代次数（固定为 3）
     * - `outputLength`: 输出长度（32 字节 = 256 位）
     * - `salt`: 盐值（通过 Argon2ParameterSpec 传递）
     *
     * Password 仍需通过 `PBEKeySpec` 传递（因为 Argon2ParameterSpec 不包含 password 字段）。
     *
     * ## 参数配置（自适应）
     *
     * - Android API 30+: 使用系统内置 Argon2id 实现 + 正确的 Argon2ParameterSpec
     * - Android API < 30: fallback 到 PBKDF2-60K（增强安全性）
     *
     * ## OWASP 2025 推荐参数
     *
     * | 参数 | 值 | 说明 |
     * |------|------|------|
     * | memoryKiB | 65536 (64MB) | 高端设备默认，抵抗 GPU/ASIC |
     * | parallelism | 2 | 平衡性能与安全性 |
     * | iterations | 3 | 配合高内存参数 |
     * | outputLength | 32 bytes | 256 位密钥 |
     *
     * @param password 用户密码
     * @param salt 随机盐值（32 字节）
     * @param context 可选的 Android Context（用于设备内存检测），若为 null 则使用保守默认值
     * @return 派生的 256 位密钥
     */
    fun deriveKeyArgon2id(password: String, salt: ByteArray, context: Context? = null): ByteArray {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            try {
                ...
                // Argon2ParameterSpec 包含所有 RFC 9106 要求的参数：
                // memoryKiB, parallelism, iterations, outputLength, salt
                //
                // Password 通过 PBEKeySpec 传递（Argon2ParameterSpec 不包含 password）
                @Suppress("NewApi")
                val argon2Spec = try {
                ...
                // iterations 和 keySize 也由 argon2Spec 控制，此处仅作兼容性占位
    ...
    /**
     * 根据设备总内存解析 Argon2id 应使用的内存参数
     *
     * 分级策略（v3 更新：提升高端设备默认值至 64MB）：
     * - < 2GB RAM: 8MB（避免 OOM，低端设备安全优先）
     ...
     *
     * @param context 可选的 Android Context（用于 ActivityManager 内存查询），
     *                若为 null 或 API < 30 则使用保守默认值
    ...
    fun resolveEffectiveMemoryKib(context: Context?): Int {
        if (context == null || Build.VERSION.SDK_INT < Build.VERSION_CODES.R) {
            return ARGON2ID_MEMORY_KIB_DEFAULT  // API < 30 或无 Context：使用 OWASP 推荐默认值 64MB
        }
        ...
        return try {
            ...
            when {
                ...
                totalMemMB < 4096 -> {
                    ...
                    // v3: 从 16MB 提升至 64MB（OWASP 2025 推荐）
            ...
            ARGON2ID_MEMORY_KIB_DEFAULT  // 检测失败时使用 OWASP 推荐默认值 64MB
        }
    }
     ...
     * 从已有密钥通过 HKDF 派生子密钥
     *
     * HKDF（HMAC-based Key Derivation Function）基于 RFC 5869
     * 用于从主密钥派生出多个独立的子密钥
     */
     ...
     * 2. Expand：使用 PRK 和 info 字符串扩展出输出密钥材料（OKM）
     *
     * @param masterKey 输入主密钥
     * @param salt 可选盐值（可为空）
    ...
    /**
     * 构建缓存键：password + salt 的 SHA-256 哈希组合
     *
     * 使用双重哈希确保：
     * - 密码明文不会出现在缓存键中
     * - 固定长度便于 HashMap 性能优化
    ...
    /**
     * 检查 XChaCha20-Poly1305 是否可用
     *
     * XChaCha20-Poly1305 是 2025 年推荐的现代 AEAD 算法，
     * 相比 AES-GCM 具有以下优势：
     * - 24 字节随机 nonce（无需计数器管理）
     * - 纯软件实现（无 AES-NI 依赖，ARM 设备性能稳定）
     * - IETF RFC 8439 标准化
     *
     * 可用条件：Android API 26+ 且 BouncyCastle 或 Conscrypt 提供者可用
     */
     ...
     * @param data 待加密数据
     * @param password 用户密码
     * @return 加密后的数据，如果不支持则抛出 CryptoException
     */
    @Throws(CryptoException::class)
    fun encryptXChaCha20(data: ByteArray, password: String): ByteArray {
        ...
        return try {
            ...
            // 注意：不能使用 GCMParameterSpec，那是为 AES-GCM 设计的
     ...
     * 时序安全的字节数组比较（防时序攻击）
     *
     * 标准的 Arrays.equals() 或 contentEquals() 会短路返回，
