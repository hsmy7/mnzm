...
class MemoryCache(private val maxSize: Int) {
    ...
    private val cache: LruCache<String, CacheEntry> = object : LruCache<String, CacheEntry>(maxSize) {
        ...
        override fun entryRemoved(evicted: Boolean, key: String, oldValue: CacheEntry, newValue: CacheEntry?) {
            if (evicted) {
                Log.d(TAG, "Entry evicted: $key, size=${oldValue.size}")
                onEntryEvicted(key, oldValue)
            }
        }
    }
