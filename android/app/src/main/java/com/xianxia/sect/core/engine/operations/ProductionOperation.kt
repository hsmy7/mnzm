...
class ProductionOperation(
    ...
    private fun getSlotCount(buildingType: BuildingType): Int = when (buildingType) {
        BuildingType.ALCHEMY -> 3
        BuildingType.FORGE -> 2
        BuildingType.MINING -> 3
        BuildingType.HERB_GARDEN -> 6
        else -> 1
    }

    private fun isValidSlotIndex(buildingType: BuildingType, slotIndex: Int): Boolean =
    ...
    suspend fun startProductionAtomic(
        ...
        if (!isValidSlotIndex(buildingType, slotIndex)) {
            return ProductionOperationResult.Failure(
