...
class DeltaCompressor {
    ...
    private fun serializeFieldDelta(dos: DataOutputStream, delta: FieldDelta) {
        ...
        if (hasOldValue) {
            dos.writeInt(delta.serializedOldValue!!.size)
            dos.write(delta.serializedOldValue)
        }
        
        dos.writeBoolean(hasNewValue)
        if (hasNewValue) {
            dos.writeInt(delta.serializedNewValue!!.size)
            dos.write(delta.serializedNewValue)
        }
    }
