...
enum class DataZone(
    ...
    COLD(
        displayName = "冷数据区",
        description = "仅数据库，ZSTD压缩，延迟加载",
        priority = 3,
        defaultTtlMs = 3600_000L * 24 * 7,
        supportsCompression = true,
        defaultAlgorithm = CompressionType.ZSTD
    );
...
enum class CompressionType(
    ...
    fun toCacheCompressionAlgorithm(): com.xianxia.sect.data.cache.CompressionAlgorithm {
        return when (this) {
            LZ4 -> com.xianxia.sect.data.cache.CompressionAlgorithm.LZ4
            ZSTD -> com.xianxia.sect.data.cache.CompressionAlgorithm.ZSTD
        }
    }
