...
object SerializationConfig {
    ...
    val MODE_FAST = SerializationMode.BINARY
    val MODE_COMPACT = SerializationMode.BINARY_COMPRESSED
    val MODE_COMPAT = SerializationMode.JSON
...
enum class DataTypeCode(val code: Byte) {
    ...
    companion object {
        fun fromCode(code: Byte): DataTypeCode {
...
data class SerializationContext(
    val mode: SerializationMode = SerializationConfig.MODE_FAST,
    val needRandomAccess: Boolean = false,
    val needCompact: Boolean = false,
...
class SmartSerializer {
    ...
    private val binarySerializer = BinarySerializer(json)
    private val jsonSerializerImpl = KotlinJsonSerializerImpl(json)
    
    fun <T : Any> serialize(data: T, context: SerializationContext = SerializationContext()): SerializationResult {
        val selectedMode = selectMode(data, context)
        ...
        return try {
            when (selectedMode) {
                ...
                SerializationMode.JSON -> jsonSerializerImpl.serialize(data)
            ...
            jsonSerializerImpl.serialize(data)
        }
    }
    ...
    fun <T : Any> deserialize(data: ByteArray, clazz: Class<T>, context: SerializationContext = SerializationContext()): T? {
        ...
        return try {
            ...
            when (mode) {
                ...
                SerializationMode.JSON -> jsonSerializerImpl.deserialize(data, clazz)
    ...
    private fun <T : Any> selectMode(data: T, context: SerializationContext): SerializationMode {
        return when {
            context.needCompatibility -> SerializationMode.JSON
            context.needCompact -> SerializationMode.BINARY_COMPRESSED
            context.needRandomAccess -> SerializationMode.BINARY
            context.mode == SerializationMode.PROTOBUF -> SerializationMode.PROTOBUF
            else -> autoSelectMode(data)
        }
    }
    
    private fun <T : Any> autoSelectMode(data: T): SerializationMode {
        val estimatedSize = estimateSize(data)
        return if (estimatedSize > SerializationConfig.COMPRESSION_THRESHOLD) {
            SerializationMode.BINARY_COMPRESSED
        } else {
            SerializationMode.BINARY
        }
    }
    
    private fun <T : Any> estimateSize(data: T): Int {
        return when (data) {
            is String -> data.toByteArray(Charsets.UTF_8).size
            is ByteArray -> data.size
            is List<*> -> data.size * 64
            is Map<*, *> -> data.size * 128
            else -> 256
        }
    }
...
class BinarySerializer(private val json: Json) {
    ...
    private fun serializeValue(value: Any?, dos: DataOutputStream) {
        when (value) {
            null -> {
                dos.writeByte(DataTypeCode.NULL.code.toInt())
            }
            is Boolean -> {
                dos.writeByte(DataTypeCode.BOOLEAN.code.toInt())
                dos.writeBoolean(value)
            }
            is Byte -> {
                dos.writeByte(DataTypeCode.BYTE.code.toInt())
                dos.writeByte(value.toInt())
            }
            is Short -> {
                dos.writeByte(DataTypeCode.SHORT.code.toInt())
                dos.writeShort(value.toInt())
            }
            is Int -> {
                dos.writeByte(DataTypeCode.INT.code.toInt())
                writeVarInt(dos, value)
            }
            is Long -> {
                dos.writeByte(DataTypeCode.LONG.code.toInt())
                writeVarLong(dos, value)
            }
            is Float -> {
                dos.writeByte(DataTypeCode.FLOAT.code.toInt())
                dos.writeFloat(value)
            }
            is Double -> {
                dos.writeByte(DataTypeCode.DOUBLE.code.toInt())
            ...
            is Map<*, *> -> {
                ...
                writeVarInt(dos, value.size)
                value.forEach { (k, v) ->
                    serializeValue(k, dos)
                    serializeValue(v, dos)
                }
            }
            else -> {
                dos.writeByte(DataTypeCode.OBJECT.code.toInt())
                serializeObject(value, dos)
            }
        }
    }
    
    private fun serializeObject(obj: Any, dos: DataOutputStream) {
        val typeName = obj::class.qualifiedName ?: "Unknown"
    ...
    private fun deserializeValue(dis: DataInputStream, clazz: Class<*>): Any? {
        ...
        return when (DataTypeCode.fromCode(typeCode)) {
            ...
            DataTypeCode.STRING -> {
                val len = readVarInt(dis)
                val bytes = ByteArray(len)
                dis.readFully(bytes)
                String(bytes, Charsets.UTF_8)
            }
            DataTypeCode.BYTE_ARRAY -> {
                val len = readVarInt(dis)
                val bytes = ByteArray(len)
                dis.readFully(bytes)
                bytes
            }
            DataTypeCode.INT_ARRAY -> {
                val len = readVarInt(dis)
                IntArray(len) { readVarInt(dis) }
            }
            DataTypeCode.LONG_ARRAY -> {
                val len = readVarInt(dis)
                LongArray(len) { readVarLong(dis) }
            }
            DataTypeCode.LIST -> {
                val len = readVarInt(dis)
                (0 until len).map { deserializeValue(dis, Any::class.java) }
            }
            DataTypeCode.MAP -> {
                val len = readVarInt(dis)
                (0 until len).associate {
                    val key = deserializeValue(dis, Any::class.java)
                    val value = deserializeValue(dis, Any::class.java)
                    key to value
                }
            }
            DataTypeCode.OBJECT -> {
    ...
    private fun writeVarInt(dos: DataOutputStream, value: Int) {
        var v = value
        while (v and -0x80 != 0) {
            dos.writeByte(v and 0x7F or 0x80)
            v = v ushr 7
        }
        dos.writeByte(v)
    }
    ...
    private fun writeVarLong(dos: DataOutputStream, value: Long) {
        ...
        while (v and -0x80L != 0L) {
            dos.writeByte(v.toInt() and 0x7F or 0x80)
            v = v ushr 7
        }
...
class KotlinJsonSerializerImpl(private val json: Json) {
    @OptIn(InternalSerializationApi::class)
    fun <T : Any> serialize(data: T): SerializationResult {
        @Suppress("UNCHECKED_CAST")
        val serializer = data::class.serializer() as KSerializer<T>
...
class BufferPool {
    private val pool = ConcurrentHashMap<Int, ByteBuffer>()
    private val maxPoolSize = 16
    
    fun acquire(): ByteBuffer {
        return pool.values.firstOrNull()?.also { pool.remove(it.capacity()) }
            ?: ByteBuffer.allocate(SerializationConfig.BUFFER_INITIAL_SIZE)
    }
    
    fun release(buffer: ByteBuffer) {
        if (pool.size < maxPoolSize && buffer.capacity() <= SerializationConfig.BUFFER_MAX_SIZE) {
            buffer.clear()
            pool[buffer.capacity()] = buffer
        }
    }
