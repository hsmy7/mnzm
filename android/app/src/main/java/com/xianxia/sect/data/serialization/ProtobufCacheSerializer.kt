...
class ProtobufCacheSerializer @Inject constructor() {
    ...
    fun <T : Any> serialize(
        ...
        val (compressedData, usedCompression) = if (rawData.size >= context.compressThreshold) {
            ...
            rawData to CompressionType.LZ4
        }
...
class LightweightCompressionLayer {
    ...
    fun compress(data: ByteArray, algorithm: CompressionType): Pair<ByteArray, CompressionType> {
        return when (algorithm) {
            CompressionType.LZ4 -> compressLZ4(data) to CompressionType.LZ4
            CompressionType.ZSTD -> compressZstd(data) to CompressionType.ZSTD
        }
    }
    
    fun decompress(data: ByteArray, algorithm: CompressionType, originalSize: Int): ByteArray {
        return when (algorithm) {
