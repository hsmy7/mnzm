        ...
        for (tier in 1..6) {
            ...
            for (grade in PillGrade.entries) {
                ...
                pills.add(PillTemplate(
                    ...
                    description = "${tierName}${g}修炼速度丹，提升境界修炼速度${(applyGrade(speedPct, grade) * 100).roundToInt()}%，持续90天",
                ...
                pills.add(PillTemplate(
                    ...
                    description = "${tierName}${g}功法速度丹，提升功法熟练度修炼速度${(applyGrade(speedPct, grade) * 100).roundToInt()}%，持续90天",
                    price = (tierPrice(tier) * grade.priceMultiplier).roundToInt(),
                ...
                pills.add(PillTemplate(
                    ...
                    description = "${tierName}${g}孕养速度丹，提升装备孕养等级修炼速度${(applyGrade(speedPct, grade) * 100).roundToInt()}%，持续90天",
                    price = (tierPrice(tier) * grade.priceMultiplier).roundToInt(),
            ...
            for (tier in 1..6) {
                ...
                for (grade in PillGrade.entries) {
                    ...
                    pills.add(PillTemplate(
                        ...
                        rarity = rarity,
                        pillType = pillType,
                        description = "${tierName}${g}${attrName}丹，增加${val_}点${attrName}，持续90天",
                        price = (tierPrice(tier) * grade.priceMultiplier).roundToInt(),
                        duration = 3,
                        cannotStack = true,
            ...
            for (tier in 1..6) {
                ...
                for (grade in PillGrade.entries) {
                    ...
                    pills.add(PillTemplate(
                        ...
                        description = "${tierName}${g}${config.descName}丹，增加${v1}点${config.attr1}和${v2}点${config.attr2}，持续90天",
        ...
        for (tier in 1..6) {
            ...
            for (grade in PillGrade.entries) {
                ...
                pills.add(PillTemplate(
                    ...
                    rarity = rarity,
                    pillType = "critRate",
                    description = "${tierName}${g}暴击率丹，增加${(cr * 100).roundToInt()}%暴击率，持续90天",
                    price = (tierPrice(tier) * grade.priceMultiplier).roundToInt(),
                    critRateAdd = cr,
                    duration = 3,
                    cannotStack = true,
