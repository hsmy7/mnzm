            ...
            for (tier in 1..6) {
                ...
                for (grade in PillGrade.entries) {
                    ...
                    pills.add(PillTemplate(
                        ...
                        rarity = rarity,
                        pillType = pillType,
                        description = "${tierName}${g}${attrName}丹，增加${val_}点${attrName}，持续9旬",
                        price = (tierPrice(tier) * grade.priceMultiplier).roundToInt(),
                        duration = 3,
                        cannotStack = true,
        ...
        for (tier in 1..6) {
            ...
            for (grade in PillGrade.entries) {
                ...
                pills.add(PillTemplate(
                    ...
                    rarity = rarity,
                    pillType = "critRate",
                    description = "${tierName}${g}暴击率丹，增加${(cr * 100).roundToInt()}%暴击率，持续9旬",
                    price = (tierPrice(tier) * grade.priceMultiplier).roundToInt(),
                    critRateAdd = cr,
                    duration = 3,
                    cannotStack = true,
                ...
                pills.add(PillTemplate(
                    ...
                    rarity = rarity,
                    pillType = "critEffect",
                    description = "${tierName}${g}暴击效果丹，增加${(ce * 100).roundToInt()}%暴击效果，持续9旬",
                    price = (tierPrice(tier) * grade.priceMultiplier).roundToInt(),
                    critEffectAdd = ce,
                    duration = 3,
                    cannotStack = true,
