...
class MapCameraState(
    val scaleRange: ClosedFloatingPointRange<Float> = 0.5f..3f
) {
    ...
    private val maxOffsetX: Int get() = (canvasWidth.toInt() - screenWidth).coerceAtLeast(0)
    private val maxOffsetY: Int get() = (canvasHeight.toInt() - screenHeight).coerceAtLeast(0)
    ...
    fun focusOnWorld(worldX: Float, worldY: Float) {
        ...
        offsetX = (screenWidth / 2f - canvasX).coerceIn(-maxOffsetX.toFloat(), 0f)
        offsetY = (screenHeight / 2f - canvasY).coerceIn(-maxOffsetY.toFloat(), 0f)
        hasInitialized = true
    }
    ...
    fun applyDrag(dx: Float, dy: Float) {
        if (maxOffsetX > 0 || maxOffsetY > 0) {
            offsetX = (offsetX + dx).coerceIn(-maxOffsetX.toFloat(), 0f)
            offsetY = (offsetY + dy).coerceIn(-maxOffsetY.toFloat(), 0f)
        }
    }

    fun applyZoom(zoomFactor: Float, centroid: Offset) {
        val newScale = (scale * zoomFactor).coerceIn(scaleRange.start, scaleRange.endInclusive)
        ...
        offsetX = offsetX.coerceIn(-maxOffsetX.toFloat(), 0f)
        offsetY = offsetY.coerceIn(-maxOffsetY.toFloat(), 0f)
    }
