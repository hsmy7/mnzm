...
object ChangelogData {
    val entries: List<ChangelogEntry> = listOf(
        ChangelogEntry(
            ...
            date = "2026-05-27",
            changes = listOf(
                "新增仓库建筑（3x2格/1000灵石/无上限），半屏驻守界面含1个驻守弟子防盗战斗槽位",
                "宗门仓库容量改为动态50+50×仓库数，超限物品直接遗失+弹窗通知",
                "灵植长老移至天枢殿同行管理，灵植阁改为半径6格光环建筑（1个灵植弟子）",
                "种子成熟时间按品级分档（凡品3年→天品120年），详情由月改为年显示",
                "月俸忠诚每月±1（原每3次），矿工每月-1忠诚，入住住所每月+1忠诚",
                "偷盗触发改为道德+忠诚双门槛，脱离改为忠诚概率判定",
                "灵矿场/锻造坊/炼丹炉/灵植阁/仓库移除建造上限，建造栏自动关闭",
                "招募界面按钮自适应等宽，灵植弟子/驻守弟子新增详情按钮",
                "性能优化：SideEffect消除主线程bitmap绘制，UI采样降频减少Compose重组",
                "修复safeDropColumns部分设备DROP COLUMN崩溃、种植卡片不显示、弟子选择限制等问题"
            )
        ),
        ChangelogEntry(
            version = "3.1.20",
