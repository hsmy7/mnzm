...
| selection_highlight | 普通点击选中金色描边（动态叠加，不烘焙 chunk） | ✅ | ✅ | ✅ | 2026-08-10 WP3（RenderFlags 双端开关 + 总线脏帧防错位）；**2026-09 R3.3/B11**：Vulkan/GLES 几何改由 C++ `scene_draw.h::buildOverlayLayers` 生成（overlayFlags bit5 驱动，占地矩形/线宽/alpha 常量在 C++ 算）；**B18-臂β**：旧逐 rect 绘制臂（`drawSelectionHighlight` 等四辅助函数）已退役删除，恒走 C++ 生成几何 |
...
| grid_overlay | 放置/移动模式全视口网格线（RenderFrame.gridOverlayVisible 标志驱动，范围钳制到世界边界；Canvas drawLine / Vulkan 薄 quad） | ✅ | ✅ | ✅ | 2026-08-11（从 Compose GridOverlay 迁移至 native——同帧同相机，消除拖拽相位差）；**2026-09 R3.3/B11**：逐列/逐行 rect 改由 C++ 生成并合批（消每帧最坏 258 次 drawRect 跨线，overlayFlags bit1 驱动）；**同批修复前置缺陷 A**——行范围补乘俯视 Y 压缩系数（旧 Vulkan 漏乘致视口底部缺横线、与 Canvas 不一致），**网格线口径由 SceneOverlayProtocolGuardTest 在仍存的两路（Canvas 兜底 / C++）上锁定**（Vulkan 逐 rect 路已随 B18-臂β 退役） |
...
| texture_compression | Vulkan GPU 图集 ASTC 4x4 LDR 压缩（KTX1 容器全字段校验，16MB→4MB；设备不支持/资产损坏全链回退 RGBA 视觉零差异；**Canvas / RGBA 回退臂像素源自 2026-09 B15 起为构建期离线产物（`atlas-rgba-raw.bin` + mip 链），运行时只做一次性解码/映射 + upload，零 Canvas 拼装、零逐精灵循环**） | ✅ | ➖（仅 Vulkan 路径压缩；像素源走离线 RGBA 产物 2048²，`sourceScale 0.5` 契约零变化） | ✅ | 2026-08-10 WP7（KtxLoader 校验 + AtlasManifestSyncTest 守卫 + 构建管线 build-atlas.mjs）；**2026-09 B15**：AtlasOfflineRgbaSyncTest 7 用例（产物几何/mip 级联/golden/静态门禁）+ verify-offline-rgba-equivalence.mjs 四层全绿（3a 逐位复现 41/41）；`SectAtlasAssembler` 退役 338→111 行 |
| gesture_pan | 拖拽平移 | ✅ | ✅ | ✅ | 手势引擎共用 |
| gesture_tap | 点击建筑 | ✅ | ✅ | ✅ | 手势引擎共用 |
| gesture_longpress | 长按拖动 | ✅ | ✅ | ✅ | 手势引擎共用 |
| gesture_fling | 惯性滑行 | ✅ | ✅ | ✅ | 手势引擎共用 |
| shared_constants | 双端共享渲染常量 codegen 收敛（LOD 阈值/阴影常量/瓦片索引/语义建筑索引——LAYOUT 单一数据源，Kotlin SpriteAtlasDef + C++ TextureAtlas.h 双产物自动一致） | ✅ | ✅ | ✅ | 2026-08-13 批次 2（SpriteCodegenSyncTest 双端常量全等守卫）；**2026-09 R3.3/B11**：新增 LAYOUT.overlay 30 项叠加层视觉常量（颜色/不透明度/线宽），双端生成 scene_uv_tables.h 与 SpriteAtlasDef，**B18-臂β 后消费者 = C++ 几何生成（四类全消费）+ Canvas 兜底手绘**（Kotlin 逐 rect 绘制臂已退役）⇒ 两路共用单源（SceneUvTablesMirrorGuardTest 逐位锁） |
