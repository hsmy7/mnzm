...
/**
 * 宗门地图地面 REPEAT 源缩放的回归守卫（防「地图变成米色空隙」）。
 *
 * ## 背景
 * [com.xianxia.sect.ui.game.sect.SectAtlasAssembler] 为防低端机 OOM 把图集按
 * atlasBitmapScale = ATLAS_BITMAP_MAX_EDGE / ATLAS_W = 2048/4096 = 0.5 缩放到 2048，但
 ...
 * 源矩形未缩放时米色占比显著；正确实现地面应连续纯地面色，米色占比≈0。
 *
...
class ReproGroundScaleTest {
    ...
    fun `ground fill covers whole view - no cream gaps when atlas is 0_5 scaled`() {
        ...
        // 源矩形未缩放（地面源裁到相邻槽位+透明）时米色占比显著（>10%）。正确实现应 <1%。
        assertTrue(
