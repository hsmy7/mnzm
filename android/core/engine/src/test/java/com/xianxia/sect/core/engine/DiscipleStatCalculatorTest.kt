...
class DiscipleStatCalculatorTest {
    ...
    fun `calculateQingyunPeakBonus - 内门弟子有长老加成`() {
        ...
        val elder = createDisciple(discipleType = "inner",
            realm = 9).let { it.copy(skills = it.skills.copy(teaching = 90)) }
    ...
    fun `calculateQingyunPeakBonus - 死亡长老无加成`() {
        val disciple = createDisciple(discipleType = "inner", realm = 9)
        val elder = createDisciple(discipleType = "inner", realm = 9).let { it.copy(isAlive = false,
            skills = it.skills.copy(teaching = 90)) }
        val bonus = DiscipleStatCalculator.calculateQingyunPeakCultivationSpeedBonus(
            disciple,
            qingyunPreachingElder = elder
        )
    ...
    fun `calculateQingyunPeakBonus - 执事传道加成`() {
        ...
        val master = createDisciple(discipleType = "inner",
            realm = 9).let { it.copy(skills = it.skills.copy(teaching = 85)) }
        val bonus = DiscipleStatCalculator.calculateQingyunPeakCultivationSpeedBonus(
            disciple,
    ...
    fun `calculateQingyunPeakBonus - 传道长老teaching120上限10percent`() {
        val disciple = createDisciple(discipleType = "inner", realm = 9)
        val elder = createDisciple(discipleType = "inner",
            realm = 9).let { it.copy(skills = it.skills.copy(teaching = 120)) }
        val bonus = DiscipleStatCalculator.calculateQingyunPeakCultivationSpeedBonus(
            disciple,
            qingyunPreachingElder = elder
        )
    ...
    fun `calculateQingyunPeakBonus - 传道长老teaching84每4点1percent`() {
        val disciple = createDisciple(discipleType = "inner", realm = 9)
        val elder = createDisciple(discipleType = "inner",
            realm = 9).let { it.copy(skills = it.skills.copy(teaching = 84)) }
        val bonus = DiscipleStatCalculator.calculateQingyunPeakCultivationSpeedBonus(
            disciple,
            qingyunPreachingElder = elder
        )
