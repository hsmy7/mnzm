...
class SettlementCoordinatorTest {
    ...
    // ============================================================
    // 7. 非焦点域热控分批 — computeNonFocusedBatch 逻辑
    // ============================================================

    /**
     * 复制 computeNonFocusedBatch 的热控分批逻辑：
     * - 首次结算（nonFocusedLastSettleMonth==0）→ batchMonths=1
     * - 常温 → batchSize=1
     * - shouldReduceWorkload → batchSize=6
     * - shouldEmergencySave → batchSize=12
     * - 未达批次阈值时最少处理1个月（永不跳过修炼）
     */
    private fun computeBatchMonths(
        nonFocusedLastSettleMonth: Int,
        currentAbsoluteMonth: Int,
        shouldReduceWorkload: Boolean,
        shouldEmergencySave: Boolean
    ): Pair<Int, Int> {
        // 返回 (newLastSettleMonth, batchMonths)
        if (nonFocusedLastSettleMonth == 0) {
            return Pair(currentAbsoluteMonth, 1)
        }
        val monthsSince = currentAbsoluteMonth - nonFocusedLastSettleMonth
        if (monthsSince <= 0) {
            return Pair(nonFocusedLastSettleMonth, 1)
        }
        val batchSize = when {
        ...
        return if (monthsSince >= batchSize) {
            Pair(currentAbsoluteMonth, monthsSince)
        } else {
            Pair(nonFocusedLastSettleMonth, 1)
        }
    }

    @Test
    fun `热控分批 - 首次结算batchMonths为1`() {
        val (lastMonth, batch) = computeBatchMonths(
            nonFocusedLastSettleMonth = 0,
            currentAbsoluteMonth = 13,
            shouldReduceWorkload = false,
            shouldEmergencySave = false
        )
        assertEquals(13, lastMonth)
        assertEquals(1, batch)
    }

    @Test
    fun `热控分批 - 常温每月结算batchMonths为1`() {
        val (lastMonth, batch) = computeBatchMonths(
            nonFocusedLastSettleMonth = 13,
            currentAbsoluteMonth = 14,
            shouldReduceWorkload = false,
            shouldEmergencySave = false
        )
        assertEquals(14, lastMonth)
        assertEquals(1, batch)
    }
    ...
    fun `热控分批 - 发热时每6月结算一次`() {
        val (lastMonth, batch) = computeBatchMonths(
            nonFocusedLastSettleMonth = 13,
            currentAbsoluteMonth = 19,  // 6个月后
            shouldReduceWorkload = true,
            shouldEmergencySave = false
        )
        assertEquals(19, lastMonth)
        assertEquals(6, batch)
    }

    @Test
    fun `热控分批 - 严重发热时每12月结算一次`() {
        val (lastMonth, batch) = computeBatchMonths(
            nonFocusedLastSettleMonth = 13,
            currentAbsoluteMonth = 25,  // 12个月后
            shouldReduceWorkload = true,
            shouldEmergencySave = true
        )
        assertEquals(25, lastMonth)
        assertEquals(12, batch)
    }

    @Test
    fun `热控分批 - 未达批次阈值时最少处理1个月`() {
        // 发热 batchSize=6，但只过了3月 → batchMonths=1（永不跳过修炼）
        val (lastMonth, batch) = computeBatchMonths(
            nonFocusedLastSettleMonth = 13,
            currentAbsoluteMonth = 16,  // 3个月后，未达6
            shouldReduceWorkload = true,
            shouldEmergencySave = false
        )
        assertEquals(13, lastMonth)  // lastSettleMonth 不更新
        assertEquals(1, batch)
    }

    @Test
    fun `热控分批 - monthsSince为0时batchMonths为1`() {
        val (lastMonth, batch) = computeBatchMonths(
            nonFocusedLastSettleMonth = 13,
            currentAbsoluteMonth = 13,  // 同月
            shouldReduceWorkload = false,
            shouldEmergencySave = false
        )
        assertEquals(13, lastMonth)
