...
class MissionSystemTest {
    ...
    fun `MissionDifficulty - 敌人境界范围正确且Min小于Max`() {
        MissionDifficulty.values().forEach { difficulty ->
            assertTrue(
                "enemyRealmMin(${difficulty.enemyRealmMin}) should be <= enemyRealmMax(${difficulty.enemyRealmMax}) " +
                    "for $difficulty",
    ...
    fun `COMBAT_RANDOM触发率递增`() {
        assertTrue(MissionTemplate.EXPLORE_ABANDONED_MINE.triggerChance < MissionTemplate.EXPLORE_ANCIENT_CAVE
            .triggerChance)
        assertTrue(MissionTemplate.EXPLORE_ANCIENT_CAVE.triggerChance < MissionTemplate.EXPLORE_ANCIENT_BATTLEFIELD
            .triggerChance)
        assertTrue(MissionTemplate.EXPLORE_ANCIENT_BATTLEFIELD.triggerChance < MissionTemplate.EXPLORE_CORE_BATTLEFIELD
            .triggerChance)
    }
