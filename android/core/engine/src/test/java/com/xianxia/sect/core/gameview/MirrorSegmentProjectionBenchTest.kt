...
class MirrorSegmentProjectionBenchTest {
    ...
    /**
     * R2.4/B09 G2 重测：**列级信封形状的消费侧基线**（每旬修炼热路径脏列——
     * cultivation/currentHp/currentMp，即 C++ 写屏障稳态产出形状）。
     *
     * ⚠️ 口径校正（B18-P3，施工卡勘误）：本测的"全脏投影臂"是**测试侧参考信封
     * 形状**（全字段行 + 生产 typed 行投影），**不是已删的生产回滚臂**——
     * B18-臂3 删的是非列级导出臂、臂2 删的是投影旗标，两处删除都不触及本测构造的
     * 两种信封形状。故两条硬性质**全保留、不弱化**：
     *   ① 列级臂不得明显慢于全脏参考（相对成本 sanity 门）；
