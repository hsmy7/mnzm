 ...
 *
 * Robolectric 环境必需：component 表中 String/枚举列
...
class ProductionProcessorTest : ProductionProcessorTestBase() {
    ...
    private fun simulateBatchFill(emptyCount: Int, candidates: MutableList<String>): Int {
        var filled = 0
        while (filled < emptyCount && candidates.isNotEmpty()) {
    ...
    // T4：仓库满时溢出转邮件且统计按实际入库（单 store 语义）
    // ═══════════════════════════════════════════════════════════════
}
