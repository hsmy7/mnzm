...
class RecruitService @Inject constructor(
    ...
    /**
     * batch-16：GameEngineCore Provider（native 臂取镜像服务用）。
     * Provider 断构造环（GameEngineCore→CultivationService→本类）；缺省
     * null 供测试无参构造——native 臂关闭走回退（既有测试零改动）。
     */
    ...
    companion object {
        ...
        fun processAutoRecruit(state: MutableGameState): Int {
            val filter = resolveAutoRecruitFilter(state) ?: return 0
            ...
            if (remaining <= 0) {
                DomainLog.i(TAG,
                    "processAutoRecruit: monthly limit reached ($currentCount/${GameConfig.RECRUIT_MONTHLY_LIMIT})")
                return 0
            }
            ...
            DomainLog.i(TAG,
                "processAutoRecruit: auto-recruited $recruited disciples " +
                    "(monthly $newRecruitCount/${GameConfig.RECRUIT_MONTHLY_LIMIT}), " +
                "${keepManual.size} left for manual review")
            return recruited
        }

        /**
         * 自动招募过滤器解析：惰性空闲 / 过滤器为空 /
         * 无效值剔除后为空三种情况返回 null（本轮无需招募）。
         */
        private fun resolveAutoRecruitFilter(state: MutableGameState): Set<Int>? {
            // 惰性检查：无符合条件的弟子时跳过
            if (RecruitLazyState.autoRecruitIdle) return null

            val rawFilter = state.gameData.autoRecruitSpiritRootFilter
            if (rawFilter.isNullOrEmpty()) return null
         ...
         *
         * 月变真相源切换后 autoRecruit 在 C++ 侧执行（C++ GameState
    ...
    fun refreshRecruitList(year: Int) {
        if (tryNativeRefreshRecruitList(year)) return
    ...
    /**
     * batch-16 native 臂：年度招募列表刷新直调点（读档自愈/开机兼容）经
     * RECRUIT_REFRESH_TX 复用 C++ year_settlement 候选生成链（SYSTEM 分区
     * 与 Kotlin 臂消费序逐位同源）。AUTHORITATIVE 年结由 C++
     * runYearSettlement 权威执行、不达本入口；年结回退臂（native 未就绪）
     * 时本臂各门控自然失配 → Kotlin 原路径（双实现并行契约）。
     *
     * 门控序：Provider 缺省（测试）→ flag → 镜像服务可空判空（handover
     * findings 13）→ **世界已导入守卫**（worldMapSects 非空 = 基线导入完成；
     * 开机路径 initializeWorldAndServices 在导入前调用、C++ 侧或持上次会话
     * 旧世界——必须走 Kotlin 臂保持既有行为）→ **差值门预检**（与 C++
     * kRecruitRefreshIntervalYears 同款：门内场景 C++ 链零生成而 Kotlin 臂
     * 会生成，预检防双臂分叉）。失败信封/降级返回 false → Kotlin 原路径。
     */
    private fun tryNativeRefreshRecruitList(year: Int): Boolean {
        val sync = resolveRecruitNativeSync() ?: return false
        val snapshot = stateStore.gameDataSnapshot
        if (snapshot.worldMapSects.isEmpty() ||
            year - snapshot.lastRecruitYear <
            CultivationEventProcessor.RECRUIT_REFRESH_INTERVAL_YEARS
        ) return false
        val data = GameEngineNativeOps.tryExecuteNative(
            stateSyncService = sync,
            actionId = ActionIds.RECRUIT_REFRESH_TX,
            paramsJson = params { put("year", year) }
        ) ?: return false
        DomainLog.d(
            TAG, "refreshRecruitList: native year=$year, generated " +
                "${data.long("generated")} new recruits " +
                "(auto ${data.long("autoRecruited")})"
        )
        // 新增弟子到列表 → 复位惰性（C++ 链已复位引擎侧同款瞬态；Kotlin
        // 内存侧同步复位——与回退臂同款残差）
        resetAutoRecruitIdle()
        RecruitLazyState.autoRejectIdle = false
        return true
    ...
    /**
     * native 臂公共门控解析（Provider 缺省 = 测试构造面 → flag → 镜像服务
     * 可空局部判空——测试 mock 未 stub stateSyncServiceRef 时 null → 回退，
     * 防调用点内非空检查 NPE，handover findings 13）。
     * @return 镜像服务；任一门控失配返回 null（回退 Kotlin 原路径）
     */
    private fun resolveRecruitNativeSync(): StateSyncService? {
        val coreProvider = gameEngineCoreProvider
        if (!NativeEngineFlag.authoritative || coreProvider == null) return null
        val sync: StateSyncService? = coreProvider.get().stateSyncServiceRef
        return sync
    }
    ...
    fun ageRecruitList(year: Int) {
        if (tryNativeAgeRecruitList()) return
        stateStore.update {
     ...
     */
    private fun tryNativeAgeRecruitList(): Boolean {
        val sync = resolveRecruitNativeSync() ?: return false
        val data = GameEngineNativeOps.tryExecuteNative(
            stateSyncService = sync,
            actionId = ActionIds.RECRUIT_AGE_TX,
            paramsJson = params { }
        ) ?: return false
        DomainLog.d(
            TAG, "ageRecruitList: native removed ${data.long("removed")}, " +
                "remaining ${data.long("remaining")} recruits"
        )
        return true
    }
}
