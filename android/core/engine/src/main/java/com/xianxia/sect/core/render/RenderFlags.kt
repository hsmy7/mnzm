 ...
 * - [buildingShadows]：建筑投影阴影（WP3）——默认关闭（产品决策移除建筑阴影，
 *   双后端同步消失）
 ...
 * - [renderScaleEnabled]：渲染分辨率缩放（平板省电；false → renderScale
 *   恒 1.0 直渲）
 * - [refreshRateDeclaration]：帧率↔刷新率联动声明（false → 仅
 *   ≤30fps 声明）
 *
 * ## 关闭语义
...
data class RenderFlags(
    val buildingShadows: Boolean = false,
