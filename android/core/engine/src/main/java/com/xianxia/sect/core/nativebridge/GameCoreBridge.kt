...
object GameCoreBridge {
     ...
     *
     * 端口总数前后对照：**+1**（注入仅初始化期一次，零每帧/每事务跨线）。
     *
     * 注入时机：`GameDataNativeBridge.injectFrom` —— native 库加载后、
     * 引擎结算前一次（幂等）。失败时 C++ 侧保留**头文件内联默认兜底**，
     * 不阻断引擎启动（与 `nativeSetGameConfig` 同语义）。
     */
    external fun nativeSetGameData(json: String)
