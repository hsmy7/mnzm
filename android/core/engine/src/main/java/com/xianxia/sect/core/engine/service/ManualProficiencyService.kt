...
class ManualProficiencyService @Inject constructor() {
     ...
     *   null 时内部线性扫描）
     */
    @Suppress("CyclomaticComplexMethod", "ReturnCount") // 卫语句密集的列级直读结算
    fun processManualProficiencySingle(
        ...
        // 批量模式下从 pending 读累积视图。
        // 用 containsKey 区分"pending 显式 null（移除计划）"与"pending 无该条目
        // （首次处理）"——两者混读旧 data 残留条目时，同一批量周期内
        // 弟子被处理两次（或调用方预置 null=移除）会使残留条目复活并叠加双倍增长。
