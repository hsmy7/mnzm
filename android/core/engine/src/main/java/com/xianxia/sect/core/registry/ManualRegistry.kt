...
class ManualRegistry : BaseTemplateRegistry<ManualDatabase.ManualTemplate>() {
    ...
    override fun loadTemplates(): Map<String, ManualDatabase.ManualTemplate> {
        ...
        if (!_isInitialized) {
            DomainLog.w(TAG, "Accessing templates before initialization. Return empty map.")
            return emptyMap()
        }
        ...
        return try {
            synchronized(initLock) {
                ...
                if (result.isSuccess) {
                    _isInitialized = true
                    markInitialized()
                    DomainLog.i(TAG, "ManualRegistry initialized successfully with ${getCount()} templates")
                } else {
                    DomainLog.e(TAG, "Failed to initialize ManualDatabase", result.exceptionOrNull())
                }
