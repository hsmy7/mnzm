...
class SavePipeline @Inject constructor(
    ...
    private fun startConsumer() {
        ...
        DomainLog.d(TAG, "SavePipeline consumer started")
    }
    ...
    fun enqueue(request: SaveRequest): Boolean {
        val offered = try {
            ...
            DomainLog.e(TAG, "Failed to enqueue save request for slot=${request.slot}", e)
            false
        }

        if (!offered) {
            DomainLog.w(TAG, "Save queue full, dropping save request for slot=${request.slot}, source=${request.source}")
        } else {
            DomainLog.d(TAG, "Save request enqueued: slot=${request.slot}, source=${request.source}")
        }
    ...
    private suspend fun consumeLoop() {
        for (request in saveChannel) {
            ...
            try {
                ...
                if (!result.success) {
                    DomainLog.w(TAG, "Save failed: slot=${result.slot}, source=${result.source}, error=${result.errorMessage}, timedOut=${result.timedOut}")
                }
            } catch (e: Exception) {
                DomainLog.e(TAG, "Unexpected error processing save request for slot=${request.slot}", e)
            } finally {
        ...
        val result = try {
            val monitoringResult = saveLoadCoordinator.executeSaveWithMonitoring(
                ...
                saveOperation = {
                    ...
                    val saveSucceeded = withTimeoutOrNull(timeoutMs) {
                        saveStorage.save(request.slot, saveSnapshot)
                    }
            ...
            if (monitoringResult.success) {
                DomainLog.d(TAG, "Save completed: slot=${request.slot}, source=${request.source}, duration=${durationMs}ms")
                SavePipelineResult(
                    success = true,
            ...
            val durationMs = System.currentTimeMillis() - startTime
            DomainLog.w(TAG, "Save cancelled for slot=${request.slot}: ${e.message}")
            SavePipelineResult(
                success = false,
                durationMs = durationMs,
                slot = request.slot,
            ...
            val durationMs = System.currentTimeMillis() - startTime
            DomainLog.w(TAG, "Save timed out for slot=${request.slot} after ${timeoutMs}ms")
            SavePipelineResult(
                success = false,
                durationMs = durationMs,
                slot = request.slot,
            ...
            val durationMs = System.currentTimeMillis() - startTime
            DomainLog.e(TAG, "Save execution failed for slot=${request.slot}: ${e.message}", e)
            SavePipelineResult(
                success = false,
                durationMs = durationMs,
                slot = request.slot,
    ...
    suspend fun waitForCurrentSave(timeoutMs: Long = 10_000L): Boolean {
        ...
        return try {
            ...
            DomainLog.w(TAG, "waitForCurrentSave interrupted", e)
            false
        }
    }
    ...
    suspend fun shutdownGracefully() {
        ...
        try {
            ...
            DomainLog.w(TAG, "Error waiting for consumer to finish during graceful shutdown: ${e.message}")
        }
        scope.cancel()
        DomainLog.d(TAG, "SavePipeline graceful shutdown completed")
    }
    ...
    fun shutdown() {
        ...
        consumerJob?.cancel()
        scope.cancel()
        DomainLog.d(TAG, "SavePipeline force shutdown completed")
    }
}
