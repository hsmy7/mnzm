...
class DiscipleBreakthroughHandler @Inject constructor(
    ...
    fun performBreakthrough(
        ...
        while (shouldContinue && d.realm > 0) {
            // 修为未满或非满血蓝：无需/不可突破
            if (d.cultivation < d.maxCultivation || !cultivationCore.isDiscipleFullHpMp(d, state)) break
        ...
        // 循环前置 break 路径两计数均为 0，未发生突破尝试时加成保留
    ...
    private fun attemptAutoPill(d: Disciple, pillTargetRealm: Int, state: MutableGameState,
        data: GameData): Pair<Double, Disciple?> {
        ...
        if (warehousePill != null) {
            // 逐颗扣减（quantity>1 减一保留，=1 移除）——
            // 整条移除（EntityStore.minus 按相等元素删除、忽略 quantity）
            // 会一次突破尝试吃掉整叠丹药。
        ...
        // 储物袋丹药兜底（同样逐颗扣减，非整条删除）
        val bestPill = d.equipment.storageBagItems
            .filter { it.itemType == ITEM_TYPE_PILL && it.effect?.pillType == "breakthrough" && it
                .effect?.targetRealm == pillTargetRealm }
    ...
    /** 突破字段精准写回 */
    private fun writeBackBreakthroughFields(tables: DiscipleTables, updatedDisciples: List<Disciple>) {
    ...
    private fun elderEffectiveComprehension(
        ...
        val eid = elderId.toIntOrNull()
        val elderEligible = eid != null && tables.isAlive[eid] == 1 && disciple.realm >= tables.realms[eid]
        if (!elderEligible || eid == null) return 0
        return allDisciples[elderId]?.getBaseStats()?.comprehension ?: tables.comprehensions[eid]
    }

    /** 长老悟性加成输入：弟子类型匹配且长老已任命时取有效悟性，否则 0 */
    private fun elderComprehensionFor(
        disciple: Disciple,
        tables: DiscipleTables,
        allDisciples: Map<String, Disciple>,
        elderId: String,
