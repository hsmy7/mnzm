    ...
    data class ManualProficiency(
        ...
        val progressToNextLevel: Double
            get() {
                ...
                return when (mastery) {
                    MasteryLevel.NOVICE -> (proficiency / thresholds.getValue(MasteryLevel.SMALL_SUCCESS)).coerceIn(0.0,
                        1.0)
                    MasteryLevel.SMALL_SUCCESS -> ((proficiency - thresholds.getValue(MasteryLevel
                        .SMALL_SUCCESS)) / (thresholds.getValue(MasteryLevel.GREAT_SUCCESS) - thresholds
                            .getValue(MasteryLevel.SMALL_SUCCESS))).coerceIn(0.0, 1.0)
                    MasteryLevel.GREAT_SUCCESS -> ((proficiency - thresholds.getValue(MasteryLevel
                        .GREAT_SUCCESS)) / (thresholds.getValue(MasteryLevel.PERFECTION) - thresholds
                            .getValue(MasteryLevel.GREAT_SUCCESS))).coerceIn(0.0, 1.0)
