...
class HpMpRecoveryService @Inject constructor() {
    ...
    fun recoverHpMpSingleColumn(
        ...
        val multiplier = phasesToSettle.toDouble()
        return settleColumnRecovery(tables, id, HpMpLevels(curHp, curMp, maxHp, maxMp), zones, multiplier)
    }

    /** 恢复映射打包：装备/功法/熟练度映射（热点循环共享构建） */
    private data class RecoveryMaps(
        val eqMap: Map<String, EquipmentInstance>,
        val manualMap: Map<String, ManualInstance>,
        val profMap: Map<String, ManualProficiencyData>
    )

    /** 恢复映射解析：入参为 null 时内部构建 */
    private fun resolveRecoveryMaps(
        state: MutableGameState,
        id: Int,
    ...
    private fun buildHpMpColumnInput(
        ...
        return DiscipleStatCalculator.HpMpColumnInput(
    ...
    /** 单列恢复写回：clamp/加成求值序不变，负值视为满保持不变 */
    /** HP/MP 当前与上限四元组 */
    private data class HpMpLevels(val curHp: Int, val curMp: Int, val maxHp: Int, val maxMp: Int)

    private fun settleColumnRecovery(
        tables: DiscipleTables,
        id: Int,
        levels: HpMpLevels,
        zones: RecoveryZones,
        ...
        val hpRecovery = zones.calculateRecovery(levels.maxHp, multiplier)
        val mpRecovery = zones.calculateRecovery(levels.maxMp, multiplier)
        val curHp = levels.curHp
        val curMp = levels.curMp
        val newHp = if (curHp < 0) curHp else (curHp + hpRecovery).coerceAtMost(levels.maxHp)
        val newMp = if (curMp < 0) curMp else (curMp + mpRecovery).coerceAtMost(levels.maxMp)
    ...
    fun applyMonthlyDurationDecay(tables: DiscipleTables, id: Int, focusedPhaseCount: Int = 0) {
        ...
        // 丹药效果衰减仅作用于 pillEffects 体系——
        // 旧 cultivationSpeedBonus 组件列已弃用，不参与衰减
