...
object WorldMapGenerator {
    ...
    @Suppress("UnusedParameter") // prefix: 语义形参：签名表达 API 决策域（调用点可读性与协议完整性优先），当前策略不消费
    private fun generateFallbackName(prefix: String, usedNames: MutableSet<String>): String {
        ...
        var name: String
        do {
            name = chars[rng.nextInt(chars.size)] + chars[rng.nextInt(chars.size)] + suffixes[rng.nextInt(suffixes
                .size)]
    ...
    private fun generateSectLevelAndDisciples(level: Int): SectLevelInfo {
        ...
        val disciples = distributeInitialRealms(maxRealm)
    ...
    private fun generateDisciplesForLevel(level: Int): Map<Int, Int> {
        ...
        return distributeInitialRealms(SectLevel.maxRealmForLevel(level))
    }

    /**
     * 初始 50 名弟子境界分布（generateSectLevelAndDisciples / generateDisciplesForLevel
     * 共享）：maxRealm 以上境界按权重分配，余数按权重降序逐名补齐。
     */
    private fun distributeInitialRealms(maxRealm: Int): MutableMap<Int, Int> {
        ...
        if (realmRange.isEmpty()) return disciples

        val weights = realmWeights(realmRange)
        val assigned = assignWeightedCounts(disciples, realmRange, weights, normalCount)
        distributeRealmRemainder(disciples, realmRange, weights, normalCount - assigned)
        return disciples
    }
...
// ==================== 初始境界分布纯函数（文件级——object 函数数受 TooManyFunctions 约束） ====================

/** 高境界权重表：9→3 / 8→2 / 7→2 / 其余→1 */
private fun realmWeights(realmRange: IntRange): Map<Int, Int> =
    realmRange.associateWith { realm ->
        when (realm) {
            9 -> 3
            8 -> 2
            7 -> 2
...
/** 按权重整除分配各境界人数，返回已分配总数 */
private fun assignWeightedCounts(
    disciples: MutableMap<Int, Int>,
    realmRange: IntRange,
    weights: Map<Int, Int>,
    normalCount: Int
): Int {
...
/** 权重整除后的余数按权重降序逐名补齐 */
private fun distributeRealmRemainder(
    disciples: MutableMap<Int, Int>,
    realmRange: IntRange,
    weights: Map<Int, Int>,
    remaining: Int
