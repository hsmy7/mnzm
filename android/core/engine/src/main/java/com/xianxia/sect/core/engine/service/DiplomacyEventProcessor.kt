...
class DiplomacyEventProcessor @Inject constructor(
    ...
    fun checkAllianceExpiry(year: Int) {
        ...
        val expiredAlliances = data.alliances.filter { year - it.startYear >= com.xianxia.sect.core.config.FavorConfig
            .ALLIANCE_DURATION_YEARS }

        if (expiredAlliances.isEmpty()) return

        stateStore.update { gameData = gameData.copy(
            alliances = gameData.alliances.filter { year - it.startYear < com.xianxia.sect.core.config.FavorConfig
                .ALLIANCE_DURATION_YEARS },
