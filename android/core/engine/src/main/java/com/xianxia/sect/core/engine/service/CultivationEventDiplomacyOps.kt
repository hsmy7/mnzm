...
/**
 * CultivationEventProcessor 侦察/外交域 Ops 扩展。
 */
...
internal fun CultivationEventProcessor.applyScoutInfoExpiry(state: MutableGameState, year: Int, month: Int) {
        ...
        updatedScoutInfo.forEach { (sectId, _) ->
            updatedDetails[sectId] = (updatedDetails[sectId] ?: SectDetail(sectId = sectId))
                .copy(scoutInfo = updatedScoutInfo[sectId] ?: SectScoutInfo())
        }
        ...
        state.gameData = state.gameData.copy(scoutInfo = updatedScoutInfo, worldMapSects = updatedWorldMapSects,
