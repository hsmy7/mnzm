...
class AutoBuyService @Inject constructor(
    ...
    fun executeAutoBuy(year: Int, month: Int, state: MutableGameState) {
        ...
        for (entry in data.autoBuyList) {
            ...
            val merchantItem = newMerchantItems.getOrNull(matchIdx)
                ?.takeIf { it.quantity > 0 }
                ?: continue
            val delta = purchaseMerchantItem(state, merchantItem, matchIdx, newMerchantItems)
            purchasedCount += delta.purchased
            skippedNoFunds += delta.skippedNoFunds
        }
    ...
    /** 单条目自动购买计数 */
    private data class AutoBuyEntryResult(val purchased: Int = 0, val skippedNoFunds: Int = 0)

    /** 单条目自动购买：仓库满/灵石不足跳过，成功则扣商人库存并入仓库 */
    private fun purchaseMerchantItem(
        state: MutableGameState,
        merchantItem: MerchantItem,
        matchIdx: Int,
        newMerchantItems: MutableList<MerchantItem>
    ): AutoBuyEntryResult {
        // 检查仓库容量
        if (!state.canAddToWarehouse(merchantItem)) {
            DomainLog.i(TAG,
                "自动购买跳过（仓库满）: ${merchantItem.name}")
            return AutoBuyEntryResult()
        }

        // 计算可买数量
        val buyQty = calculateBuyQuantity(
            state.gameData.spiritStones, merchantItem.price, merchantItem.quantity)
        if (buyQty <= 0) return AutoBuyEntryResult(skippedNoFunds = 1)
        val cost = merchantItem.price * buyQty

        // 通过钱包扣除灵石（检查扣除结果，失败则跳过该物品）
        val deductResult = spiritStoneWallet.deduct(state, cost, SpiritStoneGrade.LOW, SpiritStoneReason.Purchase,
            SpiritStoneSource.MerchantTrade)
        if (deductResult !is DeductResult.Success) return AutoBuyEntryResult(skippedNoFunds = 1)
        ...
        return AutoBuyEntryResult(purchased = 1)
    }
