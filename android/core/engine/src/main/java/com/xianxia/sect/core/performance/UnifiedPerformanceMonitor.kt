...
class UnifiedPerformanceMonitor @Inject constructor(
    ...
    private fun emitWarning(warning: PerformanceWarning) {
        synchronized(_performanceWarnings) {
            ...
            if (existingOfType.isEmpty() ||
                ...
                DomainLog.w(TAG, warning.message)
    ...
    private fun processFrameTime(frameTimeNs: Long) {
        ...
        if (frameTimeNs > FRAME_TIME_WARNING_THRESHOLD_NS) {
            ...
            if (frameTimeNs > FRAME_TIME_CRITICAL_THRESHOLD_NS) {
                DomainLog.w(TAG, "Critical frame drop: ${frameTimeNs / 1_000_000.0}ms (${droppedCount} frames dropped)")
    ...
    fun endOperationTimer(operation: String, startTime: Long) {
        ...
        synchronized(operationMetrics) {
            ...
            if (durationMs > 100) {
                DomainLog.w(TAG, "Slow operation: $operation took ${durationMs}ms")
    ...
    private fun processPerformanceSnapshot(snapshot: PerformanceSnapshot) {
        ...
        if (snapshot.isPerformanceCritical) {
            DomainLog.e(TAG, "CRITICAL PERFORMANCE: FPS=${String.format("%.1f", frameStats.fps)}, AvgFrameTime=${String.format("%.2f", frameStats.averageFrameTime / 1_000_000)}ms")
            ...
            DomainLog.w(TAG, "PERFORMANCE WARNING: FPS=${String.format("%.1f", frameStats.fps)}, AvgFrameTime=${String.format("%.2f", frameStats.averageFrameTime / 1_000_000)}ms")
    ...
    fun resetStats() {
        ...
        DomainLog.i(TAG, "Performance stats reset")
    }
    ...
    fun recordMonthlyEventEnd(
        ...
        if (durationMs > MONTHLY_EVENT_SLOW_THRESHOLD_MS) {
            DomainLog.w(TAG, "Slow monthly event: $operationName took ${durationMs}ms " +
    ...
    fun logMonthlyEventStats() {
        ...
        if (summaries.isEmpty()) {
            DomainLog.i(TAG, "No monthly event metrics recorded")
            return
        }
