...
class ExplorationService @Inject constructor(
    ...
    suspend fun consumePendingPatrolResults(): List<BattleResultUIData> {
        ...
        val defenseResults = stateStore.gameData.value.pendingPatrolBattleResults
        if (defenseResults.isNotEmpty()) {
            stateStore.update { gameData = gameData.copy(pendingPatrolBattleResults = emptyList()) }
        }
    ...
    suspend fun resolveBeastAttackPayTribute(beastLevelId: String) {
        ...
        stateStore.update {
            val deductResult = spiritStoneWallet.deduct(this, tribute, SpiritStoneGrade.LOW,
                SpiritStoneReason.BeastTribute, SpiritStoneSource.Internal)
            if (deductResult !is com.xianxia.sect.core.wallet.DeductResult.Success) return@update
    ...
    private suspend fun MutableGameState.resolveBeastFightInternal(
        ...
        if (defenders.isEmpty()) {
            ...
            battleLogs = (battleLogs + BattleLog(
                ...
                type = BattleType.PVE,
                attackerName = level.beastName.ifEmpty { "妖兽" },
                defenderName = if (targetSect.isPlayerSect) "玩家宗门" else targetSect.name,
                result = BattleResult.LOSE,
                details = loot.toDetailString(level.beastName)
            )).takeLast(GameConfig.Logs.MAX_BATTLE_LOGS)
            if (gameData.patrolBattleResultPopup) {
                gameData = gameData.copy(pendingPatrolBattleResults = gameData.pendingPatrolBattleResults + BattleResultUIData(
                    battleLogId = "", victory = false,
                    teamMembers = emptyList(), rewards = emptyList(),
                    lootedItems = loot.toRewardItems(), isBeastDefense = true
                ))
            }
        ...
        if (deadDefenders.isNotEmpty()) {
            disciples = DiscipleStatCalculator.applyGriefToRelatives(
                disciples, deadDefenders, gameData.gameYear
            )
        }
                ...
                worldMapSects = gameData.worldMapSects.map { sect ->
                    if (sect.id == targetSect.id) {
                        sect.copy(garrisonSlots = sect.garrisonSlots.map { slot ->
                            if (slot.discipleId in deadIds) GarrisonSlot(index = slot.index) else slot
                        })
                    } else sect
                }
            )
        ...
        if (result.victory) {
            disciples = disciples.map { d ->
                if (d.id in survivorIds && d.isAlive) {
                    ...
                    if (m.talentIds.any { id ->
                        ...
                        val r = rngManager.getRng(RngPartition.BATTLE).nextInt(17)
                        ...
                        when (r) {
                            ...
                            12 -> cb.basePhysicalAttack++; 13 -> cb.baseMagicAttack++
                            14 -> cb.basePhysicalDefense++; 15 -> cb.baseMagicDefense++
                            16 -> cb.baseSpeed++
                        }
                    }
                    m
            ...
            if (sr > 0) {
                spiritStoneWallet.add(this, sr.toLong(), SpiritStoneGrade.LOW,
                    SpiritStoneSource.Battle)
                allRewards.add(BattleRewardItem(
                    name = "灵石", quantity = sr, rarity = 1, type = "spiritStones"
                ))
            }
        } else {
            val loot = lootCalculator.computeLootPlan(gameData, this)
        ...
        val teamMems = result.battle.team.map { m ->
            BattleLogMember(id = m.id, name = m.name, realm = m.realm,
                realmName = m.realmName, hp = m.hp, maxHp = m.maxHp,
                mp = m.mp, maxMp = m.maxMp, isAlive = !m.isDead, portraitRes = m.portraitRes)
        }
        ...
        val rds = result.log.rounds.map { r ->
            BattleLogRound(roundNumber = r.roundNumber,
        ...
        battleLogs = (battleLogs + BattleLog(
            year = gameData.gameYear, month = gameData.gameMonth, type = BattleType.PVE,
            attackerName = level.beastName.ifEmpty { "妖兽" },
            defenderName = if (targetSect.isPlayerSect) "玩家宗门" else targetSect.name,
            result = if (result.victory) BattleResult.WIN else BattleResult.LOSE,
            teamMembers = teamMems, enemies = enems, rounds = rds,
            turns = result.turnCount,
            teamCasualties = teamMems.count { !survivorIds.contains(it.id) },
            beastsDefeated = if (result.victory) level.count else result.battle.beasts.count { it.isDead },
            details = if (result.victory) "成功抵御${level.beastName}袭击" else "被${level.beastName}击败，宗门受损"
        )).takeLast(GameConfig.Logs.MAX_BATTLE_LOGS)

        if (gameData.patrolBattleResultPopup) {
            val looted = if (result.victory) emptyList() else lootCalculator.computeLootPlan(gameData, this).toRewardItems()
            gameData = gameData.copy(pendingPatrolBattleResults = gameData.pendingPatrolBattleResults + BattleResultUIData(
                battleLogId = "", victory = result.victory,
                teamMembers = teamMems, rewards = allRewards,
                lootedItems = looted, isBeastDefense = true
            ))
        }
    ...
    suspend fun recallDiscipleFromTeam(teamId: String, discipleId: String): Boolean =
        explorationTeamManager.recallDiscipleFromTeam(teamId, discipleId)

    /** 完成探索（成功或失败） */
    suspend fun completeExploration(teamId: String, success: Boolean, survivorIds: List<String>) =
        explorationTeamManager.completeExploration(teamId, success, survivorIds)
