...
class DiscipleSlotCleanup @Inject constructor(
    ...
    private fun clearElderSlots(slots: ElderSlots, discipleId: String): ElderSlots {
        var updated = clearElderTitleFieldsTail(clearElderTitleFields(slots, discipleId), discipleId)
        ...
        updated = updated.copy(
            ...
            forgeDisciples = clearDirectDiscipleSlots(updated.forgeDisciples, discipleId),
            spiritMineDeaconDisciples = clearDirectDiscipleSlots(updated.spiritMineDeaconDisciples, discipleId)
        )

        return updated
    }

    /** 长老职务字段清理：副宗主至内门长老逐字段命中清空（序不变） */
    private fun clearElderTitleFields(slots: ElderSlots, discipleId: String): ElderSlots {
        var updated = slots
    ...
    /** 纳徒/青云传道长老清理：gate 扫描 10 长老字段清单对齐 */
    private fun clearElderTitleFieldsTail(slots: ElderSlots, discipleId: String): ElderSlots {
        var updated = slots

        // 纳徒长老也必须清理——双槽位可经此槽残留（gate 扫描覆盖 10 个长老字段，
        // 清理清单必须与之一一对应）
    ...
    /** 单个亲传弟子列表清理：命中弟子重置为空槽（mapNotNull 移除语义保留） */
    private fun clearDirectDiscipleSlots(
