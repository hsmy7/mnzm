...
object RedeemCodeManager {
    ...
    suspend fun validateCode(
        ...
        if (expireYear != null && expireMonth != null) {
            val totalExpireMonths = expireYear * 12 + expireMonth
        ...
        DomainLog.d(TAG, "Code validation passed: ${redeemCode.code}")
        return RedeemResult(
            success = true,
            message = "验证成功"
        )
    ...
    private fun recordUsedCode(code: String, record: UsedCodeRecord) {
        if (usedCodesRecord.size >= MAX_USED_CODES_RECORD_SIZE) {
            ...
            if (oldestKey != null) {
                ...
                DomainLog.w(TAG, "Used codes record at capacity ($MAX_USED_CODES_RECORD_SIZE), evicted oldest entry: $oldestKey")
            }
        }
