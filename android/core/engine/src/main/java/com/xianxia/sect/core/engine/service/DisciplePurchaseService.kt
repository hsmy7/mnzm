...
class DisciplePurchaseService @Inject constructor(
    ...
    private fun applyPurchaseDecisions(
        ...
        for (decision in decisions) {
            ...
            // 设计要求：先扣仓库再扣灵石，仓库失败则跳过购买（|| 短路保持原守卫序）
            val stocked = !state.hasWarehouseStock(item) || !state.addToWarehouseAndBag(item, dId, year, month)
            if (stocked) continue
    ...
    private fun processManualPurchases(
        ...
        for (item in listedItems.filter { it.type == ITEM_TYPE_MANUAL }) {
            ...
            for (ctx in (groupA + groupB)) {
                if (countPurchases(decisions, ctx.id, ITEM_TYPE_MANUAL) < MAX_MANUAL_PURCHASES) {
    ...
    private fun processEquipmentPurchases(
        ...
        for (item in listedItems.filter { it.type == ITEM_TYPE_EQUIPMENT }) {
            ...
            for (ctx in (groupA + groupB)) {
                if (countPurchases(decisions, ctx.id, ITEM_TYPE_EQUIPMENT) < MAX_EQUIPMENT_PURCHASES) {
    ...
    private fun processPillPurchases(
        ...
        for (item in pillItems) {
            ...
            for (ctx in interested) {
                if (countPurchases(decisions, ctx.id, ITEM_TYPE_PILL) < MAX_PILL_PURCHASES) {
    ...
    private fun <T> MutableGameState.appendDeductedBagItem(
        ...
        discipleTables.storageBagItems[discipleId] = bagItems +
            StorageBagItem(
                ...
                obtainedYear = year,
                obtainedMonth = month,
                // 袋条目持有数据（minRealm/slot/manualType 供取回重建）
    ...
    private fun MutableGameState.hasWarehouseStock(item: MerchantItem): Boolean {
        return when (item.type.lowercase(Locale.ROOT)) {
            ITEM_TYPE_EQUIPMENT -> hasEquipmentWarehouseStock(item)
            ITEM_TYPE_MANUAL -> hasManualWarehouseStock(item)
            // 设计要求：丹药按 grade 匹配合适品质
            ITEM_TYPE_PILL -> hasPillWarehouseStock(item)
            else -> false
        }
    }
}
...
private fun MutableGameState.hasManualWarehouseStock(item: MerchantItem): Boolean =
    manualStacks.any {
        !it.isLocked && it.name == item.name && it.rarity == item.rarity && it.quantity >= 1
    }

/** 丹药仓库库存判定：额外按品级显示名匹配（listing 无品级时默认"中品"） */
private fun MutableGameState.hasPillWarehouseStock(item: MerchantItem): Boolean =
    pills.any {
