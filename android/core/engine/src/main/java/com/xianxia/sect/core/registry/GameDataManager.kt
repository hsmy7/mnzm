...
object GameDataManager {
    ...
    fun initialize(context: Context): Boolean {
        ...
        return synchronized(initLock) {
            try {
                DomainLog.i(TAG, "Starting GameDataManager initialization...")
                ...
                DomainLog.i(TAG, "GameDataManager initialized successfully in ${elapsed}ms")
                DomainLog.i(TAG, "Memory usage: ${getMemoryUsage()} bytes")

                true
            } catch (e: Exception) {
                DomainLog.e(TAG, "Failed to initialize GameDataManager", e)
                false
            }
        }
    }
