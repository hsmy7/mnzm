...
class LevelGenerator @Inject constructor(
    ...
    private fun generateBeastLevel(
        ...
        val defValue = (stats.defense * layerMult * (beastConfig.defMod + defVariance)).toInt()
    ...
    private fun generateCaveLevel(
        ...
        val guardianName = "${guardianPrefixes[rng.nextInt(guardianPrefixes.size)]}" +
            "${guardianSuffixes[rng.nextInt(guardianSuffixes.size)]}"
        val realmName = GameConfig.Realm.getName(caveRealm)
        val caveName = "${caveNamePrefixes[rng.nextInt(caveNamePrefixes.size)]}$realmName" +
            "${caveNameSuffixes[rng.nextInt(caveNameSuffixes.size)]}"
