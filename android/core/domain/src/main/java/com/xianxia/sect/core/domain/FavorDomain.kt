...
/**
 * 好感度领域纯函数集合（关系查询/相识判定/更新维护面）。
 *
 * 纯计算函数族（送礼增长/交易价格/拒绝概率/偏好修正/衰减值）为文件级
 * 顶层函数——与本对象同文件同包，语义同为好感度领域纯函数。
 *
 * 纯函数意味着：
 * - 无副作用，可单元测试
 * - 输入相同 → 输出始终相同
...
fun calculateTradePriceMultiplier(
    ...
    val relation = FavorDomain.findFavor(sectRelations, playerSectId, sectId)
    return when {
