...
/**
 * 检查玉符（氪金货币）字段是否非法：
 * - [jadeSymbols] 为负 → 钳 0；持有上限 [GameConfig.Jade.MAX_HOLDING]
 *   （余量覆盖单日最大增量"计时 + 广告"，防止手改 Int.MAX_VALUE 后当日发放 Int 溢出回绕为负）
 ...
 *   `INTERVAL_MS - 1`（上限须严格小于发放阈值，否则"恰等于 10 分钟"的存档
...
object JadeSymbolNonNegativeRule : SaveValidationRule {
    ...
    override fun execute(data: SaveData, context: RuleContext): RuleOutcome {
        ...
        val fixedSymbols = gd.jadeSymbols.coerceIn(0, GameConfig.Jade.MAX_HOLDING)
