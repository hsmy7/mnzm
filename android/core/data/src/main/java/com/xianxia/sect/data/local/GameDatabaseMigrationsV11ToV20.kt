...
private const val TAG = "GameDatabase"
        ...
        internal val MIGRATION_12_13 = object : Migration(12, 13) {
            override fun migrate(db: SupportSQLiteDatabase) {
                rebuildGameDataWithoutIsGameStarted(db)
            }
        }

/**
 * v13 game_data 重建表 SQL（Room 自动生成 v13 schema，create-copy-drop-rename 目标结构）。
 * 供 MIGRATION_12_13（移除 isGameStarted 列）重建使用。
 */
private const val GAME_DATA_V13_CREATE_TABLE_SQL = """
...
private fun rebuildGameDataWithoutIsGameStarted(db: SupportSQLiteDatabase) {
    ...
    db.execSQL(GAME_DATA_V13_CREATE_TABLE_SQL)
    // 复制数据（排除 isGameStarted 列）
    // 使用紧凑单行 SQL 以避免 sqlite4java (Robolectric) 解析长多行 SQL 的问题
    val quotedCols = GAME_DATA_COLUMNS_V13.joinToString(", ") { "`$it`" }
    ...
    db.execSQL(
        "CREATE INDEX IF NOT EXISTS `index_game_data_gameYear_gameMonth` ON `game_data` (`gameYear`, `gameMonth`)"
    )
        ...
        internal val MIGRATION_13_14 = object : Migration(13, 14) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                if (!columnExists(db, "game_data", "spiritMineLastSettledMonth")) {
                    rebuildGameDataWithSpiritMineLastSettledMonth(db = db)
                } else {
        ...
        internal val MIGRATION_19_20 = object : Migration(19, 20) {
            override fun migrate(db: SupportSQLiteDatabase) {
                if (columnExists(db, "game_data", "gameSpeed")) {
                    ...
                    rebuildGameDataDroppingGameSpeed(db = db)
                } else {
                    // gameSpeed 已不存，只需加列
 ...
 */
private val GAME_DATA_COLUMNS_V13 = listOf(
...
private fun rebuildGameDataWithSpiritMineLastSettledMonth(db: SupportSQLiteDatabase) {
    ...
    """)
    val insertCols = GAME_DATA_COLUMNS_V13
    ...
    val selectParts = insertColumnExpressionBefore(
        columns = insertCols,
        beforeColumn = "librarySlots",
        literalExpression = "0 AS `spiritMineLastSettledMonth`"
    )
...
private fun rebuildGameDataDroppingGameSpeed(db: SupportSQLiteDatabase) {
    ...
    val insertCols = GAME_DATA_COLUMNS_V19
    ...
    val selectParts = insertColumnExpressionBefore(
        columns = insertCols,
        beforeColumn = "heavenly_trial_state",
        literalExpression = "'{}' AS `bloodRefinementPctTotals`"
    )
    val selectSql = selectParts.joinToString(", ")
 ...
 */
private fun insertColumnExpressionBefore(
    columns: List<String>,
    beforeColumn: String,
