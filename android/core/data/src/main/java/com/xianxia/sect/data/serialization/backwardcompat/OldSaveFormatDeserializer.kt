...
class OldSaveFormatDeserializer @Inject constructor(
     ...
     *
     * 旧 Serializable* 和新域类型字段名/结构基本相同（代表同一份数据），
     * 通过 JSON 作为中间格式做自动字段名映射；仅 equipment/manuals 两个字段
     * 结构不同（需拆分为 instances/stacks），逐一手工转换。
     */
    @Suppress("TooGenericExceptionCaught") // 防御兜底: 异常源跨IO/SDK不可枚举, 降级继续+日志留痕, 非静默吞噬
...
private val Boolean.json: kotlinx.serialization.json.JsonPrimitive get() = kotlinx.serialization.json
    .JsonPrimitive(this)
