...
17. **桌面操作优先级铁律** — 使用 computer use / 桌面自动化能力（`mcp__cua-driver-mcp__*`、`cua_driver_native__*`）时，必须严格按以下顺序选择操作方式，禁止颠倒：
    ...
    - **文字输入后必须回读校验**：`type_text` / `set_value` 返回的字符数只说明"按键已发出"，**不保证目标控件完整接收**（可能被截断、可能未聚焦到目标控件、可能落入错误元素）。输入完成后必须重新取窗口快照，**回读该控件的 `value` 并逐字/逐长度核对**；不符则清空重输，禁止在未核对的情况下继续提交。逐字符键入模式（`keystrokes`）优先用于富文本编辑器（Lexical/ProseMirror 等）
