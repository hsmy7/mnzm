...
2. **运营邮件扩展路径**：当前 `BuiltinMailConfig` 客户端内置（节日/白名单/QQ 群）→ 未来 RemoteConfig 推送（邮件属**凭据类发放**——领取失败保留凭据可重试，溢出语义见 rules/pr-review-checklist.md）
...
涉及以下变更时必须同步更新隐私政策**两个入口**（rules/design-plan-review.md 第零节 原则 5）：
