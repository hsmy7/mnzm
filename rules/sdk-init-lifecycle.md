...
> 2026-08-15 创建：源自两次真机回归教训（广告 SDK 重复初始化 / 退出游戏再登录卡死）。
> 2026-08-19 更新：登录/防沉迷验证流程重构为状态机（LoginFlowStateMachine），
> 补充"SDK 调用前必须就绪"与"状态收敛"两条核心原则。修改任何 SDK 初始化、登录、登出代码前**必须先读本文档**。
...
- `docs/login-flow-state-machine.md`：登录/防沉迷验证流程状态机重构方案（状态转移表唯一真相源）
