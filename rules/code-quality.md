...
| 打补丁式修复 | 特判分支/屏蔽症状/掩盖错误的 workaround 绕过根因（CLAUDE.md 第 15 条） | 根因修复：从症状追溯根因后用正确逻辑覆盖，验证症状不再复现 |
| 无效注释 | 纯复述代码（`// 加 1`） | 删除 |
...
- **根因修复**：任何修复都从症状追溯根因、用正确逻辑覆盖错误，禁止打补丁式绕过（CLAUDE.md 第 15 条）；修复后验证根因路径已被正确逻辑替代
