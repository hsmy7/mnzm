...
- **商人收购功能**：云游商人界面新增「收购」标签，可向商人出售仓库物品获取灵石
- 商人每年收购 1~6 种物品，收购价格 ±20% 随机浮动（精确到 0.1%）
- 收购确认弹窗：显示仓库数量、商人收购数量、最大可售数，支持数量调节和总价实时计算
- 收购数量归零后物品保留在列表中，按钮显示红色「不再收购」
- 仓库无对应物品时出售按钮灰色不可点击
...
- **活动系统**：主界面种植按钮下方新增「活动」入口，全屏活动界面支持活动列表与详情左右分栏展示
- **每日签到**：活动系统中上线每日签到功能，每周七天不同奖励（灵石/丹药/材料/储物袋），支持日历视图查看签到记录，月份自动重置

### 优化

- **GPU 全覆盖分级渲染**：覆盖 80+ SoC 型号（骁龙 8 Elite 至骁龙 4 系、天玑 9400 至天玑 6000、麒麟 9030 至麒麟 710A、Exynos 2400 至 1280、Tensor G4 至 G1、展锐虎贲），40+ 手机品牌自动适配最优画质
- **四级渲染策略**：地图精度（24×24/32×32/48×48）、渲染缩放（0.6×/0.8×/1.0×）、Bitmap 格式（RGB_565/ARGB_8888）、装饰密度（关/减半/完整）按 GPU 能力自动切换
- **Overdraw 大幅减少**：网格线低端设备从 ~100 条降至 4 条，放置/移动预览逐格矩形改为单矩形绘制，光环效果低端设备关闭
- **纹理质量分级**：低端设备贴图采样率提升 2×（减少解码内存 75%），旗舰设备贴图清晰度提升 2×
- **温控联动增强**：GPU 分级 + 热状态双因子决定渲染缩放，低端设备常温即降档
- **Android GameManager API 兜底**：Android 13+ 设备优先使用系统游戏性能等级，检测更精准

### 修复

- 修复 `SignInState` 使用 `Set<Int>` 违反 ProtoBuf 规范问题，改为 `List<Int>` 确保序列化安全
- 修复 `GpuTierDetector` EGL 资源泄漏风险，所有路径使用 try-finally 清理
- 补充 5 个新增模型类的 Compose Stability 声明，减少不必要重组

## [4.0.00] - 2026-06-08

### 重大更新

- **4.0 全新大版本**：代码架构全面重构，所有游戏系统模块化拆分
- **弟子详情重做**：修炼/装备/功法/丹药/战斗属性分区独立展示
- **存档系统重新设计**：全新数据结构，读写速度大幅提升
- **引擎性能优化**：游戏循环效率提升，月度结算速度翻倍
- **代码质量提升**：消除全部 !! / runBlocking / TODO 遗留，集成 Detekt + Lint 静态分析
