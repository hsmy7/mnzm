...
- **天枢殿尺寸：占地 18×19、精灵 18×20** — 在 18×12/16×22 基础上逐步调整为：占地宽 18（精灵宽 18 等宽，消除两侧地基空隙）、占地高 19、精灵高 20（高于占地 1 格，屋顶微出）；三源配置 + 测试镜像同步；`LAYOUT.footprints[9]` 最终 [18,19]（重新生成 SpriteAtlasDef.kt + footprint_table.h，C++ FP_W/FP_H 自动同步）；地砖映射双端 18×19 → 3（近方形 → 3×3 地砖拉伸）；旧档经 `fixupBuildingSizes` 自动改宽高并钳位坐标；KTX 图集内容不受影响（建筑槽位 rect 不变）；测试期望同步（`SpriteAtlasDefGeneratedTest` FOOTPRINT、`SoftwareCanvasBackendAtlasTest` floorTileIndex）
