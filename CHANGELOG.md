...
### 新增
- **邮件系统正式上线**：内置首封福利邮件「庆祝邮件系统上线」，含 3 个宝品储物袋

### 优化
- **邮件防刷移除**：删除 `claimed_mail_records` 审计表，读旧存档后邮件随存档回退为未领取状态，可重新领取。新游戏/重开后邮件自动重置
- **邮件UI完善**：标题/内容/按钮统一底色仅横线分隔。附件 FlowRow 多行换行。已领附件精灵图替换为绿色"已领"且名称数量不变。邮件列表显示已读/未读标记（灰/红）。未读红点改为按钮外部小圆点
- **精灵图覆盖**：测试邮件附件名称匹配精灵数据库（装备→精铁剑、材料→虎皮）。邮件物品卡片根据类型显示对应精灵（灵石/丹药/装备/材料/储物袋）
- **弟子阵亡优化**：阵亡弟子仅立绘区域覆盖红色"死亡"文字，名称和境界保持显示
- **修复问题**：储物袋堆叠显示（仓库数量不再固定显示1）、储物袋附件支持同品阶堆叠、新游戏初始化邮件加载
