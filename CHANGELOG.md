...
### 新增
- **邮件系统**：游戏主界面右上角新增邮件入口（红点角标显示未读数）。支持在线邮件拉取 + 内置邮件混合投递，附件含灵石、灵草、装备、丹药、材料、草药、种子、弟子八种类型。一键已读按钮同时完成全部标已读 + 自动领取所有附件。单封邮件支持查看详情并单独领取。30 天过期自动清理，过期未领附件直接丢失
- **附件领取防刷**：独立审计表 `claimed_mail_records`，与存档回退解耦。同一存档内不可通过反复读档重复领取，不同存档各自独立可领。容量不足时自动级联清理已读邮件和无附件邮件
- **数据库迁移 v20→v21**：新增 `mails` 表（14 字段 + 3 索引）和 `claimed_mail_records` 审计表
