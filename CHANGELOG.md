...
### 新增
- 仓库建筑：3x2格/1000灵石/无上限，半屏驻守界面，1个驻守弟子防盗战斗槽位
- 仓库容量系统：动态50+50×仓库数，超限物品直接遗失+弹窗通知
- 仓库驻守防盗：偷盗随机选仓库与驻守弟子1v1战力比拼，贼胜偷成贼败被捕，无驻守则偷成
- 百战通神天赋实装：PVE战后幸存弟子17属性中随机+1（10技能+7战斗基底，不受乘法影响）

### 变更
- 灵植阁/长老重构：长老移至天枢殿（炼丹|天工|灵植同行），灵植阁改为半径6格光环建筑
- 灵植阁1个灵植弟子(50/5+1%上限20%)仅光环内生效，长老(80/4+1%上限20%)全局生效，产量加成→成熟速度
- 灵植阁/采矿/锻造/炼丹/仓库移除建造上限，建造栏无限制建筑隐藏数量显示
- 灵植阁放置/移动显示绿色范围圈+范围内灵田绿色高亮，光环判定改为最近点距离
- 种子成熟时间按品级分档：凡品3年→天品120年，详情显示由月改为年
- 月俸忠诚改为每月±1（原每3次），矿工每月-1忠诚，入住住所每月+1忠诚
- 偷盗触发改为道德+忠诚双门槛(<30)，脱离改为忠诚概率判定，新增被偷盗提示框
- 招募界面拒绝/同意按钮自适应等宽，弟子卡详情按钮，自动关闭建造栏

### 修复
- safeDropColumns移除API级别判断，统一PRAGMA表重建修复部分设备DROP COLUMN崩溃
- 修复种植界面已种植种子库存耗尽后不显示
- 修复灵植弟子选择列表只显示内门弟子

### 优化
- SideEffect→LaunchedEffect消除主线程bitmap绘制，UI StateFlow采样降频减少Compose重组
