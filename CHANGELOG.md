...
- **测试** — 新增/扩展 8 类：RenderScalePolicyTest（20 用例：分级边界/compute 矩阵/COMPACT 恒 1.0/NaN 消毒/floorTo05）、FrameRateDeclarationPolicyTest（17 用例：60Hz 旧行为一致/120Hz 两档/异常面板/FIXED_SOURCE）、FrameSkipPolicyTest（8 用例五守卫矩阵）、SoftwareCanvasBackendRenderScaleTest（12 用例：缓冲尺寸/消毒/像素映射/重建/极小视口）、BatteryAwareControllerTest 重写（14 用例含省电模式组合）、ThermalMonitorTest +3（setTargetWorkDuration 转发/no-op/异常吞）、GameEngineCoreFpsPolicyTest +3（frameDurationNs 钳制/启动冒烟）、RenderBackendContractTest +1（物理像素契约基线）；GameViewModel 测试构造补 gpuTierDetector
