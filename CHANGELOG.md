...
- **S4（repo 写失败防静默）** — `assignDiscipleToProductionSlot` 目标槽 `updateSlot` Failure → `DomainLog.e` + 镜像回滚（用既有 `existingSlot` 快照还原）+ 跳过 `confirmAssign`；"清他处占用"循环中单槽 Failure → 仅日志不阻断主写
...
- **B1（R1 诊断）** — 点击端 else 分支 `b == null` 时记诊断日志（displayName/sectId/instanceId/坐标/宗门建筑数）；渲染端 `BUILDING_NAME_INDEX ?: 0` 兜底分支文件级 Set 去重记一次警告（防刷屏）；不做"猜最近建筑"类误导性兜底；顺手优化 `BuildingSpatialIndex.add` 两次 `findByDisplayName` 合并（行为不变）
