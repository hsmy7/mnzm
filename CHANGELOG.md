...
- **天枢殿再扩建：占地 18×12、精灵 16×22** — `BuildingFeatureBoot.kt` / `BuildingConfigService` / assets `config/buildings.json` 三源同步（gridWidth 12→18、gridHeight 6→12、spriteWidth 12→16、spriteHeight 8→22）；`LAYOUT.footprints[9]` [12,6]→[18,12]（重新生成 SpriteAtlasDef.kt + footprint_table.h，C++ FP_W/FP_H 自动同步）；地砖映射双端 12×6 → 18×12（`floorTileIndex` 模板 + `NativeBridge.cpp`）；旧档已建天枢殿经 `fixupBuildingSizes` 自动改宽高并钳位坐标；图集 KTX 内容不受影响（建筑槽位 rect 不变）；测试期望同步（`SpriteAtlasDefGeneratedTest` FOOTPRINT、`SoftwareCanvasBackendAtlasTest` floorTileIndex）
