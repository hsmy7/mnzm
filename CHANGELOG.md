...
### 修复
- **攻打被AI占领宗门直接胜利**：AI 占领宗门后忘记将 `survivingAttackers` 填入 `garrisonSlots` 变成守军（`applyAIAttackResult` 仅设置 `occupierSectId`），导致守军为空直接获胜。现改为占领时将攻打队伍存活弟子自动填入守军槽位，同时 `attackSect` 加入空守军 fallback 防御
