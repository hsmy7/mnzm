...
- **重构：弟子偷盗机制全面重做** — 不再纯百分比（1%~5%），改为境界基准×属性加成×随机波动（±20%）的灵石偷盗，上限总灵石10%、下限100灵石；新增仓库物品偷盗（材料/丹药/装备/功法），稀有度越高越容易被偷（加权随机）；新增隐匿vs守卫感知判定层（Sigmoid函数）；道德变化即时触发偷盗判定，不再等月度批量
- **调整：偷盗门槛改为仅看道德（<30）** — 忠诚不再拦截偷盗，仅影响偷盗后是否叛逃带走装备/功法
