...
- 月俸改为年俸：现有各境界弟子俸禄×12倍，每年结算一次，忠诚度按年俸发放/未发放增减
- 移除旧月薪结算系统，统一为年度结算路径
...
- Kotlin 2.0.21 → 2.2.20：K2 编译器全面优化，增量编译提速约 40%
- AGP 8.8.0 → 8.10.0：构建管线性能提升，资源处理优化
- KSP 同步升级至 2.2.20-2.0.4：注解处理更快更稳定
- Compose BOM 2025.05 → 2026.05：Compose 运行时库全年更新
- Detekt 基线清理 10%（2664 → 2399 条）：FunctionNaming 规则排除 @Composable 函数
- Gradle 配置缓存并行存储模板就位，远程构建缓存配置模板就位

## [4.1.08] - 2026-06-12

### 优化

- 全部游戏资源统一无损WebP格式：剩余47个PNG文件全部转为无损WebP，2个格式错标文件修复，平均体积减少30-40%，无损模式下画质逐像素一致

## [4.1.07] - 2026-06-12

### 新增

- 主界面宗门信息卡片右侧新增隐藏UI按钮，点击隐藏所有按钮和卡片，再次点击恢复显示，方便查看宗门地图全貌

## [4.1.06] - 2026-06-12

### 天道试炼通关奖励

天道试炼8关首通奖励系统上线。通关后地图右下角新增「通关奖励」按钮，点击可查看各关卡奖励详情。每关首次通关可领取丰厚奖励（灵石、储物袋、随机丹药/装备/功法）。领取后奖励卡片直接飞入播放动效，无需额外确认。

### 红点提醒

试炼任意关卡有可领取奖励时，主界面「活动」按钮、活动面板「天道试炼」卡片、奖励卡片右上角均显示红点提醒。每日签到今日可领取时同样显示红点，避免遗漏每日奖励。

### 每日签到流程优化

签到成功直接播放奖励飞入动画，移除中间的小屏确认弹窗，领取流程更流畅。

## [4.1.05] - 2026-06-12

### 天道试炼子关卡单独追踪

天道试炼每个大关卡的两个子关（Phase 1 妖兽战 / Phase 2 弟子战）现在独立追踪通关状态。打通 Phase 1 后即使退出，进度也会保留，下次可直接挑战 Phase 2。仅当两个子关均通关后该大关卡才算完成并解锁下一关。

### 战斗准备界面通关状态标签

子关图标下方新增红色「未通关」/ 绿色「已通关」文字，一眼区分各子关的通关状态。

## [4.1.04] - 2026-06-12

### 天道试炼战斗结算界面优化

- 用时文本与下方灰色横线间距调至5dp，横线高度1dp，布局更紧凑
- 确认按钮移至对话框底部居中，符合操作习惯

### 晕眩状态视觉提示

天道试炼战斗中，弟子或敌人受到眩晕/冰冻技能影响时，气血值位置显示红色「晕眩」文字，取代原HP数值，方便辨识控制状态。

### 修复：试炼挑战界面功法不显示

修复 `remember` 缓存导致试炼弟子功法列表在特定时机下为空且不再更新的问题，每次进入挑战界面均重新获取敌人功法数据。

## [4.1.03] - 2026-06-12

### 妖兽技能全面重设计

8种妖兽（虎/狼/蛇/熊/鹰/狐/龙/龟）技能从1-2个扩展至3-4个，共计32个技能覆盖18大技能类别。所有技能新增灵力消耗（15-35），遵循CD≥持续+1铁律。

- **虎妖（金·狂暴）**：猛虎下山（1.8x单体）→ 虎爪撕裂（0.9x二段）→ 虎啸（0.7x群体）→ 咆哮（团队物攻+20%）
- **狼妖（木·迅捷）**：狼群撕咬（0.6x三段）→ 疾风步（自身速度+30%）→ 围猎（0.5x群体）→ 狼王嚎（团队暴击+10%）
- **蛇妖（水·剧毒）**：毒牙（1.2x中毒8%）→ 毒雾（0.5x群体中毒4%）→ 缠绕（降速30%）→ 蜕皮新生（自愈25%HP）
- **熊妖（土·铁甲）**：震地（0.6x群体）→ 铁壁（自身物防+40%）→ 熊吼（嘲讽2回合）→ 坚韧熊躯（团队减伤20%）
- **鹰妖（金·神风）**：俯冲（2.0x单体）→ 鹰眼（自身暴击+20%）→ 旋风斩（0.8x群体）→ 天翔一闪（立即额外行动）
- **狐妖（火·幻魅）**：妖术（1.5x沉默）→ 狐火（1.2x灼烧5%）→ 魅惑（降物攻25%）→ 幻阵（团队伤害+20%）
- **龙妖（火·远古）**：龙息（0.8x群体魔法）→ 龙爪撕裂（1.6x物理）→ 龙威（团队双攻+25%双buff）→ 龙鳞护体（自身减伤20%）
- **龟妖（水·玄甲）**：缩壳（自身物防+50%）→ 水盾（团队法防+30%）→ 激流（0.5x群体）→ 龟甲术（团队护盾15%HP）

### BeastSkillConfig 模型扩展

新增9个字段匹配CombatSkill v2：治疗（百分比/固定值/类型）、护盾、行动提前、伤害分摊、伤害链接、多Buff列表、技能描述。

### 清理无用字段

移除 `TrialEnemyDef.beastSkills` 字段（从未被使用，技能始终来自BeastTypeConfig）。

## [4.1.02] - 2026-06-12

### 重大修复：天道试炼境界常量修正

天道试炼全部8关的敌人境界常量与GameConfig实际ID偏差1级，导致第一关本应为筑基的敌人显示为金丹，所有关卡难度整体偏高。

- 修正 `HeavenlyTrialConfig` 中7个境界常量：R_ZHUJI(7→8)、R_JINDAN(6→7)、R_YUANYING(5→6)、R_HUASHEN(4→5)、R_LIANXU(3→4)、R_HETI(2→3)、R_DACHENG(1→2)
- 试炼弟子与妖兽属性自动随境界修正同步调整

### 重大更新：天道试炼功法全面优化

全部8关24名试炼弟子功法重新配置，每名弟子满编6本最高品阶功法，技能协同最大化难度。

- **坦克（Tank）**：嘲讽+自身护盾+HP/物防强化+伤害分摊保护辅助+眩晕+心法——强制单体攻击打自己，分摊防AOE秒辅助
- **输出（DPS）**：4法术攻击模式+眩晕+心法——全法术专精迫玩家堆法防，Tank普攻物理威胁逼双防兼顾
- **辅助（Support）**：群体百分比回血+单体急救+团队双攻双防光环+速度/肉度光环+团队心法——双治疗+三光环维持铁三角不死
- 8关品阶按修正后境界逐级提升：凡品→灵品→宝品→玄品→地品→天品

### 优化：试炼界面长按查看装备/功法详情

- 试炼弟子装备卡片支持长按查看完整属性（物攻/法攻/物防/法防/速度/HP/MP/暴击率）
- 试炼弟子功法卡片长按改为 `ItemDetailDialog`，展示功法属性加成、技能详情（伤害/连击/Buff/治疗/护盾/分摊）、境界需求
- `ItemDetailDialog` 新增 `EquipmentTemplate` 和 `ManualTemplate` 两种详情类型

### 优化：试炼界面布局改进

- 信息区与挑战按钮区按9:1比例分配空间
- 装备/功法卡片改为4列网格布局，不足4个显示空白占位符
- 卡片下方文字标签移除（卡片自带名称），卡片高度回归自然尺寸
- 妖兽技能详情增强：展示伤害类型/倍率/连击/冷却/消耗 + Buff数值与持续回合

### 修复：试炼界面 ManualDatabase 未初始化崩溃

- `HeavenlyTrialService.buildDiscipleEnemy` 中 `getById()` 替换为 `allManuals[id]`，未初始化时安全返回空而非抛异常

## [4.1.01] - 2026-06-12

### 优化：奖励卡片动效全面重构

- 奖励卡片从屏幕正中央同一位置依次弹出，100ms间隔紧随其上，匀速上飘1.6秒消散，每张卡片带横线分隔
- 提取通用小屏界面组件 `RewardDisplayDialog`，统一每日签到、储物袋开启、战斗胜利、邮件领取的奖励展示
- 所有奖励场景改为先展示物品卡片小屏界面 → 确认后播放动画，不再被提示框遮挡
- 每日签到周六奖励从悟法丹×5改为20随机凡品草药
- 修复储物袋开启奖励动画不显示（对话框遮挡+selectedItem scope问题）
- 修复邮件领取灵虎血等妖兽材料不显示精灵图（getRewardSprite新增beastMaterial分支）
- 修复动画播放中途新入队卡片被清除的问题（drop(n)替代全量清空）
- 修复邮件领取无确认步骤直接播动画的问题，改用小屏界面展示

## [4.1.00] - 2026-06-11

### 新增：物品获得卡片动效

手动领取奖励时（邮件领取、每日签到、兑换码、战斗胜利、储物袋开启），屏幕中央会显示物品奖励卡片，展示物品图标、名称和数量。卡片以 100ms 间隔依次弹出，向上飘动 1.5 秒后消散，多卡片间有横线分隔。生产完成、巡逻塔等自动获得的奖励不弹出卡片。
