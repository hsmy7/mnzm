...
- **占地 = 建筑底座（宽 = 精灵宽：左右不压盖；深 = 底座进深）**：炼丹炉/问道塔/青云塔/巡视楼 占地深 3→**2**、血炼池 4×4→**4×3**，其余保持现状（院落/山丘/殿）。**只减不增**——缩小不会让旧档建筑越界/重叠，读档 `fixupBuildingSizes` 自动改尺寸且**不触发** `computeBuildingOverflowMigration` 的拆除退款；**天枢殿占地锁定 18×13 不变**（其"尺寸 ≠ 当前配置即判旧档遗留并拆除 + 1000 万灵石补偿"的迁移判据对尺寸变化极敏感，改动会误伤全部旧档天枢殿）。
