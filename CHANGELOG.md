...
- **天枢殿最终尺寸：占地 18×20、精灵 18×21** — 在 18×12/16×22 基础上继续调整：占地高 12→20、精灵宽 16→18（与占地等宽，消除两侧地基空隙）、精灵高 22→21；三源配置 + 测试镜像同步；`LAYOUT.footprints[9]` [18,12]→[18,20]（重新生成 SpriteAtlasDef.kt + footprint_table.h，C++ FP_W/FP_H 自动同步）；地砖映射双端 18×12→2 改为 18×20→3（近方形 → 3×3 地砖拉伸）；旧档经 `fixupBuildingSizes` 自动改宽高并钳位坐标；KTX 图集内容不受影响（建筑槽位 rect 不变）；测试期望同步（`SpriteAtlasDefGeneratedTest` FOOTPRINT、`SoftwareCanvasBackendAtlasTest` floorTileIndex）
