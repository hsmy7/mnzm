...
- **验证**：`GameEngineRoadOpsTest` 8 用例（成功立即回导且信封含扣后灵石/连续放置逐格回导/灵石不足 Blocked 零发送/占用 Blocked 零发送/删除回导/回导失败降级全量不崩溃/**镜像写入不吞玩家捕获**（架构守护）/可放置判定）· `GameViewModelRoadFeedbackTest` 6 用例（Blocked 提示/成功静默/删除提示/造价守卫）· C++ GTest 675/675（新增 `JsonCodecTest.RoadDataRoundTrip`/`GameDataRoadsRoundTrip`/`GameDataRoadsLenientMissingField`）· `StateSyncServiceReverseTest`/`DiffStateSyncTest`/`DiffAuthoritativeTickTest`/`RoadFacadeImplTest`/`RoadTilingTest`/`DiffRoadTest`/`DiffRoadComposeTest`（桌面 JNI 重建后）全绿 · app `GameStateStore` 系列 + engine `GameEngineCoordinationTest`/`BootSequenceControllerTest` + feature `GameViewModelTest` 回归全绿 · NDK externalNativeBuildRelease + compileReleaseKotlin + detekt 通过。
